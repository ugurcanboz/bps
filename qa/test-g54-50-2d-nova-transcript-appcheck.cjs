...
ok('release version is G54.50.2D',()=>assert(read('js/core/app-config.js').includes("var VERSION = 'G54.50.2D'")));
...
ok('new service worker cache invalidates previous UI',()=>{const sw=read('service-worker.js');assert(sw.includes("var VERSION = 'G54.50.2D-2026-07-12'"));assert(sw.includes("var CACHE_NAME = 'egt-trainer-g54-50-2d'"));});
console.log(`G54.50.2D Nova Transcript & Bootstrap App Check: ${n}/10 bestanden`);
