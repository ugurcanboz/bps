...
const ui=read('js/modules/guided-welcome-ui.js'),css=read('css/guided-welcome.css'),gate=read('js/modules/novura-auth-access.js'),index=read('index.html'),sw=read('service-worker.js');let n=0;function ok(name,fn){fn();n++;console.log('PASS',name)}
...
ok('legacy-auth-delegation',()=>{assert(gate.includes('NovuraGuidedWelcomeUI.show'));assert(gate.includes('openSheet'))});
