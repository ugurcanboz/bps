...
global.navigator={userAgent:'Node QA Phase33C'};
...
if(b2.phase!=='33C') errors.push('phase not 33C');
if(b2.lessons!==10) errors.push('B2 lessons invalid');
if(b2.expandedLessons!==10 || b2.starterLessons!==0) errors.push('B2 expansion split invalid');
if(b2.totalTasks!==430 || b2.normalTasks!==350 || b2.speakingTasks!==80) errors.push('B2 totals invalid');
Object.entries(b2.perLesson||{}).forEach(([id,row])=>{
  if(!row.ok || row.tasks!==43 || row.speakingTasks!==8 || !row.expandedContent) errors.push(id+': expanded invalid '+JSON.stringify(row));
});
...
if(diag.phase!=='33C') errors.push('diagnostics phase not 33C');
...
fs.writeFileSync(path.join(__dirname,'phase33c_node_snapshot_result.json'), JSON.stringify(result,null,2));
if(errors.length){ console.error(JSON.stringify(result,null,2)); process.exit(1); }
console.log('PASS phase33C snapshot', JSON.stringify(result.b2,null,2));
