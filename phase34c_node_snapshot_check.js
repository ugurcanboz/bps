...
global.navigator={userAgent:'Node QA Phase34C'};
...
if(c1.phase!=='34C') errors.push('phase not 34C');
if(c1.lessons!==10) errors.push('C1 lessons invalid');
if(c1.expandedLessons!==10 || c1.starterLessons!==0) errors.push('C1 expansion split invalid');
if(c1.totalTasks!==430 || c1.normalTasks!==350 || c1.speakingTasks!==80) errors.push('C1 totals invalid');
Object.entries(c1.perLesson||{}).forEach(([id,row])=>{
  if(!row.ok) errors.push(id+': '+JSON.stringify(row));
  if(row.tasks!==43 || row.speakingTasks!==8 || row.normalTasks!==35) errors.push(id+' counts invalid: '+JSON.stringify(row));
});
...
if(!diag.c1Content || !diag.c1Content.ok || diag.phase!=='34C') errors.push('diagnostics invalid');
...
fs.writeFileSync(path.join(__dirname,'phase34c_node_snapshot_result.json'), JSON.stringify(result,null,2));
if(errors.length){ console.error(JSON.stringify(result,null,2)); process.exit(1); }
console.log('PASS phase34C snapshot', JSON.stringify(result.c1,null,2));
