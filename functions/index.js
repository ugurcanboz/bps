...
const bootstrapPolicy=require('./bootstrap-policy');
...
// Der einmalige Bootstrap ist zusätzlich durch Firebase Auth, einen starken Einmalcode,
// Rate-Limit und einen globalen Firestore-Lock geschützt. Nur diese eine Funktion darf
// vor der App-Check-Ersteinrichtung aufgerufen werden. Alle übrigen privilegierten
// Funktionen bleiben weiterhin App-Check-pflichtig.
const BOOTSTRAP_CALL_OPTIONS={enforceAppCheck:false,cors:ALLOWED_ORIGINS};
const db=getFirestore();
const auth=getAuth();

// Einmaliger Systemadmin-Bootstrap. Der Quellcode enthält nur den SHA-256-Digest,
// nie den Klartextcode. Nach erfolgreichem Claim sperrt Firestore den Vorgang global.
const DEFAULT_BOOTSTRAP_CODE_SHA256='4090e984f649cb0639c91f8b832e00962f5654b1392cfd136334109ba7097336';
const BOOTSTRAP_CODE_SHA256=String(process.env.NOVURA_BOOTSTRAP_CODE_SHA256||DEFAULT_BOOTSTRAP_CODE_SHA256).trim().toLowerCase();
...
exports.bootstrapAdmin=onCall(BOOTSTRAP_CALL_OPTIONS,async request=>{
  const diagnosticId=crypto.randomBytes(6).toString('hex');
  ...
  function safeErrorInfo(error){
    return {
      ...
      message:policy.sanitizeText(error&&error.message||error||'Unbekannter Fehler',500),
      stage,
      diagnosticId
    };
  }
  function normalizedBootstrapError(error){
    if(error instanceof HttpsError) return error;
    const info=safeErrorInfo(error);
    const haystack=`${info.code} ${info.message}`.toLowerCase();
    logger.error('bootstrapAdmin unexpected failure',Object.assign({},info,{stack:policy.sanitizeText(error&&error.stack||'',2000)}));

    if(/firestore|failed[_ -]?precondition|database.*(not|nicht).*(exist|ready|bereit)|api.*(disabled|deaktiviert)|not[_ -]?found/.test(haystack)){
      return new HttpsError(
        'failed-precondition',
        'Die Firestore-Datenbank ist für den Admin-Bootstrap noch nicht bereit. Firestore im Firebase-Projekt aktivieren und Firestore sowie Functions erneut deployen.',
        {diagnosticId,stage,action:'enable-firestore'}
      );
    }
    if(/permission[_ -]?denied|insufficient.*permission|iam/.test(haystack)){
      return new HttpsError(
        'permission-denied',
        'Der Firebase-Functions-Dienst hat nicht die nötigen Rechte für Auth oder Firestore.',
        {diagnosticId,stage,action:'check-service-account'}
      );
    }
    if(/deadline|timeout|unavailable|econn|network|socket/.test(haystack)){
      return new HttpsError(
        'unavailable',
        'Firebase war beim Admin-Bootstrap vorübergehend nicht erreichbar. Bitte kurz warten und erneut versuchen.',
        {diagnosticId,stage,action:'retry'}
      );
    }
    return new HttpsError(
      'internal',
      'Der Admin-Bootstrap konnte serverseitig nicht abgeschlossen werden.',
      {diagnosticId,stage}
    );
  }
  ...
  try{
    const actor=requireAuth(request);
    if(!actor.uid||actor.isAnonymous) fail('unauthenticated','Eine normale Firebase-Anmeldung ist erforderlich.');
    const data=request.data||{};
    const courseId=String(data.courseId||'course_2026_gk').trim();
    const nickname=policy.sanitizeText(data.nickname||actor.email&&actor.email.split('@')[0]||'Administrator',40);
    const suppliedCode=String(data.bootstrapCode||'');
    if(nickname.length<3) fail('invalid-argument','Nickname braucht mindestens 3 Zeichen.');

    stage='rate-limit';
    await rateLimit(actor,'bootstrapAdmin',5,600);
    if(!bootstrapPolicy.codeMatches(suppliedCode,BOOTSTRAP_CODE_SHA256)) fail('permission-denied','Bootstrap-Code ist ungültig.');

    lockRef=db.doc('system/bootstrapAdmin');
    const learnerRef=db.doc(`courses/${courseId}/learners/${actor.uid}`);

    stage='existing-admin-check';
    const existingAdmins=await db.collection(`courses/${courseId}/learners`).where('role','==','admin').limit(1).get();
    if(!existingAdmins.empty && existingAdmins.docs[0].id!==actor.uid) fail('already-exists','Ein Systemadministrator ist bereits eingerichtet.');

    stage='bootstrap-lock';
    const now=Date.now();
    const lockResult=await db.runTransaction(async tx=>{
      const lockSnap=await tx.get(lockRef);
      const lock=lockSnap.exists?lockSnap.data()||{}:{};
      if(lock.status==='completed'){
        if(String(lock.completedByUid||'')===actor.uid) return {mode:'resume-completed'};
        fail('already-exists','Der Admin-Bootstrap wurde bereits abgeschlossen.');
      }
      const verdict=bootstrapPolicy.lockAllowsClaim(lock,actor.uid,now);
      if(!verdict.ok){
        if(verdict.reason==='completed') fail('already-exists','Der Admin-Bootstrap wurde bereits abgeschlossen.');
        fail('aborted','Ein anderer Bootstrap-Vorgang läuft bereits.');
      }
      tx.set(lockRef,{
        schema:'novura-admin-bootstrap-v1',status:'claiming',claimedByUid:actor.uid,
        claimedByEmail:actor.email||'',courseId,claimStartedAt:FieldValue.serverTimestamp(),
        claimExpiresAt:now+5*60*1000,updatedAt:FieldValue.serverTimestamp()
      },{merge:true});
      return {mode:'claim'};
    });
    lockMode=lockResult&&lockResult.mode||'claim';

    stage='auth-claims';
    const user=await auth.getUser(actor.uid);
    const existing=user.customClaims||{};
    previousClaims=existing;
    const previousCourseIds=Array.isArray(existing.courseIds)?existing.courseIds:[];
    const courseIds=Array.from(new Set([...previousCourseIds,courseId]));
    await auth.setCustomUserClaims(actor.uid,mergeClaims(existing,{
      role:'admin',admin:true,teacher:false,courseIds,assignedGroups:[],profileId:actor.uid,
      bootstrapAdmin:true
    }));
    claimsApplied=true;

    const profile={
      profileId:actor.uid,id:actor.uid,firebaseUid:actor.uid,email:actor.email||'',
      nickname,displayName:nickname,alias:nickname,role:'admin',accessRole:'admin',
      participantRole:'admin',groupId:'ADMIN',courseId,status:'active',blocked:false,
      createdVia:'one-time-admin-bootstrap',updatedAt:FieldValue.serverTimestamp()
    };

    stage='profile-and-lock-commit';
    await db.runTransaction(async tx=>{
      const lockSnap=await tx.get(lockRef);
      const lock=lockSnap.exists?lockSnap.data()||{}:{};
      const completedBySameUser=lock.status==='completed'&&String(lock.completedByUid||'')===actor.uid;
      const claimedBySameUser=String(lock.claimedByUid||'')===actor.uid;
      if(lock.status==='completed'&&!completedBySameUser) fail('already-exists','Der Admin-Bootstrap wurde bereits abgeschlossen.');
      if(!completedBySameUser&&!claimedBySameUser) fail('aborted','Bootstrap-Sperre gehört nicht zu diesem Konto.');
      ...
      tx.set(lockRef,{
        schema:'novura-admin-bootstrap-v1',status:'completed',completed:true,
        completedByUid:actor.uid,completedByEmail:actor.email||'',courseId,
        completedAt:lock.completedAt||FieldValue.serverTimestamp(),claimExpiresAt:null,
        lastDiagnosticId:diagnosticId,updatedAt:FieldValue.serverTimestamp()
      },{merge:true});
    });
    bootstrapCommitted=true;

    // Das Sicherheits-Audit darf einen bereits erfolgreich gesetzten Admin nicht wieder
    // zurückrollen. Ein Auditfehler wird protokolliert und kann später repariert werden.
    stage='security-audit';
    const bootstrapActor=Object.assign({},actor,{role:'admin'});
    try{
      await audit(courseId,bootstrapActor,'bootstrapAdmin',actor.uid,null,{role:'admin',bootstrapCompleted:true,diagnosticId});
    }catch(auditError){
      logger.error('bootstrapAdmin audit failed after successful commit',Object.assign({},safeErrorInfo(auditError),{committed:true}));
    }

    return {
      ok:true,role:'admin',courseId,
      profile:{...profile,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},
      refreshToken:true,bootstrapCompleted:true,resumed:lockMode==='resume-completed',
      emailVerificationRequired:actor.emailVerified!==true
    };
  }catch(error){
    // Nur vor dem finalen Firestore-Commit werden bereits gesetzte Claims kompensierend
    // zurückgerollt. Nach dem Commit ist der Bootstrap idempotent und bleibt gültig.
    if(!bootstrapCommitted&&claimsApplied){
      await auth.setCustomUserClaims(request.auth&&request.auth.uid,previousClaims||{}).catch(rollbackError=>{
        logger.error('bootstrapAdmin claim rollback failed',{diagnosticId,stage,message:String(rollbackError&&rollbackError.message||rollbackError)});
      });
    }
    if(!bootstrapCommitted&&lockRef&&lockMode==='claim'){
      await lockRef.set({
        status:'failed',failedByUid:request.auth&&request.auth.uid||'',lastDiagnosticId:diagnosticId,
        lastErrorStage:stage,claimExpiresAt:0,updatedAt:FieldValue.serverTimestamp()
      },{merge:true}).catch(()=>{});
    }
    throw normalizedBootstrapError(error);
  }
});
