...
exports.bootstrapAdmin=onCall(BOOTSTRAP_CALL_OPTIONS,async request=>{
  ...
  let previousClaims=null; let claimsApplied=false;
  try{
    const user=await auth.getUser(actor.uid);
    const existing=user.customClaims||{}; previousClaims=existing;
    ...
    await db.runTransaction(async tx=>{
      ...
      if(lock.status==='completed'){
        if(String(lock.completedByUid||'')!==actor.uid) fail('already-exists','Der Admin-Bootstrap wurde bereits abgeschlossen.');
        return;
      }
      if(String(lock.claimedByUid||'')!==actor.uid) fail('aborted','Bootstrap-Sperre gehört nicht zu diesem Konto.');
      ...
      tx.set(lockRef,{
        ...
        completedAt:FieldValue.serverTimestamp(),claimExpiresAt:null,updatedAt:FieldValue.serverTimestamp()
      },{merge:true});
    });
    const bootstrapActor=Object.assign({},actor,{role:'admin'});
    await audit(courseId,bootstrapActor,'bootstrapAdmin',actor.uid,null,{role:'admin',bootstrapCompleted:true});
    return {ok:true,role:'admin',courseId,profile:{...profile,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()},refreshToken:true,bootstrapCompleted:true,emailVerificationRequired:actor.emailVerified!==true};
  }catch(error){
    // Auth und Firestore sind getrennte Systeme. Falls nach Claim-Vergabe etwas fehlschlägt,
    // werden die vorherigen Claims kompensierend wiederhergestellt.
    if(claimsApplied){
      await auth.setCustomUserClaims(actor.uid,previousClaims||{}).catch(()=>{});
    }
    // Ein fehlgeschlagener Claim wird für dasselbe Konto wiederholbar, für andere aber nicht dauerhaft blockierend.
    await lockRef.set({status:'failed',failedByUid:actor.uid,lastError:policy.sanitizeText(error&&error.message||error,300),claimExpiresAt:0,updatedAt:FieldValue.serverTimestamp()},{merge:true}).catch(()=>{});
    throw error;
  }
});
