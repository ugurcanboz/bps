...
## Phase 38C.12B – Academy-Hartmodus Hörprüfung final
Status: abgeschlossen. Hörtextquelle validiert, Browser-TTS/Hilfsmodus getrennt, Transkript nur als markierter Fallback, Ergebnis speichert helperMode/audioMode. Nächster Schritt: Phase 38C.13 Grammatik & Sprachbausteine.
...
Ziel:
- B1 Academy-Hartmodus als vollständige 5-teilige Prüfung validieren.
- Nach Grammatik-Einbau sicherstellen, dass QA und Finale Bewertung wirklich alle 5 Pflichtteile nutzen.

Umgesetzt:
- B1-QA-Sessions korrigiert: `partOrder` enthält jetzt Lesen, Hören, Grammatik, Schreiben, Sprechen.
- Neue QA-Szenarien ergänzt: schwache Grammatik und Hör-Hilfsmodus.
- `validateB1HardmodeTotalQa()` ergänzt.
- Finale Bewertung markiert Hören-Hilfsmodus im Ergebnisbericht.
- Shell-Diagnose auf Phase 38C.14 aktualisiert.
- Testseite `tests_phase38c14_b1_hardmode_gesamt_qa.html` ergänzt.

Wichtig:
- Keine Prüfungsfreigabe.
- Kein offizielles Zertifikat.
- Academy-Hartmodus bleibt bewusst härter als Standardprüfungen.

Nächster Schritt:
- Phase 38D.2 – B2 Lesen hart ausbauen, jetzt auf sauberem B1-Hardmode-Fundament.


## Phase 38D.2 – B2 Lesen hart
- B2 Reading Pool von 3 auf 8 Varianten erweitert.
- Jede Lesevariante enthält 2 Texte und 8 harte Fragen.
- Fokus: Hauptaussage, Detailverständnis, Autorabsicht, indirekte Aussagen, Argumentationsstruktur, Wortbedeutung, Distraktoren.
- Testseite: tests_phase38d2_b2_lesen_hart.html
- Keine Standard-Simulation: Academy-Hardmode bleibt Leitprinzip.
...
### Ziel
Der äußere Prüfungsaufbau bleibt im Academy-Hartmodus einheitlich, aber A1, B1, B2, C1 und C2 müssen in Inhalt, Umfang, Grammatik, Bewertung und Punktdeckelung deutlich getrennt sein.

### Änderungen
- Neue Datei `data/language-level-difficulty-rules.js` erstellt.
- A1–C2 erhalten eigene Niveauprofile mit:
  - Textlängen
  - Schreib-/Sprechumfang
  - Grammatikthemen
  - Skill-Fokus
  ...
  - Ablehnungsgründen
  - Hard-Caps.
- `js/modules/language-exam-engine.js` nutzt die Profile für lokale Bewertung und Level-Caps.
- B2+ verlangt jetzt sichtbar:
  - Argumentationsansatz
  - eigene Position
  - Pro/Contra oder Abwägung
  - konkrete Beispiele
  - niveaupassende Strukturmarker/Konnektoren.
- Groq-Prüferrolle wird dynamisch pro Level formuliert, nicht mehr nur B1.
- `js/modules/language-exam-shell.js` zeigt Levelprofile direkt in den Levelkarten.
- Neue QA-Testseite `tests_phase38d2b_level_garantie.html`.

### Garantietest
- A1-Schreibantwort in B2-Prüfung fällt durch.
- gültige A1-Antwort kann A1 bestehen.
- strukturierte B2-Antwort kann B2 bestehen.
- B2-Schreibumfang ist mindestens fünfmal höher als A1-Mindestumfang.

### Nächster Schritt
Phase 38D.3 – B2 Hören hart ausbauen.

---

## Phase 38D.3 – B2 Hören hart

### Status
Abgeschlossen.

### Ziel
B2-Hören soll sich deutlich von A1/B1 unterscheiden und nicht nur als gleiches Prüfungsfenster mit anderen Texten wirken. Die Schwierigkeit wird über längere Hörtexte, Sprecherpositionen, indirekte Kritik, Bedingungen, Zielkonflikte und härtere Ablenker abgesichert.

### Änderungen
- `data/language-b2-exam-pilot.js` aktualisiert.
- B2 Listening Pool von 3 auf 8 Varianten erweitert.
- Jede Hörvariante enthält 1 längeren Hörtext und 8 Fragen.
- PassScore für B2-Hören auf 76 gesetzt.
- Hörmodus bleibt Academy-Hartmodus:
  - maximal zwei Hörvorgänge
  - Transkript verborgen
  - Transkript nur als markierter Hilfsmodus
  - Antworten erst nach Hören.
- Shell-Diagnose auf Phase 38D.3 aktualisiert.
- Neue QA-Testseite `tests_phase38d3_b2_hoeren_hart.html` ergänzt.
...
### Nächster Schritt
Phase 38D.4 – B2 Grammatik & Sprachbausteine hart ausbauen.


---

## Phase 38D.4 – B2 Grammatik & Sprachbausteine hart

Status: abgeschlossen.

Umgesetzt:
- B2-Grammatikpool von 1 auf 8 Varianten erweitert.
- 64 objektive B2-Sprachbausteine-Aufgaben integriert.
- Themen: Konnektoren, Textkohärenz, Passiv, indirekte Rede, Konjunktiv II, Präpositionen, Kasus, Nominalisierung, Satzstellung, Adjektivendungen, Partizipialattribute, Register und Fehlerkorrektur.
- B2-PassScore im Grammatikteil auf 78 % gesetzt.
- Neue QA-Testseite ergänzt: tests_phase38d4_b2_grammatik_hart.html.
- Keine Groq-Mehrbelastung: objektive Grammatik bleibt lokal auswertbar.

Nächster Schritt: Phase 38D.5 – B2 Schreiben hart ausbauen.


## Phase 38D.5 · B2 Schreiben hart
- B2-Schreibpool von 3 auf 8 Varianten erweitert.
- Schreibaufgaben auf 180–280 Wörter gesetzt.
- Strenge Pflichtpunkte, Strukturvorgaben, Registeranforderungen und Hard-Fail-Hinweise ergänzt.
- B2-Kombinationen auf 12288 erhöht.
- QA-Test `tests_phase38d5_b2_schreiben_hart.html` ergänzt.
