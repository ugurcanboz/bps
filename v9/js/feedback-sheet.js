/* BPS-Trainer V9.0.3 · Feedback System Component
...
(function () {
  ...
  const FEEDBACK_OWNER_EMAIL = "ugurcan.boz@googlemail.com";
  // Formspree uses a public form endpoint in this format: https://formspree.io/f/YOUR_FORM_ID
  // IMPORTANT: the recipient email must be configured/verified in the Formspree dashboard.
  const FORMSPREE_FORM_ID = "66207083f10340c8927161f5de24e13d";
  const FEEDBACK_ENDPOINT = `https://formspree.io/f/${FORMSPREE_FORM_ID}`;
  const ACCESS_KEY = ""; // Not required for Formspree endpoints
  ...
  async function handleFeedbackSubmit(event) {
    ...
    if (currentContext === "bug") {
      payload = {
        ...
        name: nameVal,
        email: emailVal,
        subject: $('fbSubject') ? $('fbSubject').value.trim() : "Bug in Aufgabe",
        message: descVal,
        ...gatheredMetadata
      };
    } else {
    ...
    // Formspree-compatible system fields.
    // _subject controls the email subject line in Formspree notifications.
    payload._subject = currentContext === "bug"
      ? `[BPS-Trainer Bug] ${payload.subject || 'Aufgabenfehler'}`
      : `[BPS-Trainer Feedback] ${payload.feedbackType || 'App-Feedback'}`;
    payload._format = "plain";
    payload.ownerEmail = FEEDBACK_OWNER_EMAIL;
    if (emailVal) {
    ...
    try {
      ...
      const mailtoLink = `mailto:${FEEDBACK_OWNER_EMAIL}?subject=${mailtoSubject}&body=${mailtoBody}`;
