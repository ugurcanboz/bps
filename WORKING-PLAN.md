# START HERE – G54.43.8K

Aktueller Stand: **G54.43.8K – Admin Tabbar Gesture Scroll + Logout Separation Fix**

- Admin-Tabbar ist mobil leichter horizontal scrollbar.
- Logout ist mobil von der Tabbar getrennt.
- Versionen/QA auf G54.43.8K synchronisiert.
...
## G54.43.8D – QA Bubble Always-On-Top Hotfix

**Status:** umgesetzt

**Problem:** Analyse und Fortschritt öffnen als Deep-Sheet mit sehr hohem z-index. Dadurch wurde die QA-Bubble verdeckt und konnte genau in den wichtigen Screens nicht mehr bedient werden.

**Fix:** QA-Capture setzt `egt-qa-capture-active`; QA-Bubble/Panel laufen im QA-Modus mit höchstem z-index und werden per Keep-on-top-Loop wieder an das Ende des Body gehoben. Deep-Sheets werden nur im QA-Modus unterhalb der QA-Bubble einsortiert.

**Nicht geändert:** Die funktionierende Analyse-/Fortschritt-Scrollstruktur aus G54.43.8B/C wurde nicht erneut angefasst.

**QA:** `tests_phase43u_qa_bubble_always_on_top.html`

---

## G54.43.8E – Deep-Sheet Scroll Intelligence für QA

Status: abgeschlossen.

Ziel: Die QA-Bubble erkennt bei offenen Deep-Sheets nicht nur sichtbare Elemente, sondern den echten inneren Scrollcontainer, aktuelle Scrollposition, Max-Scroll, Rest bis unten, Dock-Overlap und QA-Overlay-Overlap.

Umgesetzt:

- `js/qa/egt-visual-state-capture.js` auf G54.43.8E aktualisiert.
- Neuer JSON-Block `deepSheet` im Visual-State-Capture.
- Aktiver Deep-Sheet-Kontext wird über `.ui-sheet.show`, `.ui-deep-sheet`, `[data-ui-deep-sheet]` und Titel erkannt.
- Primary Scrollcontainer wird aus `[data-sheet-scroll]`, `.ui-deep-body`, `.ui-sheet-body`, `.analysis-v2-shell` und Fallback-Scrollable-Kandidaten ermittelt.
- Scrollwerte: `scrollTop`, `maxScrollY`, `bottomGap`, `atTop`, `atBottom`, `canScrollY`.
- Dock-Overlap-Prüfung ergänzt.
- QA-Bubble-Overlap gegenüber aktivem Sheet ergänzt.
- UI-Render zeigt kompakte Deep-Sheet Scroll-Analyse nach Capture.
- Statischer Test `tests_phase43v_deep_sheet_scroll_intelligence.html` ergänzt.

Nicht angefasst:

- Keine erneute Änderung am normalen Analyse-/Fortschritt-Scroll.
- Kein kosmetischer Umbau.

QA:

- JS Syntaxcheck bestanden.
- Statische Markerprüfung bestanden.

## Phase 43V · G54.43.8F · Monster Visual QA Detector

Status: umgesetzt.

Problem: Die QA-Bubble erkannte technische Zustände, aber keine sichtbaren inneren UI-Kollisionen wie „Vorschau“-Badge über Balken im Bereich Stärken & Schwächen.

Umsetzung:
- Monster Visual QA Detector in `js/qa/egt-visual-state-capture.js` ergänzt.
- Neuer JSON-Block `visualMonster` mit Findings, Summary und Detektoren.
- Stärken/Schwächen-spezifische Kollisionsprüfung ergänzt.
- Allgemeine Text-/UI-Kollisionsprüfung ergänzt.
- Karten-Druck/Gequetscht-Prüfung ergänzt.
- Dock-/QA-Overlay-Risikozonen ergänzt.
- QA-UI zeigt jetzt `MONSTER VISUAL QA` mit echten visuellen Befunden.
- Version synchronisiert auf G54.43.8F.

Nächster sinnvoller Schritt:
- G54.43.8G: erkannte Stärken-/Schwächen-Mobile-Kollisionen visuell beheben, z. B. Zwei-Zeilen-Layout auf iPhone.

## Phase 43W · G54.43.8G · Strength/Weakness Mobile Layout Fix

### Anlass
Der Nutzer hat visuell erkannt und per Monster QA bestätigt, dass im Analyse-/Fortschritt-Deep-Sheet die Stärken-/Schwächen-Zeilen auf iPhone überlappen. Die `Vorschau`-Badges lagen teilweise auf den Balken; mehrere Zeilen waren nur ca. 19 px hoch.

### Umsetzung
- Mobile Layout für `.analysis-v2-bar-row` auf Zwei-Zeilen-Struktur umgestellt.
- Label + Badge + Prozentwert stehen in der ersten Zeile.
- Balken steht in der zweiten Zeile über volle Breite.
- Mindesthöhe für mobile Zeilen gesetzt.
- Versionen auf G54.43.8G synchronisiert.
- Dashboard-Marker auf `data-analysis-dashboard-v2="G54.43.8G"` aktualisiert.

### QA
- Neuer Test: `tests_phase43w_strength_weakness_mobile_layout.html`
- Ergebnis: `PASS 14/14`
- JS-Syntaxchecks für QA, Renderer und AppConfig bestanden.

### Erwartung
Der Monster Detector soll nach Upload keine Badge-/Balken-Kollisionen im Stärken-/Schwächen-Bereich mehr melden.

## Phase 43X · G54.43.8H · Admin Portal Mobile Containment Fix

Ziel:
- Adminportal auf iPhone-Portait stabilisieren, ohne andere Bereiche zu überlappen.
- Horizontalen Modal-Overflow aus dem Capture beseitigen.
- Tab-Leiste weiterhin wischbar machen, aber innerhalb des Viewports begrenzen.
- Bottom-Dock bei offenem Adminportal ausblenden, damit KPI-Karten nicht verdeckt werden.

Umgesetzt:
- `css/admin-portal.css`: neuer finaler Override-Block `G54.43.8H · Admin Portal Mobile Containment Fix`.
- `#egtAdminModal`: max-width/overflow-x/box-sizing gehärtet.
- `.egt-portal-tab-group`: auf Mobile selbst als begrenzter horizontaler Scroller gesetzt.
- `body/html.egt-admin-sheet-open #egtBottomDock`: Dock wird bei offenem Adminportal ausgeblendet.
- Admin-KPI-Grids auf kleinen Portrait-Geräten auf 1 Spalte normalisiert.
- `js/qa/egt-visual-state-capture.js`: QA-Noise reduziert, Dock-Buttons werden nicht mehr als Dock-Overlap gegen sich selbst gezählt; legitime horizontale Tab-Scroller werden nicht als Offscreen-Bug gewertet.

QA-Erwartung:
- Adminportal-Capture sollte keine `#egtAdminModal scrollWidth > clientWidth` Warnung mehr liefern.
- Tabgruppe darf horizontal scrollbar sein, aber ihr sichtbarer Rahmen darf nicht rechts aus dem Viewport laufen.
- Bottom-Dock darf bei offenem Adminportal nicht sichtbar über KPI-Karten liegen.



## Phase 43Y · G54.43.8K · Admin Portal Tabbar Residual Overflow Fix

### Anlass
Der iPhone-Capture nach G54.43.8H war fast sauber, meldete aber noch `egt-admin-body.egt-portal-body` mit `scrollWidth: 468` bei `clientWidth: 440`. Zusätzlich war sichtbar, dass der `Admin Logout`-Button teilweise über den Tab `Berichte` lief.

### Umsetzung
- `css/admin-portal.css`: finaler Override für mobile Admin-Tabbar ergänzt.
- Tabgruppe nutzt nun `flex: 1 1 auto` und `width: auto`, damit sie nur den freien Platz neben Logout verwendet.
- Logout-Button ist eigenes fixes Flex-Element und überlappt die horizontal scrollbaren Tabs nicht mehr.
- Admin Body bekommt `overflow-x: visible`, während Modal/Panel weiterhin die Außenbreite begrenzen.
- Visual-State-Capture ignoriert erlaubte innere Horizontal-Scroller bei `clipped`, statt den übergeordneten Admin-Body fälschlich zu warnen.

### Erwartung nach Deploy
- Kein Offscreen-Eintrag für Tabs.
- Kein `clipped`-Eintrag für `egt-admin-body.egt-portal-body`.
- Keine Dock-Warnung bei offenem Adminportal.
- Monster-QA bleibt `pass`.
