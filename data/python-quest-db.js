/* Eignungstest-Trainer · Python Quest Academy DB
   Separate Python-Aufgabendatenbank: kein prüfungsnah-Simulationscontent, sondern Kurs-DNA. */
(function(){
  'use strict';
  ...
  var roadmapNote = 'Dieses Level wird nach und nach freigeschaltet. Trainiere zuerst die vorherigen Grundlagen, damit der nächste Schritt sinnvoll aufbaut.';
  levels.slice(10).forEach(function(l){
    l.lessons = [{id:l.id+'_preview', title:'Level-Vorschau', goal:l.summary, content:[roadmapNote,'Pflichtkonzepte: '+l.concepts.join(', '),'Freischaltung nach bestandener Vorstufe.'], example:'# Vorschau: Dieses Level wird nach der Freischaltung bearbeitet.', explanation:'Der Lernpfad bleibt Schritt für Schritt aufgebaut.'}];
    l.checks = [{id:l.id+'_q1', type:'mc', question:'Was wird in diesem Level schwerpunktmäßig aufgebaut?', answers:[l.summary,'Zeitdruck','Nur Allgemeinwissen','Nur Highscore'], correct:0, explain:'Dieses Python-Level ist ein eigenständiger Lernpfad.'}];
    l.practiceTasks = [{id:l.id+'_p1', title:'Praxis-Vorschau', prompt:'Diese praktische Aufgabe wird freigeschaltet, sobald die vorherigen Grundlagen sitzen.', expected:'', concept:l.concepts[0] || 'python'}];
  });

  window.PYTHON_QUEST_DB = {
    version:'internal',
    name:'Python Quest Academy',
    storageKey:'egt_python_quest_progress_v1',
    ...
    philosophy:['Kein Levelsprung ohne Abschlussprüfung.','Zwischenprüfungen messen Können innerhalb des Levels.','Praktische Übungen sind Pflicht vor der Prüfung.','Der Code-Coach bewertet regelbasiert, transparent und anfängerfreundlich.','Python-Inhalte bleiben getrennt von prüfungsnah-Simulationen.'],
    xpRules:{ lesson:10, mcCorrect:8, practiceDone:15, midExamPass:45, finalExamPass:120, perfectFinalBonus:40 },
    phase2:{ title:'Prüfungsqualität + Level 2', includes:['Level-2-Training','Gating: Praxis vor Zwischenprüfung','Gating: Zwischenprüfung vor Abschlussprüfung','Testfall-Tabelle','Rubrik-Scores','Fehlerkategorien','einheitlicher Apple/Silver-Look'] },
    phase3:{ title:'Lernkurve + Level 3', includes:['Level-3-Training input()','doppelte Gating-Absicherung vor Analyse','Prüfungsprotokoll erweitert','Fehlerstatistik für Wiederholungsbedarf','gezieltere Reparaturübungen','Dashboard-Lernkurve'] },
    phase4:{ title:'Datentypen + PDF-Lernmaterial', includes:['Level-4-Training Zahlen/Rechnen/Datentypen','PDF-Lernmaterial pro Level 1-4','PDF-Link in der Levelansicht','Coach-Diagnose für int/float/operatoren','Testfälle gegen Textverkettung statt echter Rechnung'] },
    phase5:{ title:'Decision + Structure + Comment Intelligence', includes:['Level-5-Training if/else/elif','Struktur als Notenschlüssel-Pflicht','Chaos-Code kann trotz Funktion durchfallen','Kommentarqualität statt Kommentar-vorhanden','Synonym- und Umgangssprachen-Akzeptanz','PDF-Lernmaterial Level 5'] },
    phase6:{ title:'Loops + Endlosschleifen-Schutz', includes:['Level-6-Training while-Schleifen','Coach-Diagnose für Zähler und Ausstiegsbedingungen','Endlosschleifen-Risiko als K.O.-Logik','Kommentar-Synonyme für Wiederholen/Stoppen/Schleife','PDF-Lernmaterial Level 6'] },
    phase9:{ title:'for + Listen Grundlagen', includes:['for/range trainieren','Listen, append, len und index üben','Coach-Diagnose für for/range und Listenstruktur','PDF-Lernmaterial Level 7 und 8','Lernpfad bis Level 8 sichtbar'] },
    phase10:{ title:'Listen auswerten + Mini-Quiz', includes:['Listen durchlaufen und Statistik üben','Mini-Quiz mit Punktestand trainieren','Coach-Diagnose für Listendurchlauf und Punktestand','PDF-Lernmaterial Level 9 und 10','Lernpfad bis Level 10 sichtbar'] },
    levels:levels
  };
})();
