# Working-Plan · G54.50.2J

## G54.50.2J · Appweite Text- und Kartenlayout-Stabilisierung
- Deep-Sheet-Kopfzeilen symmetrisch ausrichten
- Textumbruch appweit normalisieren
- Kartenbadges und Beschreibungen entkoppeln
- responsive Mindestbreiten für Kartenraster erzwingen
- visuelle Regression auf Desktop, Tablet und Mobil prüfen

# Working-Plan · G54.50.2I

## G54.50.2I · Universal Deep-Sheet Navigation

- dynamischer Zurücktitel mit tatsächlichem Zielkontext
- interaktiver aktueller Seitentitel
- aufklappbarer Navigationspfad
- einheitliches X zum Schließen des gesamten Deep Sheets
- interner Navigationsstack
- Scroll- und Fokuswiederherstellung
- responsive Kopfzeile ohne teure visuelle Effekte
- Regression gegen Rechtliches, Einstellungen, Module und Authentifizierung

---

# Working-Plan · G54.50.2H


## G54.50.2H · Rechtsdialog-Navigation und Rückweg

- Rechtliche Unterseiten erhalten einen klaren Zurück-Button im Kopfbereich.
- Zurück navigiert von Dokumenten zur Rechtsübersicht und von dort zum Informationscenter.
- X schließt weiterhin den gesamten Dialog.
- Scrollpositionen der vorherigen Ebene werden wiederhergestellt.
- Die Navigation verwendet keine zusätzlichen Animationen, Blur- oder Schatteneffekte.

## Kostenloser lokaler Admin-Bootstrap

Der erste Admin wird ohne Blaze und ohne Cloud Functions über ein lokales, privilegiertes Node-Skript eingerichtet. Das öffentliche Frontend enthält keinen Bootstrap-Code und keinen Bootstrap-Button mehr. Der globale Firestore-Lock deaktiviert den Vorgang nach Erfolg dauerhaft. Alle älteren Abschnitte zum remote `bootstrapAdmin`-Endpunkt sind historische Phasenstände und durch G54.50.2G ausdrücklich ersetzt.

# Working-Plan · G54.50.2F

## G54.50.2F · 2026-07-12 · ADMIN-BOOTSTRAP-SERVERDIAGNOSE UND IDEMPOTENTE REPARATUR

### HIER SIND WIR JETZT
Der Admin-Bootstrap gibt keine unverständliche rohe `internal`-Antwort mehr aus. Jede kritische Serverstufe wird diagnostisch erfasst, bekannte Firestore-/IAM-/Timeout-Probleme werden verständlich gemeldet und ein Teilerfolg desselben Firebase-Kontos kann sicher fortgesetzt werden. Das Sicherheits-Audit bleibt erhalten, blockiert aber keinen bereits erfolgreich gesetzten Admin mehr.

### Verbindliche Regeln
- unbekannte Backendfehler werden serverseitig protokolliert und als sichere `HttpsError`-Antwort normalisiert
- Diagnose-ID und Verarbeitungsstufe dürfen angezeigt werden, keine Secrets oder Stacktraces
- Firestore und Functions müssen gemeinsam deployt sein
- ein abgeschlossener Bootstrap eines anderen Kontos bleibt dauerhaft blockiert
- ein abgeschlossener Bootstrap desselben Kontos ist idempotent wiederholbar
- Claims werden nur vor dem finalen Commit zurückgerollt
- Auditfehler nach erfolgreichem Commit sind nicht mehr bootstrap-kritisch

---

## G54.50.2E · 2026-07-12 · NOVA-CHATVERLAUF, REPLAY UND BOOTSTRAP-ERSTZUGANG

### HIER SIND WIR JETZT
Nova ersetzt ihre Nachrichten nicht mehr, sondern zeigt den vollständigen Gesprächsverlauf untereinander in einem scrollbar lesbaren Chat. Die Begrüßung kann im Welcome-Screen bewusst neu gestartet werden. Der einmalige Admin-Bootstrap kann vor der App-Check-Ersteinrichtung ausgeführt werden; diese Ausnahme gilt ausschließlich für `bootstrapAdmin`.

### Verbindliche Regeln
- Nova-Nachrichten und bedeutende Nutzerentscheidungen bleiben während der Welcome-Sitzung lesbar.
- Replay startet einen klaren neuen Verlauf und erzeugt keine chaotischen Duplikate.
- Nur `bootstrapAdmin` darf ohne App-Check-Token aufgerufen werden.
- Einmalcode, Firebase Auth, Rate-Limit und Firestore-Lock bleiben zwingend.
- Alle übrigen privilegierten Functions behalten `enforceAppCheck: true`.
- nach Deployment der Functions muss die PWA wegen des neuen Cache-Namens vollständig aktualisiert werden.
