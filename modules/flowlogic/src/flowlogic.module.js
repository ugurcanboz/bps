    ...
    async function onClick(event){
      ...
      if(action === 'pool-preview'){
        if(window.FlowLogicScenarios && window.FlowLogicGenerator){
          ...
          var poolBatch = window.FlowLogicGenerator.generateBatch({ amount:30, seed:'UI:POOL:PHASE11', difficulty:'ctc' });
      ...
      if(action === 'v2-preview'){
        if(window.FlowLogicGenerator && typeof window.FlowLogicGenerator.generateBatch === 'function'){
          var v2 = window.FlowLogicGenerator.generateBatch({ amount:40, seed:'UI:GENERATOR:V2:PHASE11', difficulty:'ctc', minScenarioCoverage:10, minLayoutCoverage:5, minTerminologyCoverage:3 });
      ...
      if(action === 'generator-preview'){
        if(window.FlowLogicGenerator && typeof window.FlowLogicGenerator.createTask === 'function'){
          var generated = window.FlowLogicGenerator.createTask({ scenarioId:'flow_master_postbote_nachnahme', seed:'UI:GENERATOR:POSTBOTE', difficulty:'ctc' });
          ...
          var batch = window.FlowLogicGenerator.generateBatch({ amount:12, seed:'UI:GENERATOR:BATCH', difficulty:'ctc' });
