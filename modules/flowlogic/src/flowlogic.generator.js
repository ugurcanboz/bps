...
(function(){
  ...
  function policyForDifficulty(difficulty, overrides){
    ...
    difficulty = difficulty || 'novuraExams';
    if(difficulty === 'easy'){
    ...
    if(difficulty === 'novuraExams' || difficulty === 'exam'){
  ...
  function selectWithSeed(scenario, seed, policy){
    policy = policyForDifficulty(policy && policy.difficulty || 'novuraExams', policy);
  ...
  function createTask(options){
    ...
    var difficulty = options.difficulty || 'novuraExams';
  ...
  function generateBatch(options){
    ...
    var expectedErrorCount = Number((options.policy && options.policy.errorCount) || policyForDifficulty(options.difficulty || 'novuraExams', options.policy || {}).errorCount || DEFAULT_ERROR_COUNT);
  ...
  function selfCheck(options){
    ...
    scenarioIds.forEach(function(id){
      var built = createTask({ scenarioId:id, seed:'SELFTEST:PHASE11:'+id, difficulty:'novuraExams' });
    ...
    var batch = generateBatch({ amount:Number(options.amount || 80), seed:'SELFTEST:PHASE11:BATCH', difficulty:'novuraExams', scenarioIds:scenarioIds, minScenarioCoverage:Math.min(10, scenarioIds.length), minLayoutCoverage:5, minTerminologyCoverage:3 });
  ...
  if(window.FlowLogicSelfTest && typeof window.FlowLogicSelfTest.register === 'function'){
    ...
    window.FlowLogicSelfTest.register('phase9.generator-single-task-per-scenario', 'Phase 9/11: Generator erzeugt pro Szenario valide 11-Fehler-Aufgaben', function(t){
      ...
      window.FlowLogicScenarios.getAll().forEach(function(s){
        var built = window.FlowLogicGenerator.createTask({ scenarioId:s.id, seed:'SELFTEST:SINGLE:PHASE11:'+s.id, difficulty:'novuraExams' });
    ...
    window.FlowLogicSelfTest.register('phase11.generator-v2-anti-memorization', 'Phase 11: Generator V2 variiert Szenario, Layout, Begriffe und Rastermuster', function(t){
      var batch = window.FlowLogicGenerator.generateBatch({ amount:80, seed:'SELFTEST:BATCH:PHASE11:80', difficulty:'novuraExams', minScenarioCoverage:10, minLayoutCoverage:5, minTerminologyCoverage:3 });
    ...
    window.FlowLogicSelfTest.register('phase11.generator-v2-render-sample', 'Phase 11: V2-Varianten bleiben renderbar und scoringfaehig', function(t){
      ...
      ['flow_master_postbote_nachnahme','flow_master_parksensor','flow_master_login_2fa','flow_master_router_diagnose','flow_master_online_bestellung'].forEach(function(id, idx){
        var built = window.FlowLogicGenerator.createTask({ scenarioId:id, seed:'SELFTEST:RENDER:PHASE11:'+idx, difficulty:'novuraExams' });
