/* Eignungstest-Trainer · Python Quest Academy Module
   Eigenständiger Python-Lernpfad mit separater DB, Prüfungscenter, Level-Gating und Code-Coach-Anbindung. */
(function(){
  'use strict';
  var VERSION = 'python-quest';
  var DB = window.PYTHON_QUEST_DB || {levels:[], storageKey:'egt_python_quest_progress_v1'};
  var BASE_STORAGE_KEY = String(DB.storageKey || 'egt_python_quest_progress_v1').replace(/^assessments_/, 'egt_');
  ...
  function nextLevel(level){ return (DB.levels||[]).filter(function(l){ return l.level === (level.level+1); })[0] || null; }
    function activeUserId(){ try{ if(window.EGTAdminPortal && EGTAdminPortal.state){ var snap=EGTAdminPortal.state(); if(snap && snap.learner && (snap.learner.userId || snap.learner.code)) return String(snap.learner.userId || snap.learner.code); } return localStorage.getItem('egt_active_learner') || 'GUEST'; }catch(e){ return 'GUEST'; } }
  function storageKey(){ return BASE_STORAGE_KEY + ':' + String(activeUserId() || 'GUEST').toUpperCase().replace(/[^A-Z0-9]+/g,'-'); }
  ...
  function readProgress(){ try{ return normalizeProgress(JSON.parse(localStorage.getItem(storageKey())||'{}')); }catch(e){ return defaultProgress(); } }
  function saveProgress(p){ try{ p.updatedAt=nowIso(); p.userId=activeUserId(); localStorage.setItem(storageKey(), JSON.stringify(p)); }catch(e){} return p; }
  ...
  function ensureShell(){
    ...
    bd.innerHTML = '<div class="python-quest-shell"><header class="python-quest-head"><div><div class="python-quest-kicker">Python Quest Academy</div><h2 id="pythonQuestTitle">Python lernen mit Prüfungslogik</h2><p id="pythonQuestSub">Level, Praxis, Zwischenprüfung, Abschlussprüfung und Code-Coach im gleichen UI-Design.</p></div><button class="python-quest-close" type="button" data-py-action="close">✕</button></header><main class="python-quest-body" id="pythonQuestBody"></main></div>';
  ...
  function renderLearningCurve(progress){
    ...
    return '<section class="python-quest-panel"><h3>Lernkurve & Fehlerdiagnose</h3><p>Hier siehst du deine letzten Abgaben, typische Fehler und den nächsten sinnvollen Übungsschritt.</p><div class="python-quest-stats">'+
  ...
  function renderRoadmap(progress){
    return renderStats(progress)+nav()+
      '<section class="python-quest-panel"><h3>Python-Level 1–30</h3><p>Der Lernpfad führt dich von den Grundlagen bis zu kleinen Projekten. Neue Level werden freigeschaltet, sobald die vorherigen Prüfungen bestanden sind.</p><div class="python-roadmap">'+(DB.levels||[]).map(function(l){ var st=levelState(progress,l.id); var cls=(st.finalExam&&st.finalExam.passed)?'done':(isUnlocked(l,progress)?'active':'locked'); return '<button class="'+cls+'" data-py-action="open-level" data-level="'+esc(l.id)+'">L'+l.level+'<br><small>'+esc(l.band)+'</small></button>'; }).join('')+'</div></section>';
  }
  ...
  function analyzeCode(level, type){
    ...
    var coach = window.EGTLearningCoachEngine;
    ...
    try { if(window.EGTCoachDNA){ var uid=activeUserId(); window.EGTCoachDNA.recordResult({userId:uid, learnerId:uid, mode:'python', title:'Python Lernpfad · '+(level.title||level.id), category:'Python', total:100, score:Number(result.score||0), percent:Number(result.score||0), cats:{Python:{n:100,r:Number(result.score||0),t:0}}, errorCategories:result.errorCategories||result.errors||[]}); } } catch(e) { console.warn('Python Coach DNA', e); }
  ...
  function handleAction(el){
    ...
    if(action === 'open-coach'){ if(window.EGTLearningCoach && typeof window.EGTLearningCoach.openHub === 'function') window.EGTLearningCoach.openHub(); else alert('Coach ist noch nicht geladen.'); return; }
  }
  ...
  window.PythonQuest = { version:VERSION, init:init, open:open, close:close, render:render, progress:readProgress, analyze:localAnalyze, db:function(){ return DB; }, reset:function(){ localStorage.removeItem(storageKey()); render(); } };
