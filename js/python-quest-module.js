/* Eignungstest-Trainer V10.3.0 · Python Quest Academy Module
...
(function(){
  ...
  function renderRoadmap(progress){
    return renderStats(progress)+nav()+
      '<section class="python-quest-panel"><h3>Python-Level 1–30</h3><p>Die Level-DNA ist separat von der prüfungsnah-Datenbank. Dadurch bleibt der Eignungstest-Trainer sauber, während Python als eigener Lernpfad mit Prüfungsfreischaltung wächst.</p><div class="python-roadmap">'+(DB.levels||[]).map(function(l){ var st=levelState(progress,l.id); var cls=(st.finalExam&&st.finalExam.passed)?'done':(isUnlocked(l,progress)?'active':'locked'); return '<button class="'+cls+'" data-py-action="open-level" data-level="'+esc(l.id)+'">L'+l.level+'<br><small>'+esc(l.band)+'</small></button>'; }).join('')+'</div></section>';
  }
