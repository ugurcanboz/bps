(function(){
  ...
  function setViewportVars(){
    ...
    document.documentElement.style.setProperty('--bps-sheet-top', isMobileWidth ? (isLandscape || isShort ? '8px' : '18px') : 'clamp(18px, 4dvh, 44px)');
    document.documentElement.dataset.bpsSheetDevice = isMobileWidth ? 'mobile' : (width <= 1100 ? 'tablet' : 'desktop');
  ...
  function cleanupLegacyPractice(){
    ...
    document.querySelectorAll('.training-hub-sheet,.training-category-card,.training-category-modes,.training-sheet-launch-card,.training-inline-preview,.training-native-menu,.training-sheet-router,.clean-sheet-router,.clean-training-sheet,.clean-sheet-backdrop,.warn.small,.exam-options,.db-actions').forEach(el=>{
      ...
      el.setAttribute('data-retired-by', 'bps-deep-sheet-v8.2.2');
    });
  }
  ...
  function render(){
    ...
    body.innerHTML = `
      <div class="bps-sheet-layout">
        <nav class="bps-category-nav" aria-label="Trainingskategorien">${CATEGORIES.map(renderCategoryButton).join('')}</nav>
        <section class="bps-mode-panel">
    ...
    body.querySelectorAll('[data-cat]').forEach(btn => btn.addEventListener('click', ()=>{ activeCategory = Number(btn.dataset.cat) || 0; state().activeSheetCategory = activeCategory; render(); }));
  ...
  function open(input){
    ...
    setTimeout(()=>{ try{ body.focus({preventScroll:true}); }catch(_){} }, 40);
  }

  function close(){
    const backdrop = $(BACKDROP_ID);
    const sheet = $(SHEET_ID);
    document.body.classList.remove(STYLE_CLASS);
  ...
  function patchApp(){
    ...
    App.setTrainingSheetCategory = function(index){ activeCategory = Number(index) || 0; state().activeSheetCategory = activeCategory; render(); };
  ...
  window.addEventListener('resize', ()=>{ setViewportVars(); if(document.body.classList.contains(STYLE_CLASS)) render(); }, {passive:true});
  window.addEventListener('orientationchange', ()=>{ setTimeout(()=>{ setViewportVars(); if(document.body.classList.contains(STYLE_CLASS)) render(); }, 160); }, {passive:true});
