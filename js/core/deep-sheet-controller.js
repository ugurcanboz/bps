  ...
  function render(){
    ...
    body.innerHTML = `
      <div class="bps-sheet-layout" data-active-category="${esc(cat.key)}">
        <nav class="bps-category-nav" role="tablist" aria-label="Trainingskategorien">${CATEGORIES.map(renderCategoryButton).join('')}</nav>
    ...
    body.querySelectorAll('[data-cat]').forEach(btn => {
      ...
      btn.addEventListener('click', ()=> setCategory(Number(btn.dataset.cat) || 0));
    });
