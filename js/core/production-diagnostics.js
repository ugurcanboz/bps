/* BPS-Trainer V7.5.0 · Production Diagnostics
   Berechnet Feature-Readiness anhand von Architektur, Verträgen, Modulen und Runtime. */
(function(){
  "use strict";
  if(window.ProductionDiagnostics && window.ProductionDiagnostics.__version === "7.5.0") return;
  const VERSION="7.5.0";
  ...
  async function run(){
    ...
    const items = [
      scoreItem("AppConfig", bool(window.AppConfig && AppConfig.__version===VERSION), 10, "zentrale Pfade"),
      ...
      scoreItem("Router", bool(window.AppRouter && AppRouter.__version===VERSION), 7, "Hash/Routing"),
      ...
      scoreItem("Hosting", bool(document.querySelector('link[href^="./"]') || location.protocol==="file:"), 7, "relative Pfade")
    ];
  ...
  function render(report){
    ...
    if(!report) return "";
    return `<div class="architecture-guard-card"><b>Systemdiagnose V${VERSION}</b><br><span class="small">Score: ${report.score}% · ${report.ready ? "bereit" : "prüfen"}</span><div class="architecture-guard-grid">${report.items.map(i=>`<div class="architecture-guard-row"><span>${i.name}</span><strong class="${i.ok?'architecture-guard-ok':'architecture-guard-warn'}">${i.detail}</strong></div>`).join("")}</div></div>`;
  }
