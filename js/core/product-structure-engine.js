/* Eignungstest-Trainer · Product Structure Engine
   Phase 26: zentrale Produktlogik für Simulation Center, Training Center und spätere CTC-Pool-Erweiterungen. */
(function(){
  'use strict';
  ...
  var SIMULATION_PROFILES = Object.freeze({
    'it:bps': Object.freeze({ branch:'it', simType:'bps', mode:'bps', label:'IT / FISI · BPS', timePerQuestion:50, helpAllowed:false, coachDuring:false, coachAfter:true, pool:'it-bps' }),
    'it:ctc': Object.freeze({ branch:'it', simType:'ctc', mode:'ctcLohr', label:'IT / FISI · CTC-Lohr', durationLabel:'ca. 33–35 Min.', timePerQuestion:null, helpAllowed:false, coachDuring:false, coachAfter:true, pool:'it-ctc', examStructure:'ctc-lohr-real' }),
    'sozial:bps': Object.freeze({ branch:'sozial', simType:'bps', mode:'bps', label:'Sozialpädagogik · BPS', timePerQuestion:50, helpAllowed:false, coachDuring:false, coachAfter:true, pool:'sozial-bps' }),
    'kaufm:bps': Object.freeze({ branch:'kaufm', simType:'bps', mode:'bps', label:'Kaufmännisch · BPS', timePerQuestion:50, helpAllowed:false, coachDuring:false, coachAfter:true, pool:'kaufm-bps' })
  });
  ...
  function key(branch, simType){ return String(branch||'it') + ':' + String(simType||'bps'); }
  function profile(branch, simType){ return SIMULATION_PROFILES[key(branch, simType)] || SIMULATION_PROFILES['it:bps']; }
  function setBranch(branch){ try{ localStorage.setItem('bps_branch', branch); }catch(e){} try{ if(window.AppModuleHost && typeof AppModuleHost.setBranch === 'function') AppModuleHost.setBranch(branch); }catch(e2){} }
