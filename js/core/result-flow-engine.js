...
(function(){
  ...
  function html(value){ var raw=String(value == null ? "" : value); return window.EGTDOMSecurity ? EGTDOMSecurity.sanitizeHTML(raw) : raw; }
  function getById(ctx, id){
    try { if(typeof ctx.$ === "function") return ctx.$(id); } catch(e) {}
    try { return document.getElementById(id); } catch(e) { return null; }
  }
  function setHtml(ctx, id, value){ var el = getById(ctx, id); if(el) { if(window.EGTDOMSecurity) EGTDOMSecurity.setHTML(el,value); else el.innerHTML = html(value); } return !!el; }
  function appendHtml(ctx, id, value){ var el = getById(ctx, id); if(el) { if(window.EGTDOMSecurity) EGTDOMSecurity.appendHTML(el,value); else el.insertAdjacentHTML("beforeend", html(value)); } return !!el; }
  ...
  function fallbackSummary(ctx){
    ...
    var mode = selectedMode(ctx);
    var target = mode === "novuraExams" ? 70 : mode === "assessments" ? 75 : 80;
