...
(function(){
  ...
  const VERSION = 'G54.46.2-trusted-session-domain';
  ...
  function create(deps){
    ...
    function gateStatus(){
      ...
      let legacyAllowed=false,liveTrusted=false;
      try{
        const sec=window.EGTSecurityContext;
        legacyAllowed=!!(sec&&sec.legacyCodeLoginAllowed&&sec.legacyCodeLoginAllowed());
        const snap=sec&&sec.snapshot?sec.snapshot():null;
        const expected=role==='dozent'?'teacher':role;
        liveTrusted=!!(snap&&snap.claimsTrusted===true&&snap.user&&!snap.user.isAnonymous&&String(snap.user.uid||'')===String(s.firebaseUid||'')&&String(snap.trustedRole||'').toLowerCase()===expected);
      }catch(e){}
      if(role === 'admin' || role === 'teacher' || role === 'dozent') return (liveTrusted||legacyAllowed) ? { open:true, reason:liveTrusted?'trusted-live-claims':'development-legacy' } : {open:false,reason:'untrusted-privileged-session'};
      if(role === 'participant' && s.code) return (liveTrusted||legacyAllowed) ? { open:true, reason:liveTrusted?'trusted-live-participant':'development-legacy' } : {open:false,reason:'untrusted-participant-session'};
      if(role === 'demo') {
        const demo = s.demo || {};
