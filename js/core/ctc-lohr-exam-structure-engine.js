/* Eignungstest-Trainer · CTC Lohr Exam Structure Engine · G54.3
   Realer CTC-Lohr Ablauf: Blocksystem mit echten Zeitprofilen, Eingabeaufgaben
   und FlowLogic-Finalblock. Scope: ausschließlich Simulation → IT/FISI → CTC. */
(function(){
  'use strict';
  var VERSION = 'G54.5-ctc-lohr-numberseries';
  if (window.EGTCTCLohrExamStructureEngine && window.EGTCTCLohrExamStructureEngine.__version === VERSION) return;
  ...
  var BLOCKS = Object.freeze([
    ...
    { key:'ruleArithmetic', label:'3. CTC-Regelrechnung', count:6, timeModel:'perQuestion', questionSeconds:40, group:'Mathe' },
    ...
    { key:'flowLogic', label:'7. CTC-Logik: Wenn-Dann-Ablauf', count:1, timeModel:'perQuestion', questionSeconds:780, group:'Logik' }
  ]);
  ...
  function isItCtcScope(input){
    ...
    // Sicherheit: ohne explizite Angaben KEIN CTC-Scope (verhindert versehentliche Aktivierung).
    ...
    var mode = String(input.mode || profile.mode || 'ctcLohr');
    var pool = String(input.pool || profile.poolKey || profile.pool || 'it-ctc');
    var simType = String(input.simType || profile.simType || 'ctc');
  ...
  function baseMeta(q, block, localIndex){
    ...
    q.poolKey = 'it-ctc';
    q.examTarget = 'ctc';
    q.signature = q.signature || ('ctc-lohr|' + block.key + '|' + localIndex + '|' + esc(q.q));
    q.id = q.id || ('ctc_lohr_' + block.key + '_' + localIndex + '_' + hash(q.signature).toString(36));
    return q;
  }
  ...
  function ruleArithmeticQuestion(localIndex, seed){
    ...
    var ex = makeExpr(rng);
    return {
      type:'ctcRuleArithmeticInput', cat:'CTC-Regelrechnung', group:'Mathe',
      q:'Berechne die Aufgabe im Kopf und wende danach die CTC-Regel an.',
      formula: ex.text,
  ...
  function numberSeriesQuestion(localIndex, seed){
    ...
    return {
      type:'ctcNumberSeriesInput', cat:'Zahlenreihen', group:'Logik',
  ...
  function letterQuestion(localIndex, seed){
    ...
    return { type:'ctcLetterScanInput', cat:'Buchstaben-Konzentration', group:'Konzentration', q:'Zähle den abweichenden Buchstaben und gib die Anzahl ein.', sequence: arr.join(''), targetLetter: target, answer:String(count), a:[String(count)], correct:0, time:40, ex:'Der Buchstabe '+target+' kommt '+count+'-mal vor.' };
  }
  function coordQuestion(localIndex, seed){
    ...
    return { type:'ctcCoordinateTableInput', cat:'Tabellen ablesen', group:'Konzentration', q:'Lies die Koordinaten ab. Wenn eine Spalte oder Zeile nicht existiert, trage X ein.', cols:cols, rows:rows, grid:grid, prompts:prompts, answer:prompts.map(function(p){return p.answer;}).join('|'), a:[prompts.map(function(p){return p.answer;}).join('|')], correct:0, time:90, ex:'Die Tabelle wird wie Koordinaten gelesen. Fehlende Spalten/Zeilen werden mit X beantwortet.' };
  }
  function flowLogicQuestion(index, deps, profile){
    if(window.CTCFlowLogicAdapter && typeof window.CTCFlowLogicAdapter.createQuestion === 'function'){
      return window.CTCFlowLogicAdapter.createQuestion({ index:index, total:totalCount(), mode:'ctcLohr', questionProfile:profile || {branch:'it', simType:'ctc', poolKey:'it-ctc'}, source:'ctc-lohr-exam-structure-g54-2' });
    }
    return { type:'mc', cat:'CTC-Logik', group:'Logik', q:'CTC-Logik: Wenn-Dann-Ablauf konnte nicht geladen werden.', a:['Weiter'], correct:0, time:780, specialTimeSeconds:780, ex:'FlowLogic-Modul nicht verfügbar.' };
  }
  function createQuestion(input){
    input = input || {};
    if(!isItCtcScope(input)) return null;
  ...
  window.EGTCTCLohrExamStructureEngine = { __version:VERSION, blocks:BLOCKS, totalCount:totalCount, totalSeconds:totalSeconds, blockForIndex:blockForIndex, isItCtcScope:isItCtcScope, createQuestion:createQuestion, createExam:createExam, diagnostics:diagnostics };
  window.CTCLohrExamStructureEngine = window.EGTCTCLohrExamStructureEngine;
})();
