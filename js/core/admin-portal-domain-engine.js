...
(function () {
  ...
  var VERSION = 'G54.4-phase29-admin-ctc-report';
  ...
  function diagnostics() {
    ...
    return {
      ...
      provider: p ? 'EGTAdminPortal' : 'missing',
      userDatabase: !!db,
      ctcReportEngine: !!window.EGTCTCAdminReportEngine,
      ready: !!(s && s.ready),
      online: !!(s && s.online),
      role: currentRole(),
      adminOpen: isAdminOpen(),
  ...
  function ctcReportFromHistory(history, quiz, opts) {
    var engine = window.EGTCTCAdminReportEngine;
    if(!engine || typeof engine.buildReport !== 'function') return null;
    return engine.buildReport(Object.assign({ history: history || [], quiz: quiz || [] }, opts || {}, { mode:'ctcLohr', branch:'it', simType:'ctc', pool:'it-ctc' }));
  }
  function ctcReportFromResult(result) {
    var engine = window.EGTCTCAdminReportEngine;
    if(!result) return null;
    if(result.ctcReport) return result.ctcReport;
    if(engine && typeof engine.buildReport === 'function') return engine.buildReport({ record: result, mode: result.mode });
    return null;
  }
  function renderCtcReportSummary(report) {
    var engine = window.EGTCTCAdminReportEngine;
    if(engine && typeof engine.renderAdminSummary === 'function') return engine.renderAdminSummary(report);
    return '<div class="admin-domain-scope-card"><b>CTC-Auswertung</b><br><span class="small">CTC-Report-Engine nicht geladen.</span></div>';
  }
  function enrichCtcAdminEvent(payload) {
    var engine = window.EGTCTCAdminReportEngine;
    if(engine && typeof engine.toAdminEvent === 'function') return engine.toAdminEvent(payload || {}, { mode: (payload && payload.mode) || 'ctcLohr' });
    return payload || {};
  }
  ...
  function create() {
    var api = {
      ...
      ctcReportFromHistory: ctcReportFromHistory,
      ctcReportFromResult: ctcReportFromResult,
      renderCtcReportSummary: renderCtcReportSummary,
      enrichCtcAdminEvent: enrichCtcAdminEvent,
