...
(function(){
  ...
  function create(ctx){
    ...
    function activeBranchIdForQuiz(){
      ...
      try { return String(localStorage.getItem('bps_branch') || 'wissen'); } catch(e2) {}
      return 'wissen';
    }
    ...
    function generateCtcLohrQuestion(index, coach){
      var g = generators;
      var q;
      try {
        var activeProfileForCtc = activeQuestionPoolProfile('ctcLohr');
        if(window.EGTCTCLohrExamStructureEngine && typeof window.EGTCTCLohrExamStructureEngine.createQuestion === 'function'){
          q = window.EGTCTCLohrExamStructureEngine.createQuestion({ index:index, mode:'ctcLohr', questionProfile:activeProfileForCtc, source:'quiz-build-pipeline-g54-2', coach:coach });
          if(q){
            q.group = q.group || groupFor(q.cat);
            q.signature = q.signature || (signature(q) + '|ctc-lohr-real|' + index);
            return attachBranchPoolMeta(q, 'ctcLohr');
          }
        }
      } catch(realCtcError) {
        try { console.warn('[QuizBuildPipeline] CTC Lohr real exam fallback:', realCtcError && realCtcError.message ? realCtcError.message : realCtcError); } catch(logErr) {}
      }
      if(index < 40){
        if(index < 28){
          q = bank({ group:'Allgemeinwissen', excludeCategory:'Satzergänzung' }, function(){ return callGenerator(g.knowledge, 'medium', function(){ return {}; }); }, 'medium', 'ctcLohr');
        } else {
          q = bank({ group:'Allgemeinwissen', category:'Satzergänzung' }, function(){ return callGenerator(g.sentenceCompletion, 'medium', function(){ return callGenerator(g.knowledge, 'medium', function(){ return {}; }); }); }, 'medium', 'ctcLohr');
        }
        q.block = '1. Allgemeinwissen';
      } else if(index < 49){
        var mathLevel = index < 44 ? 'easy' : 'medium';
        q = bank({ group:'Mathe' }, function(){ return callGenerator(pick(choice, [g.ctcMathSprint, g.bookTextMath]), mathLevel, function(){ return callGenerator(g.math, mathLevel, function(){ return {}; }); }); }, mathLevel, 'ctcLohr');
        q.block = '2. Mathe';
      } else if(index < 67){
        if(index < 52){
          ...
          q = bank({ group:'Logik' }, function(){ return callGenerator(pick(choice, [g.matrixBook, g.bookNumberSeries, g.bookVisualLogic, g.series, g.matrix, g.opinionFact, g.statementLogic, g.symbolSeries]), 'medium', function(){ return callGenerator(g.logic, 'medium', function(){ return {}; }); }); }, 'medium', 'ctcLohr');
        }
        q.block = '3. Logik';
      } else if(index < 82){
      ...
      q.signature = signature(q) + '|ctc|' + (q.signatureSeed || index);
      ensureRandomId(q);
      return attachBranchPoolMeta(q, 'ctcLohr');
    }
    ...
    function generateQuestionForModeInternal(mode, index, total, coach){
      ...
      else if(mode === 'ctcLohr') q = generateCtcLohrQuestion(index, coach);
      else if(mode === 'ctc') q = attachBranchPoolMeta(buildAdaptiveQuestion(index, total), mode);
    ...
    function buildQuizInternal(mode){
      ...
      // CTC-Lohr: Die Exam-Structure-Engine liefert bereits eine fest sortierte,
      ...
      if(mode === 'ctcLohr'){
        var coachCtc = null;
        try { coachCtc = buildCoach(mode, m); } catch(eC) { coachCtc = null; }
        var ctcOut = [];
        for(var ci=0; ci<amount; ci++){
          var cq = generateQuestionForMode(mode, ci, amount, coachCtc);
          cq = asObj(cq);
        ...
        return ctcOut.slice(0, amount).map(repairQuestion);
      }
      var seen = new Set();
      var out = [];
      var coach = null;
      try { coach = buildCoach(mode, m); } catch(e1) { coach = null; }
      var memoryQs = (mode !== 'ctc' && m.memory) ? buildMemoryQuestions(mode, m) : [];
      ...
      if(orchestrator && typeof orchestrator.build === 'function'){
        try {
          return orchestrator.build({
            ...
            setupMode:function(currentModeValue){ if(currentModeValue === 'ctc' && typeof ctx.setupMode === 'function') ctx.setupMode(currentModeValue, m); },
