...
(function(){
  ...
  function generateNovuraExamsLohr(ctx){
    var g = ctx.generators;
    var index = toIndex(ctx.index);
    var q;
    try {
      ...
      if(window.NovuraExamsStructureEngine && typeof window.NovuraExamsStructureEngine.createQuestion === 'function'){
        var realQ = window.NovuraExamsStructureEngine.createQuestion({ index:index, mode:ctx.mode, questionProfile:profile, source:'question-factory-g54-2' });
        if(realQ) return withMeta(ctx, realQ, ctx.mode);
      }
    } catch(realNovuraExamsError) {
      try { console.warn('[EGTQuestionFactory] Novura Exams Lohr real exam fallback:', realNovuraExamsError && realNovuraExamsError.message ? realNovuraExamsError.message : realNovuraExamsError); } catch(logErr) {}
    }
    if(index < 40){
      if(index < 28){
      ...
      q = fromBank(ctx, { group:'Mathe' }, function(){ return pick(ctx.choice, [g.novuraExamsMathSprint, g.bookTextMath])(index < 44 ? 'easy' : 'medium'); }, index < 44 ? 'easy' : 'medium');
      q.block = '2. Mathe';
    } else if(index < 67){
      if(index < 52){
    ...
    q.signature = ctx.stableSignature(q) + '|novuraExams|' + (q.signatureSeed || index);
    ensureRandomId(q);
    return withMeta(ctx, q, ctx.mode);
  }

  function generateNovuraExams(ctx){
  ...
  function resolveSource(ctx){
    ...
    if(mode === 'novuraExams') return { type:'fixed-novura-exams-blocks', mode:mode };
    if(mode === 'novuraExams') return { type:'adaptive-novuraExams-generator', mode:mode };
  ...
  function generate(ctx){
    ...
    try {
      ...
      if(ctx.mode === 'novuraExams') return generateNovuraExamsLohr(ctx);
      if(ctx.mode === 'novuraExams') return generateNovuraExams(ctx);
