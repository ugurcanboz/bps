...
(function(){
  ...
  var NOVURA_EXAMS_BLOCKS = ['1. Allgemeinwissen','2. Mathe','3. Logik','4. Konzentration','5. EDV Kenntnisse'];
  ...
  function renderCategoryStats(ctx){
    ...
    if(selectedMode(ctx) === 'novuraExams'){
      var blocks = NOVURA_EXAMS_BLOCKS.concat(['Nicht bearbeitet']);
  ...
  function buildTipItems(ctx, percent){
    ...
    var tips = [];
    if(selectedMode(ctx) === 'novuraExams') tips.push('Prüfungsstrategie: Im Konzentrationsblock nicht vorne festbeißen. Erst sichere Aufgaben holen, dann schwere markieren und später prüfen.');
  ...
  function renderTips(ctx, percent){
    ...
    if(selectedMode(ctx) === 'novuraExams'){
      ...
      NOVURA_EXAMS_BLOCKS.forEach(function(block){
        var items = answered.filter(function(h){ return h && h.block === block; });
        if(items.length){
