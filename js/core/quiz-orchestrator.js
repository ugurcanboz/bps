...
(function(){
  ...
  function build(ctx){
    ...
    // Novura Exams-Lohr: feste, eindeutige Blockfolge direkt aus der Exam-Structure-Engine.
    ...
    if(mode === 'novuraExams'){
      var novuraExamsCoach = null;
      try { novuraExamsCoach = buildCoach(mode, modeDef); } catch(eCC) { novuraExamsCoach = null; }
      var novuraExamsList = [];
      for(var cIdx=0; cIdx<amount; cIdx++){
        var novuraExamsQ = generateCoreQuestion({ mode: mode, index: cIdx, total: amount, coach: novuraExamsCoach, modeDef: modeDef, state: state });
        novuraExamsQ = novuraExamsQ || {};
        if(!novuraExamsQ.signature || String(novuraExamsQ.signature).indexOf('|pos') === -1){
          novuraExamsQ.signature = (novuraExamsQ.signature || signature(novuraExamsQ)) + '|pos' + cIdx;
        }
        novuraExamsList.push(novuraExamsQ);
      }
      return repairList(novuraExamsList.slice(0, amount), repairQuestion);
    }
    ...
    var memoryQs = [];
    if(mode !== 'novuraExams' && modeDef.memory){
