...
(function(){
  ...
  function buildRecord(ctx){
    ...
    try { if(window.EGTCTCAdminReportEngine && typeof EGTCTCAdminReportEngine.enrichRecord === 'function') record = EGTCTCAdminReportEngine.enrichRecord(record, Object.assign({}, ctx, { state: state, history: state.history || ctx.history, quiz: state.quiz || ctx.quiz })); } catch(e) { console.warn('CTC Admin Report enrich fallback', e); }
    return record;
  }
  ...
  function dispatchHooks(ctx, latest){
    ...
    try {
      ...
      if(adminPortal && typeof adminPortal.trackEvent === "function") {
        ...
        try { if(window.EGTCTCAdminReportEngine && typeof EGTCTCAdminReportEngine.toAdminEvent === 'function') adminPayload = EGTCTCAdminReportEngine.toAdminEvent(adminPayload, ctx); } catch(e2) { console.warn('CTC Admin Event enrich fallback', e2); }
  ...
  function persistResult(ctx){
    ...
    return {ok:!!storageResult.ok, latest:latest, storage:storageResult, hooks:hooks};
  }
