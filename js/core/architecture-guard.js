/* Eignungstest-Trainer V7.4.1 · Architecture Guard
   Prüft Core/State/Module/UI-Basis, ohne bestehende V7.3.1/V7.4.0-UI zu ersetzen. */
(function(){
  "use strict";
  if(window.ArchitectureGuard && window.ArchitectureGuard.__version === "7.4.1") return;
  const VERSION = "7.4.1";
  const REQUIRED_IDS = ["start","appNav","sectionIntro","modeGrid","healthPanel","premiumDashboard","mobileTopNav"];
  const REQUIRED_GLOBALS = ["AppState","AppEvents","AppRouter","AppComponents","TrainerModules","AppModuleLoader","App"];
  const REQUIRED_FILES = [
    "./css/app.css?v=7.4.1",
    "./css/mobile.css?v=7.4.1",
    "./css/ui-lock.css?v=7.4.1",
    "./js/core/event-bus.js?v=7.4.1",
    "./js/core/state-manager.js?v=7.4.1",
    "./js/core/router.js?v=7.4.1",
    "./js/components/component-registry.js?v=7.4.1",
    "./js/module-registry.js?v=7.4.1",
    "./js/modules/module-loader.js?v=7.4.1"
  ];
  ...
  function checkVersions(){
    const versions = [window.AppState && AppState.__version, window.AppEvents && AppEvents.__version, window.AppRouter && AppRouter.__version, window.AppComponents && AppComponents.__version, window.TrainerModules && TrainerModules.__version, window.AppModuleLoader && AppModuleLoader.__version].filter(Boolean);
  ...
  async function run(){
    ...
    const summary = {version:VERSION, ok:bad===0, score: Math.max(0, Math.round(100 - bad*18 - warn*6)), bad, warn, checks, timestamp:new Date().toISOString()};
    window.__ARCHITECTURE_HEALTH__ = summary;
    if(window.AppEvents) window.AppEvents.emit("architecture:health", summary);
    try{ console.info("[ArchitectureGuard]", summary); }catch(e){}
    return summary;
  }
