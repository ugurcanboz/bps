/* Eignungstest-Trainer · CTC Admin Report Engine · G54.4 / Phase 29
   Zweck: CTC-Lohr-Blockauswertung für Admin-/Dozentenportal, Coach-Hooks
   und Ergebnis-Persistence. Scope bleibt ausschließlich Simulation → IT/FISI → CTC. */
(function(){
  'use strict';
  var VERSION = 'G54.4-phase29-ctc-admin-report-engine';
  if (window.EGTCTCAdminReportEngine && window.EGTCTCAdminReportEngine.__version === VERSION) return;
  ...
  var DEFAULT_BLOCKS = [
    ...
    { key:'ruleArithmetic', label:'3. CTC-Regelrechnung', expected:6, seconds:240, group:'Mathe' },
    ...
    { key:'flowLogic', label:'6. CTC-Logik: Wenn-Dann-Ablauf', expected:1, seconds:780, group:'Logik' }
  ];
  ...
  function blocks(){
    try {
      if(window.EGTCTCLohrExamStructureEngine && Array.isArray(window.EGTCTCLohrExamStructureEngine.blocks)) {
        return window.EGTCTCLohrExamStructureEngine.blocks.map(function(b){
          return {
            key: b.key,
            label: b.label,
  ...
  function isCtcContext(input){
    ...
    var simType = String(input.simType || record.simType || profile.simType || 'ctc');
    var pool = String(input.pool || input.poolKey || record.poolKey || profile.poolKey || profile.pool || 'it-ctc');
    if(mode === 'ctcLohr' && branch === 'it' && simType === 'ctc' && pool === 'it-ctc') return true;
    if(record && record.ctcReport && record.ctcReport.scope === 'it-ctc-lohr') return true;
    return false;
  }
  ...
  function resolveBlock(item, question, index){
    ...
    return blockFromLabel(item.ctcBlockKey || question.ctcBlockKey || item.blockKey || question.blockKey || item.block || question.block || item.cat || question.cat) || blockByIndex(index);
  }
  ...
  function taskAnswerMode(item, question){
    ...
    if(/Input|FlowLogic|ctcRule|ctcLetter|ctcCoordinate/i.test(type)) return 'input';
  ...
  function buildReport(input){
    input = safeObj(input);
    if(!isCtcContext(input)) return null;
    var state = safeObj(input.state);
    var record = safeObj(input.record);
    ...
    return {
      ...
      scope: 'it-ctc-lohr',
      mode: 'ctcLohr',
      title: 'CTC-Lohr Blockauswertung',
  ...
  function enrichRecord(record, ctx){
    ...
    record.ctcReport = report;
    record.ctcBlockStats = compactStats(report);
    record.ctcWeakestBlock = report.weakestBlock;
    record.exam = Object.assign({}, record.exam || {}, { ctcLohr:true, ctcReportVersion:VERSION, ctcTotalSeconds:report.totalSeconds });
    return record;
  }
  function toAdminEvent(payload, ctx){
    ...
    payload.ctcReport = report;
    payload.ctcBlockStats = compactStats(report);
    payload.ctcWeakestBlock = report.weakestBlock;
    payload.ctcReportSummary = report.blocks.map(function(b){ return b.label + ': ' + b.correct + '/' + b.total + ' (' + b.percent + '%)'; }).join(' · ');
    return payload;
  }
  function renderAdminSummary(report){
    report = report && report.blocks ? report : buildReport(report || {});
    if(!report) return '<div class="ctc-admin-report-card"><b>CTC-Auswertung</b><br><span class="small">Keine CTC-Lohr-Daten verfügbar.</span></div>';
    var rows = report.blocks.map(function(b){
      return '<div class="ctc-admin-report-row" data-status="'+b.status+'"><b>'+b.label+'</b><span>'+b.correct+'/'+b.total+' · '+b.percent+'%</span><small>offen '+b.open+' · Ø '+b.avgSeconds+'s</small></div>';
    }).join('');
    return '<section class="ctc-admin-report-card"><div class="coach-badge">CTC-Lohr</div><h3>Blockauswertung</h3><p class="small">'+report.durationLabel+'</p><div class="ctc-admin-report-grid">'+rows+'</div><p class="small"><b>Schwächster Block:</b> '+(report.weakestBlock ? report.weakestBlock.label + ' (' + report.weakestBlock.percent + '%)' : '-')+'</p></section>';
  }
  ...
  window.EGTCTCAdminReportEngine = {
    __version: VERSION,
    blocks: blocks,
    totalExpected: totalExpected,
    totalSeconds: totalSeconds,
    isCtcContext: isCtcContext,
  ...
  window.CTCAdminReportEngine = window.EGTCTCAdminReportEngine;
})();
