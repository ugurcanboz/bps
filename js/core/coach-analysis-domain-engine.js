...
(function(){
  ...
  function create(deps){
    ...
    function recommendation(ready, weaknesses, memory){
      return safeCall(deps.RecommendationEngine, 'build', [obj(ready), arr(weaknesses), obj(memory)], { title:'Coach sammelt Daten', text:'Starte weitere Läufe, damit der Coach belastbar empfehlen kann.', mode:'ctcLohr' });
    }
    ...
    function currentFocus(){
      return safeCall(deps.TrainingFocusEngine, 'current', [], { key:'auto', label:'KI Auto', group:null, mode:'ctcLohr', hint:'KI darf den Schwerpunkt frei empfehlen.' });
    }
