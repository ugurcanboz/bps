...
(function () {
  ...
  var DEFAULT_PROFILE = Object.freeze({
    ...
    modeDefaults: Object.freeze({ jogging: 'general', bps: 'general', ctcLohr: 'it' }),
  ...
  var PROFILES = Object.freeze({
    it: Object.freeze({
      ...
      modeDefaults: Object.freeze({ ctcLohr: 'it-ctc', bps: 'it-bps', jogging: 'it-jogging' }),
      aliases: Object.freeze(['bookIT','it','itScenario','ctcMathSprint','bookTextMath','matrixBook','bookNumberSeries','bookVisualLogic','focusScanner','symbolSearch','tableComparePro','codeCompare','routeMemory','bigEDVMulti','bigEDVCovered']),
      ...
      guard: 'branch-it-preserve-ctc-blocks'
    }),
    kaufm: Object.freeze({
      branch: 'kaufm',
      label: 'Kaufmännisch / Verwaltung QuestionPool',
      modeDefaults: Object.freeze({ bps: 'kaufm-bps', jogging: 'kaufm-jogging' }),
      ...
      guard: 'branch-kaufm-weighted-bps'
    }),
    sozial: Object.freeze({
      branch: 'sozial',
      label: 'Sozialpädagogik QuestionPool',
      modeDefaults: Object.freeze({ bps: 'sozial-bps', jogging: 'sozial-jogging' }),
      ...
      guard: 'branch-sozial-weighted-bps'
    }),
  ...
  function normalizeBranch(branch) {
    ...
    try {
      key = String(localStorage.getItem('bps_branch') || '').trim().toLowerCase();
      if (PROFILES[key]) return key;
    } catch (e2) {}
    return 'wissen';
  }
  ...
  function resolve(branch, mode, options) {
    ...
    raw.poolKey = (profile.modeDefaults && profile.modeDefaults[selectedMode]) || (profile.modeDefaults && profile.modeDefaults.bps) || 'general';
