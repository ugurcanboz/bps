/* Eignungstest-Trainer.3.0 · Learning Coach V2 Premium Engine
...
(function(){
  ...
  function readinessScore(){
    ...
    return {
      ...
      streak: streak,
      avgMs: avgMs,
      label: confidence < 35 ? 'Daten sammeln' : (ctcRaw >= 75 ? 'prüfungsnah stabil' : bpsRaw >= 75 ? 'Grundniveau stark, Transfer noch schärfen' : 'aufbaubereit'),
      summary: s
    };
  }
  function trainingModes(){
    return [
      ...
      { id:'bps-sprint', title:'Sprint', skill:'mixed', difficulty:4, count:8, tone:'Tempo + Breite', promise:'gemischt, kurz, prüfungsnah' },
  ...
  function dailyChallenge(){
    ...
    if(!m.dailyChallenges[key]){
      ...
      m.dailyChallenges[key] = { id:'daily_'+key, date:key, mode:mode, skill:rec.skill || 'mixed', difficulty: mode==='ctc-hard'?5:Math.max(3, rec.difficulty || 3), count: mode==='ctc-hard'?8:6, done:false, bestQuote:null, title: mode==='ctc-hard'?'CTC Speed-Reiz': mode==='revenge'?'Fehler-Revanche': mode==='bps-sprint'?'Sprint':'5-Minuten Fokus' };
      saveMemory(m);
    }
  ...
  function modeProfile(mode, profile){
    ...
    if(p.mode === 'bps-sprint'){ p.skill='mixed'; p.difficulty=4; p.count=Math.max(8, p.count); p.reason='Sprint: gemischt, schnell, breit.'; }
  ...
  function evaluatePythonSubmission(payload){
    ...
    var result = {
      ...
      checkedAt:nowIso(),
      engine:'EGTLearningCoachEngine · python_exam v10.5.0 phase-10-quiz-mastery',
