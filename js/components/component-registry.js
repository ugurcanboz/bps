/* Eignungstest-Trainer.0.2 · Component Registry
   Zentrale UI-Bausteine für künftige Features. Bestehende HTML-Struktur bleibt geschützt. */
(function(){
  "use strict";
  if(window.AppComponents && window.AppComponents.__version === "9.0.2") return;
  ...
  register("card", (props, h) => `<div class="ui-card ${h.esc(props.className||"")}">${props.title ? `<h3>${h.esc(props.title)}</h3>` : ""}${props.body || ""}</div>`);
  ...
  window.AppComponents = Object.freeze({__version:"9.0.2", register, render, names});
})();
