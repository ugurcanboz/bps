...
(function(){
  ...
  /* Dozenten-/Admin-Analytics: Fehleranalyse-Diagramm + Prognose-Gauge (SVG) */
  function learnerPrognosis(x){
    var mods=(x&&x.modules)||{}, answered=0, correct=0, errs=0;
    var perMod=[];
    Object.keys(mods).forEach(function(k){ var m=mods[k]||{}; var a=Number(m.answered||0), c=Number(m.correct||0); answered+=a; correct+=c;
      var e=0; Object.keys(m.recurringErrors||{}).forEach(function(r){ e+=Number(m.recurringErrors[r]||0); }); errs+=e;
      if(a>0) perMod.push({ mod:k, answered:a, correct:c, q:Math.round(c/a*100), errors:e });
    });
    if(answered<3) return null;
    var acc=correct/answered*100;
    var dataBonus=Math.min(6, answered/15);
    var errPenalty=Math.min(12, (errs/Math.max(1,answered))*40);
    var prob=Math.max(3, Math.min(97, Math.round(50+(acc-70)*2.0+dataBonus-errPenalty)));
    perMod.sort(function(a,b){ return a.q-b.q; });
    return { prob:prob, accuracy:Math.round(acc), answered:answered, errors:errs, perMod:perMod, critical:perMod[0]||null };
  }
  function learnerChartsHtml(x){
    var pg=learnerPrognosis(x);
    if(!pg) return '<div class="egt-analytics-card"><h4>\ud83d\udcca Fehleranalyse & Prognose</h4><div class="egt-empty-state">Noch zu wenig Daten (mind. 3 beantwortete Aufgaben).</div></div>';
    var ang=Math.PI*(1-pg.prob/100), r=52, cx=70, cy=70;
    var px=cx+Math.cos(ang)*r, py=cy-Math.sin(ang)*r;
    var col=pg.prob>=70?'#4ade80':pg.prob>=45?'#fbbf24':'#f87171';
    var arc='M '+(cx-r)+' '+cy+' A '+r+' '+r+' 0 0 1 '+px.toFixed(1)+' '+py.toFixed(1);
    var gauge='<svg viewBox="0 0 140 84" class="egt-gauge"><path d="M '+(cx-r)+' '+cy+' A '+r+' '+r+' 0 0 1 '+(cx+r)+' '+cy+'" fill="none" stroke="rgba(148,163,184,.22)" stroke-width="11" stroke-linecap="round"/><path d="'+arc+'" fill="none" stroke="'+col+'" stroke-width="11" stroke-linecap="round"/><text x="'+cx+'" y="'+(cy-6)+'" text-anchor="middle" font-size="21" font-weight="900" fill="'+col+'">'+pg.prob+'%</text><text x="'+cx+'" y="'+(cy+10)+'" text-anchor="middle" font-size="8.5" font-weight="700" fill="#94a3b8">BESTEHENSPROGNOSE</text></svg>';
    var rows=pg.perMod.slice(0,6).map(function(m){
      var c=m.q>=75?'#4ade80':m.q>=55?'#fbbf24':'#f87171';
      return '<div class="egt-chart-row"><span class="egt-chart-label">'+escapeHtml(m.mod)+'</span>'+
        '<span class="egt-chart-track"><span class="egt-chart-fill" style="width:'+m.q+'%;background:'+c+'"></span></span>'+
        '<span class="egt-chart-val" style="color:'+c+'">'+m.q+'%</span>'+
        '<span class="egt-chart-meta">'+m.answered+' Aufg.'+(m.errors?' \u00b7 '+m.errors+' Fehler':'')+'</span></div>';
    }).join('');
    var errAgg={};
    Object.keys((x&&x.modules)||{}).forEach(function(k){ var re=(x.modules[k]||{}).recurringErrors||{}; Object.keys(re).forEach(function(e){ errAgg[e]=(errAgg[e]||0)+Number(re[e]||0); }); });
    var errChips=Object.keys(errAgg).sort(function(a,b){ return errAgg[b]-errAgg[a]; }).slice(0,4)
      .map(function(e){ return '<span class="egt-err-chip">'+escapeHtml(e)+' <b>\u00d7'+errAgg[e]+'</b></span>'; }).join('');
    return '<div class="egt-analytics-card"><h4>\ud83d\udcca Fehleranalyse & Prognose</h4>'+
      '<div class="egt-analytics-grid"><div class="egt-gauge-wrap">'+gauge+
      '<small>Basis: '+pg.answered+' Aufgaben \u00b7 \u00d8 '+pg.accuracy+' % \u00b7 Ziel 70 %'+(pg.critical?'<br>Kritisch: <b>'+escapeHtml(pg.critical.mod)+'</b> ('+pg.critical.q+' %)':'')+'</small></div>'+
      '<div class="egt-chart-rows">'+rows+'</div></div>'+
      (errChips?'<div class="egt-err-row"><b>Wiederkehrende Fehler:</b> '+errChips+'</div>':'')+'</div>';
  }
  ...
  /* ============================================================
     CLOUD-HIGHSCORE \u00b7 Online-Bestenliste je Kurs \u00fcber Firestore.
     Supabase-Config hat keine Zugangsdaten und bleibt deaktiviert;
     genutzt wird die vorhandene, funktionierende Infrastruktur.
     Dokumente: courses/{courseId}/highscores/{CODE}
     ============================================================ */
  async function highscoreSubmit(entry){
    entry=entry||{};
    await loadSync(); await signIn();
    var code=normalizeCode(entry.code||''); if(!code) throw new Error('Kein Teilnehmer-Code.');
    var ref=docRef('courses/'+state.courseId+'/highscores/'+code);
    var prev={}; try{ var snap=await state.sync.fsMod.getDoc(ref); if(snapExists(snap)) prev=snap.data()||{}; }catch(e){}
    var pct=Math.max(0,Math.min(100,Math.round(Number(entry.percent)||0)));
    var doc={
      code:code,
      name:String(entry.name||prev.name||code).slice(0,24),
      bestPercent:Math.max(Number(prev.bestPercent||0), pct),
      lastPercent:pct,
      lastMode:String(entry.mode||'').slice(0,48),
      runs:Number(prev.runs||0)+1,
      updatedAt:nowIso()
    };
    await state.sync.fsMod.setDoc(ref, doc, {merge:true});
    return doc;
  }
  async function highscoreTop(limit){
    await loadSync(); await signIn();
    var qs=await state.sync.fsMod.getDocs(colRef('courses/'+state.courseId+'/highscores'));
    var rows=[]; qs.forEach(function(d){ var x=d.data()||{}; rows.push(x); });
    rows.sort(function(a,b){ return (b.bestPercent||0)-(a.bestPercent||0) || (b.runs||0)-(a.runs||0); });
    return rows.slice(0, Math.max(1, Math.min(50, limit||20)));
  }
