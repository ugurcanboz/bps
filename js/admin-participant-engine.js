...
(function(){
  ...
  var INTERNAL_VERSION = 'g29-portal-politur';
  ...
  async function loadSync(){
    throw new Error('Online-Sync ist in Phase 2 Stable bewusst deaktiviert.');
  }
  async function signIn(){
    throw new Error('Online-Sync ist in Phase 2 Stable bewusst deaktiviert.');
  }
  function docRef(path){ throw new Error('Online-Sync ist deaktiviert: '+path); }
  function colRef(path){ throw new Error('Online-Sync ist deaktiviert: '+path); }
  ...
  async function init(){
    ...
    state.online=false;
    state.ready=true;
    state.error='Lokaler PWA-Modus aktiv';
  ...
  function snapshot(){ return { internalState:"bereit", online:state.online, ready:state.ready, error:state.error, courseId:state.courseId, learner:state.learner, profile:state.profile }; }
  ...
  var ROLE_PERMISSIONS = {
    admin:{ viewOverview:true, viewUsers:true, manageUsers:true, viewAllLearners:true, viewAssignedLearners:true, viewSystem:true, viewRoadmap:true, manageRoadmap:true, exportReports:true, viewReports:true, clearCache:true, manageQuestions:true },
    dozent:{ viewOverview:true, viewUsers:false, manageUsers:false, viewAllLearners:false, viewAssignedLearners:true, viewSystem:false, viewRoadmap:false, manageRoadmap:false, exportReports:true, viewReports:true, clearCache:false, manageQuestions:false },
  ...
  function reportJson(role, onlyHelp){
    var rows=reportRows(role).filter(function(r){ return !onlyHelp || (r.help!=='stabil' && r.help!=='Keine Daten'); });
    return JSON.stringify({ exportedAt:nowIso(), version:'G29', role:role||currentPortalRole(), scope:reportScopeLabel(role), summary:reportSummary(role), participants:rows }, null, 2);
  }
  function reportText(role){
    var sum=reportSummary(role), rows=reportRows(role);
    var lines=[];
    lines.push('BPS-Trainer · G29 Bericht');
  ...
  function systemInfoHtml(){
    ...
    var cacheName='bps-trainer-g29-portal-politur';
    var build=(window.TRAINER_BUILD_VERSION||'G29-PORTAL-POLITUR');
    var online = (typeof navigator!=='undefined' && 'onLine' in navigator) ? (navigator.onLine?'online':'offline') : 'unbekannt';
    var cards=''
      +systemCardHtml('Version','G29','Portal-Finalisierung & UX-Politur','ok')
      ...
      +systemCardHtml('IndexedDB',window.indexedDB?'verfügbar':'nicht verfügbar','lokale Datenbasis', window.indexedDB?'ok':'warn')
      +systemCardHtml('Teilnehmer',profiles+' Profile','lokal auf diesem Gerät','info')
      +systemCardHtml('Aufgaben',taskCount()+' Aufgaben','geladene Aufgabenbank','info')
      +systemCardHtml('Status',online,'Netzwerkstatus','neutral');
    return '<div class="egt-system-dashboard">'+cards+'</div><div class="egt-admin-kv egt-system-kv"><b>Aktiver Kurs</b><span>'+escapeHtml(state.courseId)+'</span><b>Rolle</b><span>'+escapeHtml(roleLabel())+'</span><b>Cache-Name</b><span>'+escapeHtml(cacheName)+'</span><b>Modus</b><span>Lokaler PWA-Betrieb · keine Cloud-Synchronisierung aktiv</span></div><div class="egt-system-diagnostics" data-system-diagnostics>Cache-/Service-Worker-Details noch nicht ausgelesen. Klicke auf „Diagnose aktualisieren“.</div>';
  }
  async function systemDiagnosticsText(){
    var lines=[];
    lines.push('BPS Trainer · G29 Systemdiagnose');
    lines.push('Build: '+(window.TRAINER_BUILD_VERSION||'G26-SYSTEMINFO-DIAGNOSE'));
  ...
  function roadmapHtml(){
    return '<div class="egt-roadmap-list"><div><b>✅ Erledigt</b><span>Neon UI stabil, Rollen/Gruppenmodell, Dozent A/B, Teilnehmerprofile, Balkenansicht, Aufgabenbank und Berichte.</span></div><div><b>🟣 Aktuell</b><span>G29: Portal-UX, Begriffe, Navigation und Kachelstandard finalisiert.</span></div><div><b>🔵 Nächstes Ziel</b><span>G30: Berichte fachlich verfeinern, Dateinamen verbessern und Förderübersichten ausbauen.</span></div><div><b>🟡 Danach</b><span>G31: Admin-Wartung, Datenverwaltung, Backup/Import und sichere Löschbestätigung.</span></div></div>';
  }
  ...
  function switchTab(tab){
    ...
    if(tab==='admin'){ renderLearners(); renderAdminStats(); }
    if(tab==='dozent'){ renderDozentStats(); renderDozentLearners(); }
  ...
  function questionBankQualityReportText(){
    var a=qbankDeepAudit();
    var lines=[];
    lines.push('BPS Trainer · G29 Aufgabenbank-Qualitätsprüfung');
