  ...
  async function fetchProfile(code){
    ...
    if(state.online){
      ...
      if(access.exists()){
        ...
        var learnerId=acc.learnerId || acc.learner_id || acc.profileId || acc.profile_id || id;
        /* Login-Fix G34.2:
           Zugangscodes werden nach Registrierung korrekt auf "used" gesetzt.
           Das darf die spätere Anmeldung NICHT blockieren, sobald der Code mit einem
           Teilnehmerprofil verknüpft ist. Nur widerrufene/deaktivierte Codes werden
           für Login gesperrt. */
        if(status === 'revoked' || status === 'deleted' || status === 'disabled' || status === 'blocked') throw new Error('Dieser Zugang ist deaktiviert.');
        if(status === 'expired') throw new Error('Dieser Zugang ist abgelaufen.');
        if(status === 'used' && !learnerId) throw new Error('Zugangscode wurde bereits eingelöst, aber kein Profil ist verknüpft. Bitte Admin kontaktieren.');
        var lsnap=await fs.getDoc(docRef('courses/'+state.courseId+'/learners/'+learnerId));
      ...
      var directSnap=await fs.getDoc(docRef('courses/'+state.courseId+'/learners/'+id));
      if(directSnap.exists()){
        var direct=directSnap.data()||{};
        direct.userId=direct.userId||code; direct.code=direct.code||code; direct.loginName=direct.loginName||code; direct.id=id;
        if(direct.status && direct.status!=='active') throw new Error('Dieser Zugang ist deaktiviert.');
        return { id:id, profile:direct };
      }
      throw new Error('Zugang nicht gefunden. Bitte Admin kontaktieren.');
    }
