...
(function(){
  ...
  function ensureShell(){
    ...
    sh.innerHTML = ''+
      ...
      '<header class="bps-coach-header">'+
        ...
        '<div class="bps-coach-title"><b id="bpsCoachTitle">KI Coach </b><span id="bpsCoachSubtitle">adaptiv · lokal · prüfungsnah</span></div>'+
  ...
  function open(title, subtitle, html, options){
    ...
    $('bpsCoachTitle').textContent = title || 'KI Coach';
  ...
  function renderAiStatusCard(){
    ...
    var err = remoteStatus.lastError ? '<small class="bps-ai-error">'+esc(remoteStatus.lastError)+'</small>' : '';
    return '<div class="bps-coach-section bps-ai-live-card '+esc(cls)+'"><h3>Groq Live-Coach</h3><p><b>Status:</b> '+esc(state)+meta+'</p><p class="bps-ai-worker">Worker: '+esc(worker)+'</p>'+err+'<div class="bps-coach-chiprow"><button class="bps-coach-chip primary" data-coach-action="ai-demo">Live-Test</button><button class="bps-coach-chip" data-coach-action="ai-health">Verbindung prüfen</button><button class="bps-coach-chip" data-coach-action="ai-toggle">'+esc(enabled?'Auf lokal umstellen':'Groq aktivieren')+'</button><button class="bps-coach-chip" data-coach-action="ai-clear">Chat leeren</button></div><p class="bps-coach-source">Frontend 38B.2 · '+esc(client && client.__version ? client.__version : 'Client noch nicht geladen')+'</p></div>';
  }
  function renderCoachLoading(query){
    return '<div class="bps-coach-section bps-ai-thinking"><h3>Groq denkt gerade …</h3><p>'+esc(shorten(query, 120))+'</p><div class="bps-ai-dots"><span></span><span></span><span></span></div></div>';
  }
  function renderRemoteReplyText(text, meta){
    meta = meta || {};
    var source = meta.fallback ? 'Lokaler Fallback' : 'Groq Live-Coach über Cloudflare';
    return '<div><b>'+esc(meta.title || 'Live-Coach Antwort')+'</b></div><div class="bps-coach-section bps-ai-reply"><p>'+lineBreaks(text || '')+'</p></div><p class="bps-coach-source">Quelle: '+esc(source)+(meta.level?' · Niveau '+esc(meta.level):'')+'</p>';
  }
  ...
  async function checkAiHealth(openResult){
    ...
    try{
      ...
      if(openResult) open('KI Coach ', 'Groq Verbindung geprüft', '<div class="bps-coach-section bps-ai-live-card online"><h3>Verbindung steht</h3><p>Cloudflare Worker und Groq antworten sauber.</p><p class="bps-ai-worker">Modell: '+esc(res.model || 'Groq')+'</p><div class="bps-coach-chiprow"><button class="bps-coach-chip primary" data-coach-action="ai-demo">Live-Test starten</button><button class="bps-coach-chip" data-coach-action="hub">Zur Übersicht</button></div></div>', { pauseTimer: quizVisible() });
      return true;
    }catch(e){
      ...
      if(openResult) open('KI Coach ', 'Fallback aktiv', '<div class="bps-coach-section bps-ai-live-card warn"><h3>Groq gerade nicht erreichbar</h3><p>Die App bleibt stabil. Der lokale Coach übernimmt automatisch.</p><small>'+esc(e && e.message ? e.message : String(e))+'</small><div class="bps-coach-chiprow"><button class="bps-coach-chip primary" data-coach-action="ai-health">Erneut prüfen</button><button class="bps-coach-chip" data-coach-action="hub">Zur Übersicht</button></div></div>', { pauseTimer: quizVisible() });
      return false;
    }
  }
  ...
  function renderCommandCenter(){
    ...
    return '<section class="bps-coach-command">'+
      '<div class="bps-command-head"><span>Command Center · </span><b>'+esc(r.label || 'Readiness')+'</b><button type="button" data-coach-action="command-toggle">Schließen</button></div>'+
      ...
      '<div class="bps-coach-section"><h3>UI-Modi</h3><div class="bps-coach-mode-grid">'+(c.modes || []).map(function(m){ return modeBtn(m.skill,m.difficulty,m.count,m.title,m.promise,m.id); }).join('')+'</div></div>'+ 
  ...
  function renderSimulationHistory(c) {
    ...
    var sims = sessions.filter(function(s) {
      return ['ctcLohr', 'bps', 'ctc', 'simulation', 'ctcBosch', 'eignungstest'].includes(s.mode);
    });
    if(!sims.length) {
      return '<div class="bps-coach-section"><h3>Simulationen</h3><p class="small">Noch keine Simulationen absolviert. Starte eine Simulation über die Startseite.</p></div>';
    }
    
    var list = sims.slice(0, 3).map(function(s) {
      var dateStr = s.at ? s.at.split('T')[0] : '';
      var modeLabel = s.mode === 'ctcLohr' ? 'CTC Lohr' : s.mode === 'bps' ? 'BPS' : s.mode === 'ctc' ? 'CTC' : 'Simulation';
      ...
      return '<div class="bps-coach-sim-row">'+
        '<span><b>' + esc(modeLabel) + '</b> <small>' + esc(dateStr) + '</small></span>' +
        '<span>' + s.correct + '/' + s.answered + ' (' + pct + '%)</span>' +
        '<span class="bps-coach-badge ' + badgeCls + '">' + badgeText + '</span>' +
        '</div>';
    }).join('');
    
    return '<div class="bps-coach-section bps-sim-history-card"><h3>Simulationen Verlauf</h3>' +
      '<div class="bps-coach-sim-list">' + list + '</div>' +
      '</div>';
  }
  ...
  function renderUserDatabaseCoachCard(){
    ...
    return '<div class="bps-coach-section bps-userdb-coach-card"><h3>Persönlicher Coach-Kontext</h3>'+ 
      '<div class="bps-userdb-coach-grid"><div><b>'+esc(name)+'</b><span>Profil</span></div><div><b>'+esc(c.totalTests || 0)+'</b><span>Signale</span></div><div><b>'+esc((c.averageScore || 0)+'%')+'</b><span>Ø Leistung</span></div><div><b>'+esc(c.riskLevel || 'offen')+'</b><span>Status</span></div></div>'+ 
      '<p><b>Fokus:</b> '+esc(focus)+'. '+esc(c.recommendation && c.recommendation.text || 'Starte eine Simulation, damit ich präziser werde.')+'</p>'+ 
      '<div class="bps-coach-chiprow"><button class="bps-coach-chip primary" data-coach-action="ask" data-query="Zeig mir meinen persönlichen Trainingsplan">Trainingsplan</button><button class="bps-coach-chip" data-coach-action="ask" data-query="Was ist meine größte Schwäche?">Schwäche erklären</button></div>'+ 
    '</div>';
  }
  ...
  function renderHub(){
    ...
    return ''+
      '<section class="bps-coach-hero">'+
        '<div><span class="bps-coach-kicker">Coach  · UI UX Polish</span><h2>Dein Coach führt die nächste sinnvolle Runde.</h2><p>Kurze Diagnose, klare Empfehlung, gezielte Revanche. Öffne ◈ für Readiness, Lernpfad und Prüfungsampel.</p></div>'+
  ...
  function renderConversation(reply){
    ...
    return renderChatLog() + action + '<div class="bps-coach-section"><h3>Weiterfragen</h3><p>Du kannst unten schreiben oder über 🎤 diktieren. '+esc(remoteCoachEnabled() ? 'Groq Live-Coach ist aktiv.' : 'Lokaler Coach-Modus ist aktiv.')+'</p></div>';
  }
