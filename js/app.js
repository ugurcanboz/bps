/* STABLE SINGLE-FILE BUNDLE  · js/src wurde bewusst entfernt, damit GitHub Pages nur diese Runtime lädt. */
/* Bootstrap, Version, Feature Flags, PWA Config, Hilfsfunktionen und Framework-Metadaten. */
"use strict";

/*
===============================================================================
Eignungstest-Trainer V7.5.7 · Stabilitäts-Audit + Matrix-Pflichtübernahme
===============================================================================
Struktur:
...
window.App = (() => {
  const STORE_KEY = "eignungstest_trainer_v901_results";
  ...
  const APP_VERSION = "internal";
  const PROFILE_KEY = "eignungstest_trainer_profile_v901";
  ...
  const FOCUS_KEY = "eignungstest_trainer_focus_v901";
  ...
  const PWA_CONFIG = Object.freeze({
    ...
    note:" Local File Guard Stable: GitHub-Pages-Fallback, Desktop/Mobile getrennt, relative Pfade, Update-Prompt und temporäre Cache-Diagnose stabilisiert."
  });
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const CloudHighscoreEngine = {
    ...
    cloudButtons(note="") {
      ...
      return `<div class="cloud-refresh-row cloud-runtime-actions">${safeNote}<button class="ghost cloud-refresh-btn" type="button" data-action="cloud-refresh" onclick="try{App.refreshCloudRanking()}catch(e){console.error(e)}">Ranking neu laden</button><button class="ghost cloud-test-btn" type="button" data-action="cloud-test" onclick="try{App.addCloudTestScore()}catch(e){console.error(e)}">Cloud-Testeintrag</button></div>`;
    },
    renderShell() {
  ...
  const CoachEngine = {
    build(results) {
      ...
      return {version:"8.4.2",readiness,memory,weaknesses,cognitive,recommendation,adaptive,dynamicMix,learning,simulation,focus:TrainingFocusEngine.current(),updatedAt:new Date().toISOString()};
    },
    renderDashboard(coach) {
      ...
      return `<div class="premium-card ai-card">
        <span class="coach-badge">AI Stable Core </span>
  ...
  const QuestionBankEngine = {
    normalize(raw={}, source="manual") {
      ...
      return {
        ...
        examTarget:raw.examTarget || raw.dna?.examTarget || "both"
      };
    },
    validate(item) {
  ...
  const QualityEngine = {
    ...
    validateQuestionBank() {
      try {
        ...
        return {ok:validation.ok && stats.ready && blocked.blocked===true, schema:QUESTION_BANK_SCHEMA.version, total:stats.total, sampleGroup:sample.group, runtimeImport:"disabled", externalItems:stats.total};
      } catch(error) {
...
const MODES = {
    ...
    errorTrainingPrep:{title:"29. Fehlertraining · Vorbereitung",amount:25,memory:false,instant:true,desc:"Gemischter Trainingsmodus als Vorbereitung für späteres echtes Schwächenlernen und Fehlertraining.",tags:["Fehlertraining","Vorbereitung","V1.9"]},
...
/* Question Factory, Group Mapping, EDV-Schema, Generatoren, Route- und EDV-Fragenaufbau. */
  ...
  const Generators = {
    ...
    fractionRuleEignungstest(level) {
      ...
      // Build visual: show series of fractions with results, last one as question
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  function renderPremiumDashboard(){
    ...
    if($("premiumGreeting")) $("premiumGreeting").textContent = "Willkommen im Eignungstest-Trainer";
    if($("premiumSubtitle")) $("premiumSubtitle").textContent = "© ";
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  function forceDesktopLayoutSanity(){
    try{
      const desktop = window.matchMedia ? window.matchMedia("(min-width: 901px)").matches : window.innerWidth >= 901;
      if(!desktop) return;
      document.body.classList.remove("mobile-shell-active","mobile-nav-attached");
      const start=$("start"), nav=$("appNav"), anchor=$("premiumDashboard");
      if(start && nav && nav.parentElement !== start){
        if(anchor && anchor.parentElement === start) anchor.insertAdjacentElement("afterend", nav);
        else start.insertBefore(nav, $("sectionIntro") || start.firstChild);
      }
      if(nav){ nav.classList.remove("mobile-app-nav","is-hidden"); nav.classList.add("desktop-app-nav"); }
      const top=$("mobileTopNav"); if(top){ top.classList.add("is-hidden"); top.innerHTML=""; }
    }catch(e){}
  }

  function renderAppNav(){
    // V9.1.0 Cleanup: Disabled. Wordhub-layer.js is the single source of truth for navigation.
    return;
  }
  ...
  function renderMobileTopNav(){
    // V9.1.0 Cleanup: Disabled. Wordhub-layer.js handles navigation.
    return;
  }
  ...
  function renderHighscoreSection(){
    ...
    const localCard = !localTop.length
      ? `<div class="premium-card layout-card"><span class="coach-badge">Lokaler Highscore</span><div class="coach-action">Noch kein Rekord</div><div class="small">Starte einen Test. Danach erscheint hier deine lokale Bestenliste.</div></div>`
      : `<div class="premium-card layout-card"><span class="coach-badge">Lokaler Highscore</span><div class="coach-action">Bestwert: ${hs.bestOverall.percent}%</div><div class="ai-list">${localTop.map((r,i)=>`<div>${i+1}. <b>${escHTML(r.title||r.mode)}</b> · ${r.percent}% · ${escHTML(HighscoreEngine.rankLabel(r.percent))}</div>`).join("")}</div><div class="small">Lokale Sicherheits-Bestenliste auf diesem Gerät.</div></div>`;
  ...
  function renderSettingsSection(){
    ...
    const frameworkBlock=`<div class="premium-card section-card settings-card"><b>Framework</b><p class="small">Production Feature-Ready  · Module bleiben getrennt: EDV, Route, Simulation, Storage und Cloud.</p><button class="ghost" onclick="App.showFrameworkHealth()">Framework prüfen</button></div>`;
  ...
  function renderDashboardSection(){
    ...
    const liveCard=`<div class="premium-card section-card"><b>Live Dashboard</b><p class="small">Läufe: ${stats.results.length} · Durchschnitt: ${stats.avg}% · Rekord: ${stats.best?stats.best.percent+"%":"–"}</p><button class="ghost" onclick="App.setAppSection('practice')">Training öffnen</button></div>`;
    const statsCard=`<div class="premium-card section-card"><b>Statistik</b><p class="small">Deine lokale Lernkurve, Kategorien und Fehleranalyse liegen in der Analyseansicht.</p><button class="ghost" onclick="App.showAnalysis()">Analyse öffnen</button></div>`;
    const focusCard=`<div class="premium-card section-card"><b>Fokus</b><p class="small">Der Coach priorisiert Schwächen und empfiehlt dir den nächsten Trainingsblock.</p>${CoachEngine.renderDashboard(coach)}</div>`;
    const cloudCard=`<div class="premium-card section-card"><b>Cloud</b><p class="small">Supabase Highscore und Diagnose sind hier angebunden.</p><button class="ghost" onclick="App.showFrameworkHealth()">Cloud prüfen</button></div>`;
  ...
  function stabilizeMobileBottomNav(){
    try{
      ...
      if(mobile){
        document.body.classList.add("mobile-shell-active","mobile-nav-attached");
        document.body.dataset.appSection = state.activeAppSection || "home";
        if(nav.parentElement !== document.body) document.body.appendChild(nav);
      }else{
        document.body.classList.remove("mobile-shell-active","mobile-nav-attached");
        if(start && nav.parentElement !== start){
          const anchor=$("premiumDashboard"), intro=$("sectionIntro");
          if(anchor && anchor.parentElement === start) anchor.insertAdjacentElement("afterend", nav);
  ...
  function renderSectionIntro(){
    ...
    if(section==="home"){
      el.innerHTML=`<div class="section-stack home-stack"><div class="section-heading"><h2>Startseite</h2><p>Dein zentraler Hub. Unten wechselst du Hauptbereiche, oben steuerst du den Kontext.</p></div><div class="section-grid hub-grid"><div class="hub-card section-card" onclick="App.setAppSection('practice')"><span class="hub-icon">✎</span><b>Üben</b><p>Mathe, Logik, EDV, Konzentration und Route gezielt trainieren.</p></div><div class="hub-card section-card" onclick="App.selectMode('ctcLohr')"><span class="hub-icon">▶</span><b>CTC Simulation</b><p>93 Aufgaben mit Blocklogik und Zeitdruck starten.</p></div><div class="hub-card section-card" onclick="App.setAppSection('highscore')"><span class="hub-icon">★</span><b>Highscore</b><p>Tägliche, wöchentliche, monatliche und gesamte Rangliste öffnen.</p></div></div></div>`;
      ...
      el.innerHTML=`<div class="section-stack practice-stack"><div class="section-heading"><h2>${s.title}</h2><p>${s.desc}</p></div></div>`;
    } else {
      el.innerHTML=`<div class="section-stack generic-stack"><div class="section-heading"><h2>${s.title}</h2><p>${s.desc}</p></div></div>`;
    }
  }
  ...
  function renderModes() {
    ...
    // : mobile-nav-attached wird ausschließlich in renderAppNav/stabilizeMobileBottomNav gesetzt.
    renderPremiumDashboard();
    renderAppNav();
    renderMobileTopNav();
    stabilizeMobileBottomNav();
    renderSectionIntro();
    renderModeTabs();
    ...
    setVisible($("focusControls"), false);
    ...
    if(section==="practice"){
      ...
      const selected = MODES[state.selectedMode] || MODES.ctcLohr;
      grid.innerHTML = `<div class="bps-practice-launch training-native-launch v901-launch"><button class="bps-open-sheet" type="button" onclick="App.openTrainingSheet()"><span class="bps-launch-icon">▣</span><b>Trainingsmenü öffnen</b><small>${TRAINING_SHEET_CATEGORIES.length} Kategorien · ${allTrainingModes().length} Module · Aufgaben erst nach Start</small></button><div class="bps-current-choice"><span>Aktuell ausgewählt</span><b>${escHTML((selected.title || 'Training').replace(/^\d+\.\s*/,''))}</b><small>${escHTML(selected.desc || 'Bereit zum Start')}</small></div></div>`;
      renderTrainingSheetContent();
    } else {
      grid.innerHTML="";
      if($("modeTabs")) $("modeTabs").innerHTML="";
  ...
  function renderEdvMultiAnswers(q) {
    ...
    const optionButtons = EDV_SCHEMA.map(x=>`<button type="button" class="edv-select-btn ${selected.includes(x.id)?"used":""}" data-edv-id="${x.id}"><b>${x.id}</b><span>${escHTML(x.text)}</span></button>`).join("");
    $("answers").innerHTML=`
      <div class="edv-answer-panel">
        ...
        <div class="route-action-row">
          <button type="button" class="secondary" data-action="edv-undo">Letzte entfernen</button>
          <button type="button" class="secondary" data-action="edv-clear">Auswahl leeren</button>
          <button type="button" data-action="edv-submit">Gesamtantwort werten</button>
        </div>
      </div>`;
    ...
    $("answers").innerHTML = `
      <div class="route-sequence-panel">
        ...
        <div class="route-option-grid">
          ${(q.routeOptions||q.a).map(street=>`<button type="button" class="${selected.includes(street)?"used":""}" data-route-street="${escHTML(street)}">${escHTML(street)}</button>`).join("")}
        </div>
        <div class="route-action-row">
          <button type="button" class="secondary" data-action="route-clear">Auswahl leeren</button>
          <button type="button" class="secondary" data-action="route-undo">Letzte entfernen</button>
          <button type="button" data-action="route-submit">Gesamtantwort werten</button>
        </div>
      </div>
    `;
  ...
  function showDatabaseInfo() {
    ...
    panel.innerHTML = `
      ...
      <div class="db-status-card"><b>Aufgabenbank-Import:</b><br><code>source, sourcePage, category, group, subtype, difficulty, question, answers, correct, explanation, tags, verified</code><br><span class="small">Status : Textaufgaben digital ergänzt.</span></div>
  ...
  function setDiagnosticsContent(html){
    ...
    const localPanel = $("healthPanel");
    if(localPanel){ localPanel.classList.add("show"); localPanel.innerHTML = html; }
  }
  ...
  function installHighscoreRendererV840(){
    const esc = (v)=>String(v == null ? '' : v).replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));
    const cloudCardHtml = () => `<div class="premium-card cloud-highscore-card" id="cloudHighscoreCard"><span class="coach-badge">Cloud Highscore</span><div class="coach-action">Ranking wird geladen...</div><div class="small"> Highscore Renderer · direkte Tabellenanzeige</div></div>`;
  ...
  installHighscoreRendererV840();
  function highscoreRenderWatchdogV841(){
    try{
  ...
  setInterval(highscoreRenderWatchdogV841, 2500);
  setTimeout(highscoreRenderWatchdogV841, 400);
  setTimeout(highscoreRenderWatchdogV841, 1600);
  ...
  function openMathHelp(q) {
    ...
    sheet.innerHTML = `
      ...
      <div class="wh-sheet-header">
        <div class="wh-sheet-icon c-orange">➗</div>
        <div class="wh-sheet-info">
          <div class="wh-sheet-name">💡 Mathe-Hilfe</div>
          <div class="wh-sheet-kicker">${esc(q.cat)}</div>
        </div>
        <button class="wh-sheet-close" id="mathHelpCloseBtn" aria-label="Schließen">✕</button>
      </div>
      <div class="wh-sheet-sep"></div>
      <div class="wh-sheet-body wh-math-help-body">
        <p class="wh-math-help-intro">Löse die Aufgabe Schritt für Schritt. Öffne die Abschnitte nacheinander, um selbstständig zu lernen.</p>
        
        <details class="wh-math-help-card">
          <summary><strong>1. Tipp</strong> <small>kleiner Denkanstoß</small></summary>
          <div class="wh-math-help-content">${q.tip || "Lies die Aufgabe genau und markiere die gegebenen Werte."}</div>
        </details>

        <details class="wh-math-help-card">
          <summary><strong>2. Trick / Merkhilfe</strong> <small>wichtige Regel</small></summary>
          <div class="wh-math-help-content">${q.trick || "Achte auf Stolpersteine und Einheiten-Umrechnungen."}</div>
        </details>

        <details class="wh-math-help-card">
          <summary><strong>3. Lösungsweg</strong> <small>Schritt für Schritt</small></summary>
          <div class="wh-math-help-content">${q.stepByStep || "Löse die Teilrechnungen nacheinander auf."}</div>
        </details>

        <details class="wh-math-help-card">
          <summary><strong>4. vollständige Erklärung</strong> <small>Erläuterung</small></summary>
          <div class="wh-math-help-content">${q.ex || q.explanation || "Keine detaillierte Erklärung vorhanden."}</div>
        </details>

        <details class="wh-math-help-card">
          <summary><strong>5. was wird geprüft?</strong> <small>Lernziel</small></summary>
          <div class="wh-math-help-content">${q.testedConcept || "Mathematische Grundlagen und logisches Denken."}</div>
        </details>

        <details class="wh-math-help-card">
          <summary><strong>6. ähnliche Aufgabe</strong> <small>Transferbeispiel</small></summary>
          <div class="wh-math-help-content">${q.similarQuestion || "Versuche eine ähnliche Konstellation mit anderen Zahlen zu lösen."}</div>
        </details>

        <div class="wh-math-help-mastery">
          <button type="button" id="mathHelpMasteryBtn" class="wh-btn-mastery ${isMastered ? 'is-mastered' : ''}">
