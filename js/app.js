/* STABLE SINGLE-FILE BUNDLE V8.1.2 · js/src wurde bewusst entfernt, damit GitHub Pages nur diese Runtime lädt. */
...
- MigrationPath auf V8.1.2 aktualisiert
...
window.App = (() => {
  ...
  const APP_VERSION = "8.1.2-deep-sheet-clean-router";
  ...
  const PWA_CONFIG = Object.freeze({
    ...
    note:"V8.1.2 Local File Guard Stable: GitHub-Pages-Fallback, Desktop/Mobile getrennt, relative Pfade, Update-Prompt und temporäre Cache-Diagnose stabilisiert."
  });
...
/* StorageEngine, IndexedDBEngine, DatabaseBridge und Datenmigration. */
  ...
  const DATA_MODEL = {
    ...
    migrationPath:["1.0","1.1","1.2","1.2.1","1.3","1.4","1.4.1","1.5","1.5.1","1.6","1.6.1","1.7","1.7.1","1.7.2","1.8","1.8.1","1.9","1.9.1","2.0","2.1","2.2","2.3","2.3.1","2.4","2.5","2.5.1","2.6","2.7","2.8","2.8.1","2.9","2.9.1","3.1","3.1.1","3.2","3.3","3.3.1","3.3.2","3.4","3.4.1","3.5","3.5.1","3.5.2","3.5.3","3.5.4","3.5.5","3.6.0","3.6.1","3.6.2","4.0.0","4.1.0","4.1.1","4.2.0","4.2.1","5.0.0","5.0.1","5.0.2","5.1.0","5.1.1","5.1.2","5.1.3","5.1.4","6.0.0","6.1.0","6.1.1","7.0.0","8.1.2","8.1.2"],
    futureEngine:"active",
  ...
  function fromQuestionBank(filter={}, fallback=null, level="medium") {
    ...
    return makeMC(item.category, level==="hard"?18:24, item.question, sh.opts, sh.correct, item.explanation, "mc", {
      sourceId:item.id,
      subtype:item.subtype || "mc",
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const CloudHighscoreEngine = {
    ...
    async submit(result) {
      ...
      async function postRecord(engine, body, label){
        const response=await engine.request(engine.endpoint(), {method:"POST", headers:engine.headers({"Prefer":"return=minimal"}), body:JSON.stringify(body)}, 5500);
        if(!response.ok) {
          const text = await response.text().catch(()=>"");
          const err = new Error(label+" fehlgeschlagen: "+response.status+(text?" · "+text.slice(0,220):""));
          err.status=response.status; err.body=text; throw err;
        }
        return response;
      }
      try {
        await postRecord(this, payload, "Cloud Highscore Upload");
        return {ok:true,record:payload,variant:"full"};
      } catch(firstError) {
        // Viele Supabase-Tabellen haben nur Kernspalten. Dann retten wir den Upload mit Minimal-Schema.
        await postRecord(this, minimalPayload, "Cloud Highscore Minimal-Upload");
        return {ok:true,record:minimalPayload,variant:"minimal",warning:String(firstError && firstError.message ? firstError.message : firstError)};
      }
    },
    async fetchTop(period="all") {
      ...
      async function read(engine, query){
        const response=await engine.request(engine.endpoint(query), {method:"GET", headers:engine.headers({"Cache-Control":"no-cache"})}, 5500);
        const text=await response.text().catch(()=>"");
        if(!response.ok) {
          const err=new Error("Cloud Highscore Abruf fehlgeschlagen: "+response.status+(text?" · "+text.slice(0,180):""));
          err.status=response.status; err.body=text; throw err;
        }
    ...
    async testConnection() {
      ...
      try {
        ...
        const response=await this.request(this.endpoint(qs), {method:"GET", headers:this.headers({"Cache-Control":"no-cache"})}, 4500);
        const text=await response.text().catch(()=>"");
        if(!response.ok) {
  ...
  const CoachEngine = {
    build(results) {
      ...
      return {version:"8.1.2",readiness,memory,weaknesses,cognitive,recommendation,adaptive,dynamicMix,learning,simulation,focus:TrainingFocusEngine.current(),updatedAt:new Date().toISOString()};
    },
    renderDashboard(coach) {
      ...
      return `<div class="premium-card ai-card">
        <span class="coach-badge">AI Stable Core V8.1.2</span>
...
const MODES = {
    ...
    techniqueSprint:{title:"20. Blocktraining · Technik Sprint",amount:25,memory:false,instant:true,desc:"Technisches Verständnis, Mechanik, Hebel, Rollen, Stromlaufpläne, Zahnräder und räumliches Denken.",tags:["Blocktraining","Technik","Mechanik"]},
    errorTrainingPrep:{title:"21. Fehlertraining · Vorbereitung",amount:25,memory:false,instant:true,desc:"Gemischter Trainingsmodus als Vorbereitung für späteres echtes Schwächenlernen und Fehlertraining.",tags:["Fehlertraining","Vorbereitung","V1.9"]},
...
/* Question Factory, Group Mapping, EDV-Schema, Generatoren, Route- und EDV-Fragenaufbau. */
  ...
  const Generators = {
    ...
    itScenario() {
      return choice([
        ()=>makeMC("IT Fehleranalyse",35,"Ein PC hat IP 192.168.1.50, Gateway 192.168.1.2, DNS leer. Internet per IP geht, Webseiten-Namen gehen nicht. Was ist wahrscheinlich falsch?",["DNS fehlt","RAM defekt","Monitor defekt","Tastatur defekt"],0,"Wenn IP geht, aber Namen nicht, ist DNS wahrscheinlich das Problem."),
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  function generateQuestionForMode(mode,index,total) {
    ...
    if(mode==="ctcLohr") {
      ...
      else if(index<67){ q=choice([Generators.matrixBook,Generators.series,Generators.matrix,Generators.opinionFact,Generators.statementLogic,Generators.symbolSeries])("medium"); q.block="3. Logik"; }
    ...
    const pools = {
      ...
      logicSprint:[Generators.matrixBook,Generators.series,Generators.matrix,Generators.symbolSeries,Generators.statementLogic,Generators.opinionFact,Generators.analogy],
      ...
      visualIQSprint:[Generators.matrixBook,Generators.visualIQ,Generators.visualGearsPro,Generators.visualMirror,Generators.visualCubeRotation,Generators.visualFolding,Generators.visualMatrixIQ,Generators.visualCircuit,Generators.visualMechanicsPro,Generators.visualTechnical],
      itSprint:[Generators.it,Generators.itScenario],
      knowledgeSprint:[Generators.knowledge,Generators.sentenceCompletion],
  ...
  function renderProfileManager(){
    ...
    return `<div class="premium-card profile-manager-card">
      <span class="coach-badge">Profilverwaltung V8.1.2</span>
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  const MODE_TABS = {
    ...
    block:{title:"Blocktraining", desc:"Kurze Sprintblöcke für Schwächen und Zeitdruck. Spezialblöcke für Matrizen und Satzergänzung sind hier sichtbar, aber nicht als separater Testbereich.", modes:["mathSprint","logicSprint","concentrationSprint","visualIQSprint","itSprint","knowledgeSprint","matrixOnlySprint","sentenceSprint","techniqueSprint","errorTrainingPrep"]}
  };
  ...
  const TRAINING_SHEET_CATEGORIES = [
    ...
    {key:"math", label:"Mathe", icon:"➗", note:"Grundrechnen, Kopfrechnen und Mathe-Sprints für Zeitdruck.", modes:["math","mathSprint"]},
    {key:"logic", label:"Logik", icon:"🧩", note:"Zahlenreihen, Muster, Symbolfolgen und klassische Logikaufgaben.", modes:["logic","logicSprint"]},
    ...
    {key:"visual", label:"Visual IQ", icon:"👁️", note:"Zahnräder, Spiegelung, Würfel, Faltung, Mechanik und visuelles Denken.", modes:["visualIQ","visualIQSprint","routeMemoryMode"]},
    {key:"language", label:"Sprache", icon:"✍️", note:"Englisch und Satzergänzung gezielt üben.", modes:["english","sentenceSprint"]},
  ...
  function renderAppNav(){
    ...
    // Stable rule V8.1.2: Mobile-Navigation wird direkt an den Body gehängt.
  ...
  function renderSettingsSection(){
    ...
    const frameworkBlock=`<div class="premium-card section-card settings-card"><b>Framework</b><p class="small">Production Feature-Ready V8.1.2 · Module bleiben getrennt: EDV, Route, Simulation, Storage und Cloud.</p><button class="ghost" onclick="App.showFrameworkHealth()">Framework prüfen</button></div>`;
  ...
  function renderModes() {
    ...
    // V8.1.2: mobile-nav-attached wird ausschließlich in renderAppNav/stabilizeMobileBottomNav gesetzt.
    renderPremiumDashboard();
    renderAppNav();
    renderMobileTopNav();
    ...
    if(section==="practice"){
      ...
      grid.innerHTML = `<div class="training-native-launch v812-launch"><button type="button" onclick="App.openTrainingSheet()"><span>▣</span><b>Trainingsmenü öffnen</b><small>${TRAINING_SHEET_CATEGORIES.length} Kategorien · ${allTrainingModes().length} Module · Aufgaben erst nach Start</small></button></div>`;
      renderTrainingSheetContent();
    } else {
      grid.innerHTML="";
      if($("modeTabs")) $("modeTabs").innerHTML="";
  ...
  function isExerciseIntroMode(mode) {
    return ["math","logic","general","english","it","concentrationPro","routeMemoryMode","visualIQ","mathSprint","logicSprint","concentrationSprint","visualIQSprint","itSprint","knowledgeSprint","matrixOnlySprint","sentenceSprint","techniqueSprint","errorTrainingPrep"].includes(mode);
  }
  ...
  function showDatabaseInfo() {
    ...
    panel.innerHTML = `
      ...
      <div class="db-status-card"><b>Aufgabenbank-Import:</b><br><code>source, sourcePage, category, group, subtype, difficulty, question, answers, correct, explanation, tags, verified</code><br><span class="small">Status V8.1.2: vorbereitet, runtime-deaktiviert.</span></div>
