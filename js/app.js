/* STABLE SINGLE-FILE BUNDLE V8.3.6 · js/src wurde bewusst entfernt, damit GitHub Pages nur diese Runtime lädt. */
...
- MigrationPath auf V8.3.6 aktualisiert
...
window.App = (() => {
  ...
  const APP_VERSION = "8.3.6-answer-engine-hardfix";
  ...
  const PWA_CONFIG = Object.freeze({
    ...
    note:"V8.3.6 Local File Guard Stable: GitHub-Pages-Fallback, Desktop/Mobile getrennt, relative Pfade, Update-Prompt und temporäre Cache-Diagnose stabilisiert."
  });
...
/* StorageEngine, IndexedDBEngine, DatabaseBridge und Datenmigration. */
  ...
  const DATA_MODEL = {
    ...
    migrationPath:["1.0","1.1","1.2","1.2.1","1.3","1.4","1.4.1","1.5","1.5.1","1.6","1.6.1","1.7","1.7.1","1.7.2","1.8","1.8.1","1.9","1.9.1","2.0","2.1","2.2","2.3","2.3.1","2.4","2.5","2.5.1","2.6","2.7","2.8","2.8.1","2.9","2.9.1","3.1","3.1.1","3.2","3.3","3.3.1","3.3.2","3.4","3.4.1","3.5","3.5.1","3.5.2","3.5.3","3.5.4","3.5.5","3.6.0","3.6.1","3.6.2","4.0.0","4.1.0","4.1.1","4.2.0","4.2.1","5.0.0","5.0.1","5.0.2","5.1.0","5.1.1","5.1.2","5.1.3","5.1.4","6.0.0","6.1.0","6.1.1","7.0.0","8.3.6","8.3.6"],
    futureEngine:"active",
  ...
  function fromQuestionBank(filter={}, fallback=null, level="medium") {
    const candidates = QuestionBankEngine.filter(filter);
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const CloudHighscoreEngine = {
    ...
    async refreshDashboard() {
      ...
      if(!this.isConfigured()) {
        const miss=this.missingFields();
        if(stillCurrent()) el.innerHTML=`<span class="coach-badge">Cloud Highscore · aktivierbar</span><div class="coach-action">Cloud nicht verbunden</div><div class="small">Keine Fake-Rangliste. Trage echte Supabase URL und anon key in <code>data/cloud-config.js</code> ein. Fehlend: ${escHTML(miss.join(", "))}.</div>`;
        return;
      }
      el.innerHTML=`<span class="coach-badge">Cloud Highscore · Online</span><div class="coach-action">Ranking wird geladen...</div><div class="small">Supabase wird live abgefragt. Spätestens nach wenigen Sekunden erscheint Cloud-Ergebnis oder lokaler Fallback.</div>`;
  ...
  const CoachEngine = {
    build(results) {
      ...
      return {version:"8.3.6",readiness,memory,weaknesses,cognitive,recommendation,adaptive,dynamicMix,learning,simulation,focus:TrainingFocusEngine.current(),updatedAt:new Date().toISOString()};
    },
    renderDashboard(coach) {
      ...
      return `<div class="premium-card ai-card">
        <span class="coach-badge">AI Stable Core V8.3.6</span>
...
const MODES = {
  ...
  const BLOCK_INFO = {
    "1. Allgemeinwissen": {title:"Block 1: Allgemeinwissen", text:"40 kurze Wissensfragen. Nicht extrem schwer, aber unter Zeitdruck.", rules:["Politik, Geografie, Geschichte, Alltag, Technik, Satzergänzung","Sichere Antworten sofort holen","Unsichere Fragen markieren und weitergehen"]},
...
/* Question Factory, Group Mapping, EDV-Schema, Generatoren, Route- und EDV-Fragenaufbau. */
  ...
  function groupFor(cat) {
    ...
    if(["Allgemeinwissen","Politik","Geografie","Europa","Wissenschaft","Technik","Geschichte","Natur","Wirtschaft","Alltag","Deutschland","Satzergänzung"].includes(cat) || cat.includes("Satzergänzung")) return "Allgemeinwissen";
  ...
  const EDV_SCHEMA = [
    {id:"A1", text:"Router LAN: 192.168.3.6", ok:true},
    {id:"A2", text:"Switch verbindet PC-1, PC-2, Server und Drucker", ok:true},
    {id:"A3", text:"PC-1: 192.168.3.6 / Gateway 192.168.3.6", ok:true},
    {id:"A4", text:"PC-2: 192.168.3.6 / Gateway 192.168.3.6", ok:false, nd:"Logikfehler", ex:"PC-2 liegt nicht im gleichen Netz wie Gateway 192.168.3.6."},
    {id:"A5", text:"DNS-Server zeigt auf 192.168.3.6", ok:true},
    {id:"A6", text:"DHCP verteilt 192.168.3.6 bis 192.168.3.6", ok:true},
    {id:"A7", text:"Client bekommt 10.0.0.55 aus dem DHCP-Bereich", ok:false, nd:"Logikfehler", ex:"10.0.0.55 passt nicht zum DHCP-Bereich 192.168.3.6 bis 100."},
  ...
  function levelFor(mode,index,total) {
    ...
    if(["mathSprint","textMathSprint","logicSprint","concentrationSprint","visualIQSprint","itSprint","knowledgeSprint","techniqueSprint","errorTrainingPrep"].includes(mode)) return phase<.25?"medium":"hard";
    return "medium";
  }
  ...
  function buildMatrixOnlyQuiz(amount=24) {
    ...
    let tries = 0;
    while(out.length < amount && tries < amount * 120) {
      tries++;
    ...
    while(out.length < amount) {
      const q = Generators.matrix("medium");
      q.group = groupFor(q.cat);
      q.signature = stableSignature(q) + "|matrix-fallback|" + out.length + "|" + Math.random();
      out.push(q);
    }
  ...
  const Generators = {
    ...
    itScenario() {
      return choice([
        ()=>makeMC("IT Fehleranalyse",35,"Ein PC hat IP 192.168.3.6, Gateway 192.168.3.6, DNS leer. Internet per IP geht, Webseiten-Namen gehen nicht. Was ist wahrscheinlich falsch?",["DNS fehlt","RAM defekt","Monitor defekt","Tastatur defekt"],0,"Wenn IP geht, aber Namen nicht, ist DNS wahrscheinlich das Problem."),
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  // V8.3.6 Aufgabenbank-Generatoren: bereinigte digitale Aufgaben + PDF-Bildaufgaben.
  ...
  Generators.bookSymbolArithmetic = function(level){
    // V8.3.6: schwache selbst erzeugte Symbolarithmetik-Aufgaben wurden aus der Bank entfernt.
  ...
  function generateQuestionForMode(mode,index,total) {
    ...
    if(mode==="ctcLohr") {
      ...
      else if(index<49){ q=choice([Generators.ctcMathSprint,Generators.bookTextMath])(index<44?"easy":"medium"); q.block="2. Mathe"; }
      else if(index<67){ q=choice([Generators.matrixBook,Generators.bookNumberSeries,Generators.bookVisualLogic,Generators.series,Generators.matrix,Generators.opinionFact,Generators.statementLogic,Generators.symbolSeries])("medium"); q.block="3. Logik"; }
    ...
    const pools = {
      math:[Generators.math,Generators.bookAlgebra,Generators.bookTextMath], logic:[Generators.logic,Generators.bookSymbolArithmetic,Generators.bookStatementLogic,Generators.bookRatioLogic],
  ...
  function renderProfileManager(){
    ...
    return `<div class="premium-card profile-manager-card">
      <span class="coach-badge">Profilverwaltung V8.3.6</span>
  ...
  function renderPremiumDashboard(){
    ...
    if($("premiumGreeting")) $("premiumGreeting").textContent = "Willkommen im BPS-Trainer";
    if($("premiumSubtitle")) $("premiumSubtitle").textContent = "© Ugurcan Bozkurt";
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  const MODE_TABS = {
    simulation:{title:"Simulationen", desc:"Realistische BPS-/CTC-Testläufe mit Zeitdruck und Ergebnis erst am Ende.", modes:["ctcLohr","bps","ctc","jogging"]},
    ...
    block:{title:"Blocktraining", desc:"Kurze Sprintblöcke für Schwächen und Zeitdruck. Spezialblöcke für Matrizen und Satzergänzung sind hier sichtbar, aber nicht als separater Testbereich.", modes:["mathSprint","textMathSprint","logicSprint","concentrationSprint","visualIQSprint","itSprint","knowledgeSprint","matrixOnlySprint","sentenceSprint","numberSeriesBookSprint","textComprehensionSprint","visualLogicBookSprint","techniqueSprint","errorTrainingPrep"]}
  };
  ...
  const TRAINING_SHEET_CATEGORIES = [
    ...
    {key:"math", label:"Mathe", icon:"➗", note:"Grundrechnen, Kopfrechnen und Mathe-Sprints für Zeitdruck.", modes:["math","mathSprint","textMathSprint","numberSeriesBookSprint"]},
    {key:"logic", label:"Logik", icon:"🧩", note:"Zahlenreihen, Muster, Symbolfolgen und klassische Logikaufgaben.", modes:["logic","logicSprint","numberSeriesBookSprint"]},
    {key:"matrix", label:"Matrizen", icon:"▦", note:"Gezieltes Matrixtraining inklusive der übernommenen PDF-Matrizen.", modes:["matrixOnlySprint"]},
    ...
    {key:"language", label:"Sprache", icon:"✍️", note:"Englisch und Satzergänzung gezielt üben.", modes:["english","sentenceSprint","textComprehensionSprint"]},
    {key:"knowledge", label:"Wissen", icon:"📚", note:"Allgemeinwissen, Alltag, Geschichte, Geografie, Politik und Wissens-Sprint.", modes:["general","knowledgeSprint"]},
  ...
  function renderAppNav(){
    ...
    // Stable rule V8.3.6: Mobile-Navigation wird direkt an den Body gehängt.
  ...
  function openTrainingSheet(tab){
    ...
    requestAnimationFrame(()=>{
      ...
      document.body.classList.add("training-sheet-open");
    });
  }
  ...
  function closeTrainingSheet(){
    ...
    document.body.classList.remove("training-sheet-open");
  }
  ...
  function renderSettingsSection(){
    ...
    const frameworkBlock=`<div class="premium-card section-card settings-card"><b>Framework</b><p class="small">Production Feature-Ready V8.3.6 · Module bleiben getrennt: EDV, Route, Simulation, Storage und Cloud.</p><button class="ghost" onclick="App.showFrameworkHealth()">Framework prüfen</button></div>`;
  ...
  function renderModes() {
    ...
    // V8.3.6: mobile-nav-attached wird ausschließlich in renderAppNav/stabilizeMobileBottomNav gesetzt.
    renderPremiumDashboard();
    renderAppNav();
    renderMobileTopNav();
    ...
    if(section==="practice"){
      ...
      grid.innerHTML = `<div class="training-native-launch v815-launch"><button type="button" onclick="App.openTrainingSheet()"><span>▣</span><b>Trainingsmenü öffnen</b><small>${TRAINING_SHEET_CATEGORIES.length} Kategorien · ${allTrainingModes().length} Module · Aufgaben erst nach Start</small></button></div>`;
      renderTrainingSheetContent();
    } else {
      grid.innerHTML="";
      if($("modeTabs")) $("modeTabs").innerHTML="";
  ...
  function showDatabaseInfo() {
    ...
    panel.innerHTML = `
      ...
      <div class="db-status-card"><b>Aufgabenbank-Import:</b><br><code>source, sourcePage, category, group, subtype, difficulty, question, answers, correct, explanation, tags, verified</code><br><span class="small">Status V8.3.6: Textaufgaben digital ergänzt.</span></div>
  ...
  async function showFrameworkHealth() {
    ...
    const html = `
      ...
      <div class="health-row"><span>Version</span><strong>${escHTML(summary.framework || (FRAMEWORK.name + " " + FRAMEWORK.version))}</strong></div>
  ...
  try {
    if(window.AppModuleLoader && !window.__CORE_MODULE_REGISTERED__) {
      ...
      AppModuleLoader.register({id:"core-training-framework", version:APP_VERSION, enabled:true, description:"Kerntraining, Aufgabenbank, Highscore und Cloud-Anbindung", init:function(){}});
  ...
  async function addCloudTestScore(){
    const el=$("cloudHighscoreCard");
    try {
      if(el) el.innerHTML=`<span class="coach-badge">Cloud Highscore · Test läuft</span><div class="coach-action">Testeintrag wird gespeichert...</div><div class="small">Button erkannt · Insert gestartet · Supabase antwortet gleich sichtbar.</div>`;
      const res = await CloudHighscoreEngine.insertDebugEntry();
      if(el) el.innerHTML=`<span class="coach-badge">Cloud Highscore · Test</span><div class="coach-action">${res && res.ok ? "Testeintrag gespeichert" : "Testeintrag fehlgeschlagen"}</div><div class="small">HTTP ${escHTML(res && res.status ? res.status : "?")} · ${escHTML(res && res.variant ? res.variant : "full")} · ${escHTML(res && res.authMode ? res.authMode : "auto")} · Verifikation: ${res && res.verified ? "gefunden" : "noch nicht in Topliste"} · Datensätze gelesen: ${escHTML(res && res.verifyCount != null ? res.verifyCount : "?")}</div>${res && res.warning ? `<div class="small cloud-error-note">Fallback-Hinweis: ${escHTML(res.warning)}</div>` : ""}<div class="cloud-refresh-row"><button class="ghost" type="button" data-action="cloud-refresh">Ranking neu laden</button><button class="ghost" type="button" data-action="cloud-test">Nochmals testen</button></div>`;
      setTimeout(()=>CloudHighscoreEngine.refreshDashboard(), 900);
    } catch(error) {
