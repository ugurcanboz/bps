...
/* Question Factory, Group Mapping, EDV-Schema, Generatoren, Route- und EDV-Fragenaufbau. */
  ...
  const Generators = {
    ...
    series(level) {
      ...
      else if(nd===3){const start=choice([1,2,3]),m=choice([2,3]); const len=m===3?5:6; for(let i=0;i<len;i++)seq.push(start*Math.pow(m,i)); n1=start*Math.pow(m,len); n2=start*Math.pow(m,len+1); ex=`Immer ×${m}.`;}
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  // Fragenübersicht: aktuelle Seite (30 Fragen pro Seite)
  var _navPage = 0;
  function _navPagePrev() { _navPage = Math.max(0, _navPage - 1); updateQuestionNav(); }
  function _navPageNext() {
    const total = isAdaptiveElite() ? MODES.ctc.amount : state.quiz.length;
    _navPage = Math.min(Math.ceil(total / 30) - 1, _navPage + 1);
    updateQuestionNav();
  }
  ...
  function updateQuestionNav() {
    ...
    // Seite automatisch auf aktuelle Frage ausrichten (nur wenn außerhalb sichtbarer Seite)
    const autoPage = Math.floor(state.current / PAGE_SIZE);
    if (autoPage !== _navPage) _navPage = autoPage;

    const page = Math.max(0, Math.min(_navPage, totalPages - 1));
    ...
    // Kopfzeile: Seitenanzeige + Pfeil-Buttons
    const header = document.createElement("div");
    header.className = "qnav-header";
    const prevBtn = document.createElement("button");
    prevBtn.className = "qnav-btn qnav-prev";
    prevBtn.innerHTML = "&#8249;";
    prevBtn.disabled = page <= 0;
    prevBtn.onclick = _navPagePrev;
    const label = document.createElement("span");
    label.className = "qnav-label";
    label.innerHTML = `Seite ${page + 1} / ${totalPages}<br><small>${from + 1}–${to} von ${total}</small>`;
    const nextBtn = document.createElement("button");
    nextBtn.className = "qnav-btn qnav-next";
    nextBtn.innerHTML = "&#8250;";
    nextBtn.disabled = page >= totalPages - 1;
    nextBtn.onclick = _navPageNext;
    header.appendChild(prevBtn);
    header.appendChild(label);
    header.appendChild(nextBtn);
    nav.appendChild(header);

    // Dots für aktuelle Seite
    ...
    // Fußzeile: Seitensprung-Buttons (nur bei mehr als 1 Seite)
    if (totalPages > 1) {
      const footer = document.createElement("div");
      footer.className = "qnav-footer";
      for (let p = 0; p < totalPages; p++) {
        const btn = document.createElement("button");
        btn.className = "qnav-page-btn" + (p === page ? " active" : "");
        const pFrom = p * PAGE_SIZE + 1;
        const pTo = Math.min((p + 1) * PAGE_SIZE, total);
        btn.textContent = `${pFrom}–${pTo}`;
        btn.title = `Fragen ${pFrom}–${pTo}`;
        const pCopy = p;
        btn.onclick = () => { _navPage = pCopy; updateQuestionNav(); };
