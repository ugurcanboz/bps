...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  function saveResult(percent,dur,avg) {
    ...
    const userId=activeLearnerId();
    const latest={date:new Date().toISOString(),userId,learnerId:userId,player_name:profile.name||userId||"Gast",player_id:profile.player_id||userId,mode:state.selectedMode,title:MODES[state.selectedMode].title,score:state.score,total:state.history.length,percent,duration:dur,avg,cats,exam:{...state.exam},aiSession,appVersion:APP_VERSION};
    allData.push(latest);
    ...
    try { if(window.EGTAdminPortal && EGTAdminPortal.trackEvent) EGTAdminPortal.trackEvent({userId, module:state.selectedMode, mode:state.selectedMode, score:percent, passed:percent>=70, correct:percent>=70, errors:Object.keys(cats).filter(k=>cats[k].r<cats[k].n), total:state.history.length}); } catch(e) {}
