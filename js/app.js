/* STABLE SINGLE-FILE BUNDLE V8.3.8 · js/src wurde bewusst entfernt, damit GitHub Pages nur diese Runtime lädt. */
...
- MigrationPath auf V8.3.8 aktualisiert
...
window.App = (() => {
  ...
  const APP_VERSION = "8.3.8-cloud-runtime-bridge";
  ...
  const PWA_CONFIG = Object.freeze({
    ...
    note:"V8.3.8 Local File Guard Stable: GitHub-Pages-Fallback, Desktop/Mobile getrennt, relative Pfade, Update-Prompt und temporäre Cache-Diagnose stabilisiert."
  });
...
/* StorageEngine, IndexedDBEngine, DatabaseBridge und Datenmigration. */
  ...
  const DATA_MODEL = {
    ...
    migrationPath:["1.0","1.1","1.2","1.2.1","1.3","1.4","1.4.1","1.5","1.5.1","1.6","1.6.1","1.7","1.7.1","1.7.2","1.8","1.8.1","1.9","1.9.1","2.0","2.1","2.2","2.3","2.3.1","2.4","2.5","2.5.1","2.6","2.7","2.8","2.8.1","2.9","2.9.1","3.1","3.1.1","3.2","3.3","3.3.1","3.3.2","3.4","3.4.1","3.5","3.5.1","3.5.2","3.5.3","3.5.4","3.5.5","3.6.0","3.6.1","3.6.2","4.0.0","4.1.0","4.1.1","4.2.0","4.2.1","5.0.0","5.0.1","5.0.2","5.1.0","5.1.1","5.1.2","5.1.3","5.1.4","6.0.0","6.1.0","6.1.1","7.0.0","8.3.8","8.3.8"],
    futureEngine:"active",
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const CoachEngine = {
    build(results) {
      ...
      return {version:"8.3.8",readiness,memory,weaknesses,cognitive,recommendation,adaptive,dynamicMix,learning,simulation,focus:TrainingFocusEngine.current(),updatedAt:new Date().toISOString()};
    },
    renderDashboard(coach) {
      ...
      return `<div class="premium-card ai-card">
        <span class="coach-badge">AI Stable Core V8.3.8</span>
...
/* Question Factory, Group Mapping, EDV-Schema, Generatoren, Route- und EDV-Fragenaufbau. */
  ...
  const EDV_SCHEMA = [
    {id:"A1", text:"Router LAN: 192.168.3.8", ok:true},
    {id:"A2", text:"Switch verbindet PC-1, PC-2, Server und Drucker", ok:true},
    {id:"A3", text:"PC-1: 192.168.3.8 / Gateway 192.168.3.8", ok:true},
    {id:"A4", text:"PC-2: 192.168.3.8 / Gateway 192.168.3.8", ok:false, nd:"Logikfehler", ex:"PC-2 liegt nicht im gleichen Netz wie Gateway 192.168.3.8."},
    {id:"A5", text:"DNS-Server zeigt auf 192.168.3.8", ok:true},
    {id:"A6", text:"DHCP verteilt 192.168.3.8 bis 192.168.3.8", ok:true},
    {id:"A7", text:"Client bekommt 10.0.0.55 aus dem DHCP-Bereich", ok:false, nd:"Logikfehler", ex:"10.0.0.55 passt nicht zum DHCP-Bereich 192.168.3.8 bis 100."},
  ...
  const Generators = {
    ...
    itScenario() {
      return choice([
        ()=>makeMC("IT Fehleranalyse",35,"Ein PC hat IP 192.168.3.8, Gateway 192.168.3.8, DNS leer. Internet per IP geht, Webseiten-Namen gehen nicht. Was ist wahrscheinlich falsch?",["DNS fehlt","RAM defekt","Monitor defekt","Tastatur defekt"],0,"Wenn IP geht, aber Namen nicht, ist DNS wahrscheinlich das Problem."),
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  // V8.3.8 Aufgabenbank-Generatoren: bereinigte digitale Aufgaben + PDF-Bildaufgaben.
  ...
  Generators.bookSymbolArithmetic = function(level){
    // V8.3.8: schwache selbst erzeugte Symbolarithmetik-Aufgaben wurden aus der Bank entfernt.
  ...
  function renderProfileManager(){
    ...
    return `<div class="premium-card profile-manager-card">
      <span class="coach-badge">Profilverwaltung V8.3.8</span>
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  function renderAppNav(){
    ...
    // Stable rule V8.3.8: Mobile-Navigation wird direkt an den Body gehängt.
  ...
  function renderSettingsSection(){
    ...
    const frameworkBlock=`<div class="premium-card section-card settings-card"><b>Framework</b><p class="small">Production Feature-Ready V8.3.8 · Module bleiben getrennt: EDV, Route, Simulation, Storage und Cloud.</p><button class="ghost" onclick="App.showFrameworkHealth()">Framework prüfen</button></div>`;
  ...
  function renderModes() {
    ...
    // V8.3.8: mobile-nav-attached wird ausschließlich in renderAppNav/stabilizeMobileBottomNav gesetzt.
    renderPremiumDashboard();
    renderAppNav();
    renderMobileTopNav();
  ...
  function showDatabaseInfo() {
    ...
    panel.innerHTML = `
      ...
      <div class="db-status-card"><b>Aufgabenbank-Import:</b><br><code>source, sourcePage, category, group, subtype, difficulty, question, answers, correct, explanation, tags, verified</code><br><span class="small">Status V8.3.8: Textaufgaben digital ergänzt.</span></div>
