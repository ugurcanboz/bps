...
/*
===============================================================================
Eignungstest-Trainer V7.0.1 · Deep Deep Cleanup Stable
===============================================================================
Struktur:
...
- MigrationPath auf V7.0.1 aktualisiert
...
window.App = (() => {
  const STORE_KEY = "eignungstest_trainer_v701_results";
  ...
  const APP_VERSION = "7.0.1-deep-cleanup";
  const PROFILE_KEY = "eignungstest_trainer_profile_v701";
  ...
  const FOCUS_KEY = "eignungstest_trainer_focus_v701";
  ...
  const PWA_CONFIG = Object.freeze({
    ...
    cacheName:"eignungstest-trainer-v701-deep-cleanup-cache",
    icons:["icons/icon-180.png","icons/icon-192.png","icons/icon-512.png","icons/maskable-512.png"],
    status:"indexeddb-primary-active",
    note:"V7.0.1 Deep Deep Cleanup Stable: EDV-Multi-Choice, Route-Memory und Visual-Fixes stabil zusammengeführt, Legacy-Reste bereinigt und Cache/Version vereinheitlicht."
  });
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const CoachEngine = {
    build(results) {
      ...
      return {version:"7.0.1",readiness,memory,weaknesses,cognitive,recommendation,adaptive,dynamicMix,learning,simulation,focus:TrainingFocusEngine.current(),updatedAt:new Date().toISOString()};
    },
    renderDashboard(coach) {
      ...
      return `<div class="premium-card ai-card">
        <span class="coach-badge">AI Stable Core V7.0.1</span>
  ...
  const QuestionBankEngine = {
    ...
    importBatch(rawItems=[], source="ocr-import") {
      if(!FEATURE_FLAGS.questionBankRuntimeImport) {
        ...
        QUESTION_BANK.lastImport={source, imported:0, invalid:validation.filter(v=>!v.ok).length, reviewed:validation.length, blocked:true, reason:"Question Bank Runtime Import ist in V7.0.1 bewusst deaktiviert.", createdAt:new Date().toISOString()};
  ...
  const Generators = {
    ...
    bigEDVMulti() {
      ...
      return makeMC(
        ...
        "edvmulti",
        {edvCorrectIds:correctIds, edvMultiSelected:[], edvRequiredCount:correctIds.length, block:"5. EDV Kenntnisse", signatureSeed:"edv-multi-v701-cloud-highscore"}
      );
    },
    bigEDVCovered(slot) {
      return makeMC(
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  function renderProfileManager(){
    ...
    return `<div class="premium-card profile-manager-card">
      <span class="coach-badge">Profilverwaltung V7.0.1</span>
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  function renderSettingsSection(){
    ...
    const frameworkBlock=`<div class="premium-card"><b>Framework</b><p class="small">Core Cleanup V7.0.1 · Module bleiben getrennt: EDV, Route, Simulation, Storage und Cloud.</p><button class="ghost" onclick="App.showFrameworkHealth()">Framework prüfen</button></div>`;
  ...
  function showDatabaseInfo() {
    ...
    panel.innerHTML = `
      ...
      <div class="db-status-card"><b>Aufgabenbank-Import:</b><br><code>source, sourcePage, category, group, subtype, difficulty, question, answers, correct, explanation, tags, verified</code><br><span class="small">Status V7.0.1: vorbereitet, runtime-deaktiviert.</span></div>
  ...
  function exportBackup() {
    ...
    a.href = url;
    a.download = "eignungstest-trainer-backup-v7-0-1.json";
