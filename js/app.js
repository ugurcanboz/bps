...
/* StorageEngine, IndexedDBEngine, DatabaseBridge und Datenmigration. */
  ...
  function routeQuestionBankCandidates(filter, level, modeOverride) {
    ...
    if ((activeMode === "novuraExams" || activeMode === "novuraExams") && !finalFilter.examTarget) {
      finalFilter.examTarget = "novuraExams";
    } else if (activeMode === "assessments" && !finalFilter.examTarget) {
      finalFilter.examTarget = "assessments";
    }
    return {
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const DataReadiness = {
    ...
    evaluate(results) {
      const sims=(results||[]).filter(r=>r.mode==="novuraExams" && (r.total||0)>=60);
  ...
  const TrainingFocusEngine = {
    options:{
      auto:{key:"auto",label:"KI Auto",group:null,mode:"novuraExams",hint:"KI darf den Schwerpunkt frei empfehlen."},
  ...
  const RecommendationEngine = {
    ...
    build(readiness, weaknesses, memory) {
      ...
      if(!readiness.ready) return {title:"Coach sammelt Daten",text:`Noch ${readiness.remaining} vollständige Simulation${readiness.remaining===1?"":"en"} nötig. Blocktraining zählt als Zusatzdaten, ersetzt aber die Diagnose nicht.`,mode:"novuraExams"};
      ...
      return {title:"Stabilisierung",text:"Datenbasis ist aktiv. Nächster sinnvoller Lauf: Simulation unter Prüfungsbedingungen.",mode:"novuraExams"};
    }
  };
  ...
  const DynamicGeneratorEngine = {
    ...
    buildMix(coach, mode="novuraExams") {
      const mix=[];
      const focus=TrainingFocusEngine.current();
    ...
    buildQuestion(index,total,coach) {
      ...
      const focus=TrainingFocusEngine.current();
      const mix=this.buildMix(coach,"novuraExams");
  ...
  const HighscoreEngine = (window.EGTHighscoreDashboardEngine && typeof window.EGTHighscoreDashboardEngine.create === "function") ? window.EGTHighscoreDashboardEngine.create({
    ...
    build(results=[]) {
      ...
      const bestSimulation=list.filter(r=>r.mode==="novuraExams")[0]||null;
      const byMode={};
...
/* Question Bank Schema, Quality Engine, Modusverträge, Blocklimits und Blockinfo. */
  ...
  const QualityEngine = {
    ...
    validateSimulation() {
      const oldMode = state.selectedMode;
      state.selectedMode = "novuraExams";
      const quiz = buildQuiz();
      state.selectedMode = oldMode;
      const blocks = {};
    ...
    validateHighscore() {
      try {
        const sample=[{mode:"math",title:"Mathe",score:18,total:20,percent:90,duration:"1 Min.",cats:{}},{mode:"novuraExams",title:"Simulation",score:70,total:62,percent:75,duration:"10 Min.",cats:{}}];
    ...
    validateAIEngines() {
      try {
        const validationResults=[
          {mode:"novuraExams",title:"Simulation",total:62,score:60,percent:65,cats:{Mathe:{n:9,r:5,t:1000}},aiSession:{total:62,correct:60,groups:{Mathe:{total:9,correct:5,wrong:4,avgMs:10000,pressure:2,open:1}},errors:{pressureWrong:2}}},
          {mode:"novuraExams",title:"Simulation",total:62,score:70,percent:75,cats:{"Visual IQ":{n:10,r:4,t:1000}},aiSession:{total:62,correct:70,groups:{"Visual IQ":{total:10,correct:4,wrong:6,avgMs:12000,pressure:3,open:0}},errors:{patternError:6}}},
          {mode:"novuraExams",title:"Simulation",total:62,score:75,percent:81,cats:{Konzentration:{n:15,r:8,t:1000}},aiSession:{total:62,correct:75,groups:{Konzentration:{total:15,correct:8,wrong:7,avgMs:13000,pressure:4,open:2}},errors:{timeout:2,pressureWrong:4}}}
        ];
...
const MODES = {
    ...
    assessments:{title:"Novura Assessments",amount:100,memory:true,instant:false,desc:"Novura Assessments-Prüfungssimulation mit 50 Sekunden pro Aufgabe, ohne Hilfe während der Prüfung und Auswertung erst am Ende.",tags:["Novura Assessments","50 Sek./Aufgabe","keine Hilfe"]},
    novuraExams:{title:"5. Eignungstest Elite",amount:100,memory:true,instant:false,desc:"Harter Elite-Modus mit Zeitdruck, Gedächtnis, IT, Logik, Mathematik, Konzentration und angepasster Trainingssteuerung.",tags:["Elite","Zeitdruck","Leistungsprofil"]},
    ...
    novuraExams:{title:"Novura Exams · IT Assessment",amount:(window.NovuraExamsStructureEngine&&window.NovuraExamsStructureEngine.totalCount&&window.NovuraExamsStructureEngine.totalCount())||68,memory:false,instant:false,desc:"Realer Novura Exams-Lohr Ablauf mit 7 Blöcken: 40 Allgemeinwissen in 7 Minuten, Mathe-Sprint, Regelrechnung, Zahlenreihen, Buchstaben-Zählen, Tabellenlesen und 13-Minuten-FlowLogic.",tags:["Novura Exams-Lohr","ca. 37 Min.","keine Hilfe"]}
  };
  ...
  const MODE_CONTRACTS = {
    ...
    novuraExams:{requiredCount:(window.NovuraExamsStructureEngine&&window.NovuraExamsStructureEngine.totalCount&&window.NovuraExamsStructureEngine.totalCount())||68},
    novuraExams:{adaptive:true}
  };

  const NOVURA_EXAMS_BLOCK_LIMITS = {
    "1. Allgemeinwissen": 7*60,
    "2. Mathematik Sprint": 22,
    "3. Novura Exams-Regelrechnung": 40,
    "4. Buchstaben-Konzentration": 40,
    "5. Tabellen ablesen": 90,
    "6. Novura Exams-Logik: Wenn-Dann-Ablauf": 13*60
  };
...
/* Question Factory, Group Mapping, EDV-Schema, Generatoren, Route- und EDV-Fragenaufbau. */
  ...
  let state = {
    ...
    usedQuestions:new Set(),duel:null, novuraExamsBlockRemaining:{}, novuraExamsCurrentBlock:null, shownBlockIntro:{}, pendingBlock:null, routeTimers:[], activeModeTab:"basic", activeSheetCategory:0, activeAppSection:"dashboard", exam:{enabled:false,hardcore:false,lockBack:false,started:false}
  };
  ...
  function isAdaptiveElite() {
    return state.selectedMode === "novuraExams";
  }
  ...
  function levelFor(mode,index,total) {
    ...
    if(mode==="novuraExams") return phase<.20?"medium":"hard";
    if(mode==="assessments") return phase<.3?"easy":phase<.75?"medium":"hard";
  ...
  const Generators = {
    ...
    fractionRuleEignungstest(level) {
      // Novura Exams-Regel: Zähler > Nenner → Zähler minus Nenner; Nenner > Zähler → Zähler plus Nenner
      const count = level==="hard" ? 10 : level==="easy" ? 6 : 8;
      const pairs = [];
      for(let i=0;i<count;i++){
        const a=rand(1,30); let b=rand(1,30);
        while(b===a) b=rand(1,30); // a===b ist in der Novura Exams-Regel nicht definiert → vermeiden
      ...
      const visual = `<div class="visualBox"><div class="concentration-rule"><b>Novura Exams-Regel:</b> Zähler &gt; Nenner → Zähler <b>minus</b> Nenner &nbsp;|&nbsp; Nenner &gt; Zähler → Zähler <b>plus</b> Nenner</div><div class="novuraExams-fraction-grid">${pairs.slice(0,-1).map(p=>`<div class="novuraExams-fraction solved"><span class="frac-top">${p.a}</span><span class="frac-sep"></span><span class="frac-bot">${p.b}</span><span class="frac-eq">= ${p.result}</span></div>`).join('')}<div class="novuraExams-fraction question"><span class="frac-top">${qPair.a}</span><span class="frac-sep"></span><span class="frac-bot">${qPair.b}</span><span class="frac-eq">= <b>?</b></span></div></div></div>`;
      ...
      return makeMC("Novura Exams-Regel Konzentration",35,`Wende die Novura Exams-Regel an: Was ergibt <b>${qPair.a}/${qPair.b}</b>?`,opts,optIdx(opts,correct),expl,"focusgrid",{focusHTML:visual,signatureSeed:seedStr});
    },
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  Generators.assessmentsMathTrainer = function(level){
  ...
  const QuizBuildPipelineEngine = (window.EGTQuizBuildPipelineEngine && typeof window.EGTQuizBuildPipelineEngine.create === "function") ? window.EGTQuizBuildPipelineEngine.create({
    ...
    setupMode: function(mode){ if(mode === "novuraExams") state.adaptiveMemoryPool = generateMemoryQuestions(); },
    ...
    activeBranchIdForQuiz:function(){ try { return String(localStorage.getItem("assessments_branch") || "wissen"); } catch(e) { return "wissen"; } },
  ...
  function generateQuestionForModeInternal(mode,index,total,coach) {
    ...
    if(mode==="novuraExams") {
      ...
      if(index<40){
        ...
        q = fromQuestionBank({group:"Mathe"}, () => choice([Generators.novuraExamsMathSprint,Generators.bookTextMath])(index<44?"easy":"medium"), index<44?"easy":"medium");
        q.block="2. Mathe";
      }
      else if(index<67){
      ...
      q.signature=stableSignature(q)+"|novuraExams|"+(q.signatureSeed||index); 
      if (!q.id) { q.id = "dyn_" + Math.random().toString(36).substr(2, 8); }
      return attachBranchPoolMeta(q, mode);
    }
    if(mode==="novuraExams") return attachBranchPoolMeta(buildAdaptiveQuestion(index,total), mode);
  ...
  function buildQuizInternal() {
    ...
    const memoryQs = (mode !== "novuraExams" && m.memory) ? generateMemoryQuestions().map(q=>{q.group="Gedächtnis"; q.signature=stableSignature(q); return q;}) : [];
    ...
    const finalQuiz = (mode==="novuraExams" || mode==="novuraExams") ? out : shuffle(out);
  ...
  function closeRuntimeOverlaysForHome(){
    ...
    try{
      ...
      document.body.classList.remove(
        ...
        "training-sheet-open","assessments-sheet-open","training-sheet-compact","training-sheet-desktop",
  ...
  function goHomeHard(reason){
    ...
    state.novuraExamsCurrentBlock=null;
    state.duel=null;
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  const MODE_TABS = {
    simulation:{title:"Simulationen", desc:"Realistische prüfungsnah-Testläufe mit Zeitdruck und Ergebnis erst am Ende.", modes:["novuraExams","assessments","novuraExams","jogging"]},
  ...
  const TRAINING_SHEET_CATEGORIES = [
    {key:"simulation", label:"Simulationen", icon:"🎯", note:"Prüfungsnahe Läufe: Auswahltest, Novura Exams, Elite und Jogging. Im Test gibt es keine Hinweise.", modes:["novuraExams","assessments","novuraExams","jogging"]},
  ...
  const SIMULATION_MODES = ["novuraExams","novuraExams","assessments","jogging"];

  const TOP_NAV_TABS = {
    home:[
      {key:"start", icon:"🏠", label:"Start", action:"home"},
      {key:"novuraExams", icon:"▶️", label:"Novura Exams", action:"mode", mode:"novuraExams", section:"practice"},
      {key:"elite", icon:"⚡", label:"Elite", action:"mode", mode:"novuraExams", section:"practice"},
  ...
  function openTrainingSheet(tab){
    ...
    requestAnimationFrame(()=>{
      ...
      document.body.classList.add("training-sheet-open","assessments-sheet-open");
    });
  }
  ...
  function closeTrainingSheet(){
    ...
    document.body.classList.remove("training-sheet-open","assessments-sheet-open");
  }
  ...
  function setAppSection(section){
    ...
    if(section==="simulation") { section="practice"; state.activeModeTab="simulation"; state.selectedMode="novuraExams"; }
    ...
    showOnly("start");
    if(section==="practice" && !Object.values(MODE_TABS).some(t=>t.modes.includes(state.selectedMode))) state.selectedMode=(state.activeModeTab==="simulation"?"novuraExams":"math");
    renderModes();
  ...
  function startQuiz() {
    ...
    if(window.EGTSimulation && typeof window.EGTSimulation.start === "function" && window.EGTSimulation.isReady && window.EGTSimulation.isReady()) {
      return window.EGTSimulation.start({
        ...
        modus: state.selectedMode === "novuraExams" ? "Novura Assessments" : state.selectedMode,
        ...
        zeitProFrage: state.selectedMode === "novuraExams" ? 22 : (state.selectedMode === "assessments" ? 50 : 50),
        features: {
          coachHilfe: !(state.selectedMode === "novuraExams" || state.selectedMode === "assessments" || state.selectedMode === "novuraExams"),
          merken: true,
          skip: true,
  ...
  function startQuizInternal() {
    ...
    state.testStart=new Date(); state.testEnd=null; state.exam.started=true; applyExamBodyClass(); state.novuraExamsBlockRemaining={...NOVURA_EXAMS_BLOCK_LIMITS}; state.novuraExamsCurrentBlock=null; state.shownBlockIntro={}; state.pendingBlock=null;
    showOnly("quiz");
    simulationHostCall("showQuestion", [], function(){ return showQuestion(); });
  }
  function startNovuraExamsBlockFromIntro() {
    try{ closeTrainingSheet(); }catch(e){}
  ...
  function updateTrainingCockpitUi(q, totalDisplay) {
    ...
    timerEl.dataset.label = state.selectedMode === "novuraExams" ? "Blockzeit" : "Zeit";
  ...
  function safeMathMasteryFlag(questionId) {
    try {
      ...
      return localStorage.getItem("assessments_math_understood_" + questionId) === "true";
    } catch(e) { return false; }
  }
  ...
  function showQuestionInternal(spIntro=false) {
    ...
    if(state.selectedMode==="novuraExams" && q.block && !state.shownBlockIntro[q.block] && !spIntro) {
    ...
    let totalDisplay=isAdaptiveElite()?MODES.novuraExams.amount:state.quiz.length;
    ...
    if(state.selectedMode==="novuraExams") {
      if(state.novuraExamsCurrentBlock!==q.block){state.novuraExamsCurrentBlock=q.block;}
      const isFlowLogicTask = q.type === "novuraExamsFlowLogic";
      const novuraExamsQuestionTime = Math.max(1, Number(q.specialTimeSeconds || q.time || (isFlowLogicTask ? 780 : 22)));
      state.timeLeft = novuraExamsQuestionTime;
      state.totalTimeForQuestion = novuraExamsQuestionTime;
      ...
      const novuraExamsRuleLabel = isFlowLogicTask ? 'Novura Exams-Logik: 13 Minuten · keine Hilfe · Fehler erst am Ende werten' : (q.timeModel === 'block' ? 'Novura Exams-Lohr: 7-Minuten-Wissensblock · ca. 10,5 Sek./Frage · keine Hilfe' : `Novura Exams-Lohr: ${novuraExamsQuestionTime} Sekunden · Eingabe/Antwort ohne Hilfe`);
      $("meta").innerHTML=`<span class="badge">${MODES[state.selectedMode].title}</span><span class="badge">${q.block}</span><span class="badge">${q.cat}</span><span class="badge">Frage ${state.current+1}/${totalDisplay}</span>${masteredBadge}<div class="block-timer-label">${novuraExamsRuleLabel}</div>${examStatusHtml()}`;
    } else {
      const simulationBps = state.selectedMode==="assessments";
      const trainingTime = Math.max(Number(q.time||25), 50);
      state.timeLeft=state.selectedMode==="novuraExams"?Math.max(12,Math.floor((q.time||25)*.8)):(simulationBps?50:trainingTime); state.totalTimeForQuestion=state.timeLeft;
      $("timer").textContent=state.timeLeft; $("timer").classList.remove("low","warn","critical");
      $("meta").innerHTML=`<span class="badge ${state.selectedMode==="novuraExams"?"hardcore-badge":""}">${MODES[state.selectedMode].title}</span><span class="badge">${q.cat}</span><span class="badge">Frage ${state.current+1}/${totalDisplay}</span>${masteredBadge}${simulationBps?'<span class="badge">50 Sek./Aufgabe · keine Hilfe</span>':''}${examStatusHtml()}`;
    }
    ...
    if(helpContainer) {
      const isStrictSimulation = state.selectedMode === "novuraExams" || state.selectedMode === "assessments" || state.selectedMode === "novuraExams";
  ...
  function tickTimerInternal() {
    ...
    if(state.selectedMode==="novuraExams") {
      ...
      updateTrainingCockpitUi(q, isAdaptiveElite()?MODES.novuraExams.amount:state.quiz.length);
      if(state.timeLeft<=0){ clearInterval(state.timer); if(q.type==="novuraExamsFlowLogic" && !state.history[state.current]){ submitNovuraExamsFlowLogicAnswer(true); return; } if((q.type==="novuraExamsRuleArithmeticInput" || q.type==="novuraExamsLetterScanInput" || q.type==="novuraExamsCoordinateTableInput" || q.type==="novuraExamsNumberSeriesInput") && !state.history[state.current]){ submitNovuraExamsInputAnswer(true); return; } if(!state.history[state.current])recordAnswer(null,false,true); nextQuestion(); }
    } else {
  ...
  function renderAnswers(q) {
    $("answers").innerHTML="";
    if(q.type==="novuraExamsFlowLogic") {
      return renderNovuraExamsFlowLogicAnswers(q);
    }
    if(q.type==="novuraExamsRuleArithmeticInput" || q.type==="novuraExamsLetterScanInput" || q.type==="novuraExamsCoordinateTableInput" || q.type==="novuraExamsNumberSeriesInput") {
      return renderNovuraExamsLohrInputAnswers(q);
    }
    if(q.type==="edvmulti") {
  ...
  function renderNovuraExamsLohrInputAnswers(q) {
    const answersEl = $("answers");
    if(!answersEl || !q) return null;
    const esc = escHTML;
    const previous = state.history[state.current] || null;
    if(q.type === "novuraExamsRuleArithmeticInput") {
      answersEl.innerHTML = `
        <div class="novura-exams-input-panel novuraExams-rule-panel">
          <div class="novura-exams-task-kicker">Novura Exams-Regelrechnung · Eingabe · kein Multiple Choice</div>
          <div class="novuraExams-rule-formula" aria-label="Regelrechnung">${esc(q.formula || q.q || "")}</div>
          <div class="novura-exams-rule-box">Regel: Rechne zuerst den Teil vor dem <b>/</b> und den Teil nach dem <b>/</b>. Wenn der Nenner kleiner ist als der Zähler: <b>Zähler minus Nenner</b>. Wenn der Nenner größer oder gleich ist: <b>Zähler plus Nenner</b>.</div>
          <label class="novura-exams-input-label">Deine Antwort<input id="novuraExamsAnswerInput" class="novura-exams-answer-input" inputmode="numeric" autocomplete="off" ${previous?"disabled":""} value="${previous && previous.givenIndex != null ? esc(previous.givenIndex) : ""}"></label>
          <button type="button" class="btn btn-primary novura-exams-submit" data-action="novuraExams-input-submit" ${previous?"disabled":""}>Antwort werten</button>
        </div>`;
    } else if(q.type === "novuraExamsLetterScanInput") {
      answersEl.innerHTML = `
        <div class="novura-exams-input-panel novuraExams-letter-panel">
          <div class="novura-exams-task-kicker">Buchstaben-Konzentration · Eingabe · kein Multiple Choice</div>
          <div class="novuraExams-letter-sequence" aria-label="Buchstabenfolge">${esc(q.sequence || "")}</div>
          <div class="novura-exams-rule-box">Zähle nur den abweichenden Buchstaben: <b>${esc(q.targetLetter || "")}</b>. Trage die Anzahl ein.</div>
          <label class="novura-exams-input-label">Anzahl<input id="novuraExamsAnswerInput" class="novura-exams-answer-input" inputmode="numeric" autocomplete="off" ${previous?"disabled":""} value="${previous && previous.givenIndex != null ? esc(previous.givenIndex) : ""}"></label>
          <button type="button" class="btn btn-primary novura-exams-submit" data-action="novuraExams-input-submit" ${previous?"disabled":""}>Antwort werten</button>
        </div>`;
    } else if(q.type === "novuraExamsNumberSeriesInput") {
      const seq = Array.isArray(q.sequence) ? q.sequence : [];
      answersEl.innerHTML = `
        <div class="novura-exams-input-panel novuraExams-series-panel">
          <div class="novura-exams-task-kicker">Zahlenreihen · Eingabe · kein Multiple Choice</div>
          <div class="novuraExams-series-sequence" aria-label="Zahlenreihe">${seq.map(n=>`<span class="novuraExams-series-num">${esc(n)}</span>`).join('<span class="novuraExams-series-sep">,</span>')}<span class="novuraExams-series-sep">,</span><span class="novuraExams-series-num novuraExams-series-q">?</span></div>
          <div class="novura-exams-rule-box">Erkenne das Muster der Reihe und gib die <b>nächste Zahl</b> ein.</div>
          <label class="novura-exams-input-label">Nächste Zahl<input id="novuraExamsAnswerInput" class="novura-exams-answer-input" inputmode="numeric" autocomplete="off" ${previous?"disabled":""} value="${previous && previous.givenIndex != null ? esc(previous.givenIndex) : ""}"></label>
          <button type="button" class="btn btn-primary novura-exams-submit" data-action="novuraExams-input-submit" ${previous?"disabled":""}>Antwort werten</button>
        </div>`;
    } else if(q.type === "novuraExamsCoordinateTableInput") {
      const cols = q.cols || [];
      const rows = q.rows || [];
      const grid = q.grid || [];
      ...
      const inputs = prompts.map((p,i)=>`<label class="novuraExams-coordinate-input"><span>${esc(p.key)}</span><input data-novura-exams-coord-index="${i}" maxlength="1" autocapitalize="characters" autocomplete="off" ${previous?"disabled":""} value="${esc(prevVals[i]||"")}"></label>`).join("");
      answersEl.innerHTML = `
        <div class="novura-exams-input-panel novuraExams-coordinate-panel">
          <div class="novura-exams-task-kicker">Tabellen ablesen · mehrere Eingaben · fehlend = X</div>
          <div class="novuraExams-table-wrap"><table class="novuraExams-coordinate-table">${head}${body}</table></div>
          <div class="novura-exams-rule-box">Lies die Koordinaten ab. Existiert eine Spalte oder Zeile nicht, trage <b>X</b> ein.</div>
          <div class="novuraExams-coordinate-grid">${inputs}</div>
          <button type="button" class="btn btn-primary novura-exams-submit" data-action="novuraExams-input-submit" ${previous?"disabled":""}>Antworten werten</button>
        </div>`;
    }
    if(previous) {
      ...
      const inp=$("novuraExamsAnswerInput");
      if(inp) setTimeout(()=>{ try{ inp.focus(); }catch(e){} }, 80);
    }
    return true;
  }

  function submitNovuraExamsInputAnswer(timeout=false){
    const q = state.quiz[state.current];
    if(!q || !["novuraExamsRuleArithmeticInput","novuraExamsLetterScanInput","novuraExamsCoordinateTableInput","novuraExamsNumberSeriesInput"].includes(q.type) || state.history[state.current]) return;
    let value = "";
    if(q.type === "novuraExamsCoordinateTableInput") {
      value = Array.from(document.querySelectorAll('[data-novura-exams-coord-index]')).sort((a,b)=>Number(a.dataset.novuraExamsCoordIndex)-Number(b.dataset.novuraExamsCoordIndex)).map(el=>String(el.value||"").trim().toUpperCase()).join('|');
    } else {
      const inp = $("novuraExamsAnswerInput");
  ...
  function nextQuestionInternal() {
    clearInterval(state.timer);
    const targetTotal=isAdaptiveElite()?MODES.novuraExams.amount:state.quiz.length;
    if(isAdaptiveElite()) {
  ...
  function skipQuestionInternal(){
    ...
    if(q && q.type==="novuraExamsFlowLogic") {
      const fb=$("feedback");
      if(fb){fb.innerHTML="<b>Novura Exams-Logik:</b> Diese 13-Minuten-Aufgabe muss über „Ablaufplan werten“ abgeschlossen werden."; fb.classList.remove("hidden");}
      return;
    }
    if(q && (q.type==="novuraExamsRuleArithmeticInput" || q.type==="novuraExamsLetterScanInput" || q.type==="novuraExamsCoordinateTableInput" || q.type==="novuraExamsNumberSeriesInput")) { submitNovuraExamsInputAnswer(true); return; }
    spQuestion();
  }
  function manualNextQuestionInternal(){
    const q=state.quiz[state.current];
    if(q && q.type==="edvmulti") { submitEdvMultiAnswer(); return; }
    if(q && q.type==="novuraExamsFlowLogic") { submitNovuraExamsFlowLogicAnswer(); return; }
    if(q && (q.type==="novuraExamsRuleArithmeticInput" || q.type==="novuraExamsLetterScanInput" || q.type==="novuraExamsCoordinateTableInput" || q.type==="novuraExamsNumberSeriesInput")) { submitNovuraExamsInputAnswer(); return; }
    clearInterval(state.timer);
    if(!state.history[state.current]){recordAnswer(null,false,false); state.questionStates[state.current]="skip";}
    if(isAdaptiveElite()&&state.current>=state.quiz.length-1&&state.current<MODES.novuraExams.amount-1)return nextQuestion();
  ...
  function updateQuestionNavInternal() {
    const total=isAdaptiveElite()?MODES.novuraExams.amount:state.quiz.length;
    const nav=$("questionNav");
  ...
  function renderNovuraExamsFlowLogic(q) {
    const v=$("visual");
    if(!v) return;
    try {
      if(window.NovuraExamsFlowLogicAdapter && typeof NovuraExamsFlowLogicAdapter.renderTo === "function") {
        NovuraExamsFlowLogicAdapter.renderTo(v, q, { mode:"novura-exams" });
        if(state.history[state.current]) {
          v.classList.add("novura-exams-flowlogic-locked");
        } else {
          v.classList.remove("novura-exams-flowlogic-locked");
        }
        return;
      }
      throw new Error("NovuraExamsFlowLogicAdapter fehlt.");
    } catch(error) {
      v.innerHTML = `<div class="visualBox"><b>Novura Exams-Logik konnte nicht geladen werden.</b><br><span class="small">${escHTML(error && error.message ? error.message : error)}</span></div>`;
    }
  }

  function renderNovuraExamsFlowLogicAnswers(q) {
    const box=$("answers");
    if(!box) return false;
    ...
    if(h) {
      ...
      box.innerHTML = `<div class="novura-exams-flowlogic-submit-panel novura-exams-flowlogic-result-summary">
        <div><b>Novura Exams-Logik ausgewertet</b><br><span class="small">${escHTML(h.explanation || "Ablaufplan wurde bewertet.")}</span></div>
        <span class="score-pill">${Number(fl.score || 0)} / ${Number(fl.maxScore || expected)} Punkte</span>
      </div>`;
      return true;
    }
    box.innerHTML = `<div class="novura-exams-flowlogic-submit-panel">
      ...
      <button type="button" class="btn btn-primary" data-action="novura-exams-flowlogic-submit">Ablaufplan werten</button>
    </div>`;
    return true;
  }

  function submitNovuraExamsFlowLogicAnswer(force) {
    const q=state.quiz[state.current];
    if(!q || q.type!=="novuraExamsFlowLogic" || state.history[state.current]) return;
    ...
    try { if(window.NovuraExamsFlowLogicAdapter && typeof NovuraExamsFlowLogicAdapter.summaryFromSession === "function") report=NovuraExamsFlowLogicAdapter.summaryFromSession(session); } catch(e) {}
    ...
    state.history[state.current]={
      ...
      skipped:false,
      explanation:`Novura Exams-Logik: ${scorePoints}/${maxPoints} Punkte (${percent}%). Korrekt: ${Number(summary.correct||0)}, Teilpunkte: ${Number(summary.partial||0)}, falsch: ${Number(summary.wrong||0)}, verpasst: ${Number(summary.missed||0)}.`,
      visualType:"novuraExamsFlowLogic",
      duration,
      allowed:state.totalTimeForQuestion,
    ...
    state.questionStates[state.current]=state.markedQuestions[state.current]?"mark":"done";
    updateQuestionNav();
    renderNovuraExamsFlowLogicAnswers(q);
    const fb=$("feedback");
    if(fb){ fb.innerHTML=`<b>Novura Exams-Logik ausgewertet.</b> ${scorePoints}/${maxPoints} Punkte (${percent}%).`; fb.classList.remove("hidden"); }
  ...
  function renderVisual(q) {
    ...
    if(q.type==="novuraExamsFlowLogic")renderNovuraExamsFlowLogic(q);
  ...
  function targetTotal() {return isAdaptiveElite()?MODES.novuraExams.amount:state.quiz.length;}
  function placeholderHistory(i, reason) {
    const q=state.quiz[i];
    return {q:q?.q || (reason==="notGenerated"?"Adaptive Aufgabe wurde nicht mehr erzeugt.":"Aufgabe wurde nicht bearbeitet."),cat:q?.cat||"Nicht bearbeitet",group:q?.group||(q?groupFor(q.cat):"Nicht bearbeitet"),block:q?.block||(state.selectedMode==="novuraExams"?"Nicht bearbeitet":""),answers:q?.a||["Nicht bearbeitet"],correctIndex:typeof q?.correct==="number"?q.correct:0,givenIndex:null,correct:false,timeout:reason==="timeout",skipped:true,explanation:q?.ex||(reason==="notGenerated"?"Diese Aufgabe wurde nicht erreicht und zählt deshalb als falsch.":"Diese Aufgabe wurde offen gelassen und zählt deshalb als falsch."),visualType:q?.type||"mc",duration:0,allowed:q?.time||0};
  }
  ...
  function showResultLegacy() {
    ...
    if(resultPacket && resultPacket.summary) {
      ...
      if(!summary){
        state.score=state.history.filter(h=>h&&h.correct).length;
        var total=state.history.filter(Boolean).length||1, percent=Math.round(state.score/total*100), dur=state.testStart?formatDuration(state.testEnd-state.testStart):"nicht erfasst", target=state.selectedMode==="novuraExams"?70:state.selectedMode==="assessments"?75:80, verdict=percent>=target?"Stark für diesen Modus.":percent>=target-10?"Knapp dran, aber noch instabil unter Druck.":"Noch zu fehleranfällig für einen sicheren Lauf.", avg=Math.round(state.history.reduce((s,h)=>s+(h?.duration||0),0)/total/100)/10;
        try{ if(window.EGTSimulation && typeof window.EGTSimulation.finish === "function") window.EGTSimulation.finish({ mode: state.selectedMode, total: state.quiz.length, answered: total, score: state.score, percent, duration: dur, avg }); }catch(e){}
  ...
  function renderTips(percent) {
    ...
    const tips=[];
    if(state.selectedMode==="novuraExams")tips.push("Prüfungsstrategie: Im Konzentrationsblock nicht vorne festbeißen. Erst sichere Aufgaben holen, dann schwere markieren und später prüfen.");
  ...
  function saveResultInternal(percent,dur,avg) {
    ...
    try { if(window.NovuraExamsAdminReportEngine && typeof NovuraExamsAdminReportEngine.enrichRecord === "function") latest = NovuraExamsAdminReportEngine.enrichRecord(latest, { state, history:state.history, quiz:state.quiz, mode:state.selectedMode, branch:"it", simType:"novuraExams", pool:"it-novura-exams" }); } catch(e) { console.warn("Novura Exams Admin Report fallback", e); }
    allData.push(latest);
    ...
    try { if(AdminPortalDomainEngine && AdminPortalDomainEngine.trackEvent) { let adminPayload = Object.assign({}, latest, {userId, module:state.selectedMode, mode:state.selectedMode, score:percent, rawScore:state.score, passed:percent>=70, correct:percent>=70, errors:Object.keys(cats).filter(k=>cats[k].r<cats[k].n), total:state.history.length, categoryStats:cats, coachSession:aiSession}); try { if(window.NovuraExamsAdminReportEngine && typeof NovuraExamsAdminReportEngine.toAdminEvent === "function") adminPayload = NovuraExamsAdminReportEngine.toAdminEvent(adminPayload, { state, history:state.history, quiz:state.quiz, mode:state.selectedMode, branch:"it", simType:"novuraExams", pool:"it-novura-exams" }); } catch(e2){} AdminPortalDomainEngine.trackEvent(adminPayload); } } catch(e) {}
  ...
  function passPrognosis(){
    ...
    const targets={novuraExams:{label:"Novura Exams · IT",target:70},assessments:{label:"Basisprofil+",target:75}};
    const out={};
    Object.entries(targets).forEach(([modeKey,cfg])=>{
      // Relevante Läufe: Simulation des Modus + gemischte Tests, jüngste zuerst
      const relevant=res.filter(r=>r.mode===modeKey||r.mode==="novuraExams"||r.mode==="jogging").slice(-10).reverse();
    ...
    return (out.novuraExams||out.assessments)?out:null;
  }
  function prognosisCardHtml(){
    ...
    return `<div class="prognosis-card"><div class="prognosis-title">🎯 Bestehensprognose</div>${bar(p.novuraExams)}${bar(p.assessments)}
  ...
  function bindRuntimeEvents(){
    ...
    document.addEventListener("click", function(ev){
      ...
      if(actionTarget){
        ...
        if(action === 'novura-exams-flowlogic-submit'){ ev.preventDefault(); submitNovuraExamsFlowLogicAnswer(); return; }
        if(action === 'novuraExams-input-submit'){ ev.preventDefault(); submitNovuraExamsInputAnswer(); return; }
      }
    });
    ...
    document.addEventListener("touchend", function(ev){
      ...
      else if(action === 'novura-exams-flowlogic-submit') submitNovuraExamsFlowLogicAnswer();
      else if(action === 'novuraExams-input-submit') submitNovuraExamsInputAnswer();
    }, {passive:false});
    ...
    document.addEventListener("keydown", function(ev){
      ...
      if(target && target.matches && target.matches('#novuraExamsAnswerInput, [data-novura-exams-coord-index]')){
        ev.preventDefault();
        submitNovuraExamsInputAnswer();
      }
    });
  }
    ...
    try{
      window.EGTSimulation.init({
        ...
        getQuestionNavTotal: function(){
          return isAdaptiveElite() ? MODES.novuraExams.amount : state.quiz.length;
        },
  ...
  function openMathHelp(q) {
    ...
    if (masteryBtn) {
      masteryBtn.onclick = function () {
        if (!q.id) return;
        const current = localStorage.getItem("assessments_math_understood_" + q.id) === "true";
        const newVal = !current;
        localStorage.setItem("assessments_math_understood_" + q.id, String(newVal));
        ...
        const totalDisplay = isAdaptiveElite() ? MODES.novuraExams.amount : state.quiz.length;
        if (state.selectedMode === "novuraExams") {
          const limit = NOVURA_EXAMS_BLOCK_LIMITS[q.block] || state.totalTimeForQuestion;
          ...
          $("meta").innerHTML = `<span class="badge ${state.selectedMode==="novuraExams"?"hardcore-badge":""}">${MODES[state.selectedMode].title}</span><span class="badge">${q.cat}</span><span class="badge">Frage ${state.current+1}/${totalDisplay}</span>${masteredBadge}${examStatusHtml()}`;
        }
      };
    }
  }
