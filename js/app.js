/* BPS-Trainer Runtime. */
/* Bootstrap, Feature Flags, PWA Config und Hilfsfunktionen. */
"use strict";
...
- Qualität: Guard, Systemdiagnose, Simulation-Checks

Systempflege:
- Cache-Bezeichnungen vereinheitlicht
- Legacy-EDV-Einzelfrage aus Export und Runtime entfernt
- EDV-Modul auf Multi-Choice-Gesamtauswertung stabilisiert
- MigrationPath auf V7.5.0 aktualisiert
...
window.App = (() => {
  const STORE_KEY = "eignungstest_trainer_v7500_results";
  ...
  const APP_VERSION = "7.5.8-full-function-audit";
  const PROFILE_KEY = "eignungstest_trainer_profile_v7500";
  ...
  const FOCUS_KEY = "eignungstest_trainer_focus_v7500";
  ...
  const PWA_CONFIG = Object.freeze({
    ...
    cacheName:"cache-kill-switch-v7500-no-runtime-cache",
    ...
    note:"Layout, Desktop/Mobile-Trennung, relative Pfade und Cache-Diagnose stabilisiert."
  });
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const CoachEngine = {
    build(results) {
      ...
      return {version:"7.5.8",readiness,memory,weaknesses,cognitive,recommendation,adaptive,dynamicMix,learning,simulation,focus:TrainingFocusEngine.current(),updatedAt:new Date().toISOString()};
    },
    renderDashboard(coach) {
      ...
      return `<div class="premium-card ai-card">
        <span class="coach-badge">Lernanalyse aktiv</span>
      ...
      <div class="premium-card ai-card ai-adaptive-card">
        ...
        <div class="ai-engine-note">Schwierigkeit und Zeitfenster passen sich datenbasiert an, sobald genügend Trainingsdaten vorhanden sind.</div>
      </div>
      <div class="premium-card ai-card ai-adaptive-card">
        ...
        <div class="ai-engine-note">Neue Aufgaben werden nach Schwächen, Fehlerarten und Grundabdeckung gewichtet.</div>
      </div>
      <div class="premium-card ai-card ai-final-card">
        <span class="coach-badge">Learning Memory</span>
      ...
      <div class="premium-card ai-card ai-final-card">
        <span class="coach-badge">Full Simulation</span>
...
/* Question Bank Schema, Quality Engine, Modusverträge, Blocklimits und Blockinfo. */
  const QUESTION_BANK_SCHEMA = {
    ...
    note:"Vorbereitet für spätere OCR- und Buchaufgaben. Die App kann aus einer zentralen Aufgabenliste filtern."
  };
  ...
  const QualityEngine = {
    ...
    summary() {
      ...
      return {system:FRAMEWORK.name, modes, storage, simulation, contracts, ai, questionBank, pwa, highscore, cloudHighscore, indexedDb, migration, ok:modes.every(m=>m.invalid===0) && simulation.count===93 && contracts.length===0 && ai.ok && questionBank.ok && pwa.ok && indexedDb.ok && migration.ok};
    }
  };
...
const MODES = {
    ...
    jogging:{title:"3. BPS-Jogging",amount:20,memory:false,instant:true,desc:"Kurzer gemischter Warm-up-Test für jeden Tag.",tags:["20 Aufgaben","gemischt","warm"]},
    ...
    ctc:{title:"5. BPS-Elite",amount:100,memory:true,instant:false,desc:"Harter Elite-Modus mit Zeitdruck, Gedächtnis, IT, Logik, Mathematik, Konzentration und angepasster Trainingssteuerung.",tags:["Elite","Zeitdruck","Leistungsprofil"]},
    ...
    errorTrainingPrep:{title:"21. Fehlertraining · Vorbereitung",amount:25,memory:false,instant:true,desc:"Gemischter Trainingsmodus als Vorbereitung für späteres echtes Schwächenlernen und Fehlertraining.",tags:["Fehlertraining","Vorbereitung"]},
    ctcLohr:{title:"22. BPS-Simulation",amount:93,memory:false,instant:false,desc:"5 Blöcke mit Startseiten, Blocktimern, fairer Wertung und EDV-Großschema mit 11 Fehlern.",tags:["Simulation","5 Blöcke","EDV 11 Fehler"]}
  };
  ...
  const Generators = {
    ...
    bigEDVMulti() {
      ...
      return makeMC(
        ...
        "edvmulti",
        {edvCorrectIds:correctIds, edvMultiSelected:[], edvRequiredCount:correctIds.length, block:"5. EDV Kenntnisse", signatureSeed:"edv-multi-v7500-cloud-highscore"}
      );
    },
    bigEDVCovered(slot) {
      return makeMC(
        ...
        "edvcovered",
        {block:"5. EDV Kenntnisse", signatureSeed:"edv-covered-v7500-"+slot}
      );
    }
  };
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  function coachRecommendation(stats){
    if(!stats.results.length) return {title:"Erster Lauf",text:"Starte mit BPS-Jogging und baue danach gezielt Blocktraining ein.",mode:"jogging"};
    if(stats.weakest){
  ...
  function renderPremiumDashboard(){
    ...
    if($("premiumGreeting")) $("premiumGreeting").textContent = profile.name ? `${timeGreeting()}, ${profile.name}.` : "Willkommen im BPS-Trainer";
    if($("premiumCopyright")) $("premiumCopyright").textContent = "Copyright: Ugurcan";
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  const APP_SECTIONS = {
    ...
    settings:{title:"Settings", icon:"⚙", desc:"Profil, Cloud, Datenbank und Diagnose verwalten."}
  };
  ...
  const TOP_NAV_TABS = {
    ...
    settings:[
      ...
      {key:"system", icon:"🛠️", label:"System", action:"settings"}
    ]
  };
  ...
  function renderAppNav(){
    ...
    // Stable rule V7.5.0: Mobile-Navigation wird direkt an den Body gehängt.
  ...
  function renderSettingsSection(){
    ...
    const cloudBlock=`<div class="premium-card section-card settings-card"><b>Cloud & Supabase</b><p class="small">Live-Diagnose, Online-Highscore und Cache-Neuladen.</p><button class="ghost" onclick="App.showFrameworkHealth()">Diagnose öffnen</button></div>`;
    ...
    const systemBlock=`<div class="premium-card section-card settings-card"><b>Systemdiagnose</b><p class="small">Technische Prüfung für Aufgabenbank, Modi, Speicher, Simulation und Cloud.</p><button class="ghost" onclick="App.showFrameworkHealth()">Diagnose prüfen</button></div>`;
    const blocks={profile:profileBlock,cloud:cloudBlock,audio:audioBlock,data:dataBlock,system:systemBlock};
    const secondary=`${tab!=="profile"?profileBlock:""}${tab!=="cloud"?cloudBlock:""}${tab!=="audio"?audioBlock:""}${tab!=="data"?dataBlock:""}${tab!=="system"?systemBlock:""}`;
    return `<div class="section-stack settings-stack"><div class="section-heading"><h2>Settings</h2><p>Profil, Cloud, Audio, Daten und Diagnose sind hier gebündelt.</p></div><div class="section-primary-block settings-primary-block">${blocks[tab] || profileBlock}</div><div class="section-grid settings-secondary-grid">${secondary}</div></div>`;
  }
  ...
  function renderDashboardSection(){
    ...
    const cloudCard=`<div class="premium-card section-card"><b>Cloud</b><p class="small">Supabase Highscore und Diagnose sind hier angebunden.</p><button class="ghost" onclick="App.showFrameworkHealth()">Diagnose prüfen</button></div>`;
  ...
  function renderModes() {
    ...
    // V7.5.0: mobile-nav-attached wird ausschließlich in renderAppNav/stabilizeMobileBottomNav gesetzt.
    renderPremiumDashboard();
    renderAppNav();
    renderMobileTopNav();
  ...
  function renderTips(percent) {
    ...
    if(!isAdaptiveElite())tips.push("Für gezieltes Nachtrainieren nutze den BPS-Elite-Modus."); else tips.push("Elite-Runde beendet: Wiederhole den Modus direkt, damit deine Schwächen erneut unter Druck trainiert werden.");
  ...
  function showDatabaseInfo() {
    ...
    panel.innerHTML = `
      ...
      <div class="db-status-card"><b>Aufgabenbank-Import:</b><br><code>source, sourcePage, category, group, subtype, difficulty, question, answers, correct, explanation, tags, verified</code><br><span class="small">Status: vorbereitet.</span></div>
      <div class="db-status-card"><b>Systemmodule:</b><br>Aktiv: ${FEATURE_STATUS.stable.length}<br>Geparkt: ${FEATURE_STATUS.disabled.length}</div>
  ...
  async function showFrameworkHealth() {
    setDiagnosticsContent('<b>Systemdiagnose</b><br><span class="small">Diagnose wird geladen...</span>');
    let summary = null;
    let cloudLive = null;
    let guardHtml = "";
    try { summary = QualityEngine.summary(); } catch(error) { summary = {system:FRAMEWORK.name, modes:[], simulation:{count:0}, contracts:["QualityEngine Fehler: " + String(error && error.message ? error.message : error)], storage:{engine:"unbekannt",records:0}, ok:false}; }
    ...
    const html = `
      <b>Systemdiagnose</b>
      <div class="health-row"><span>App</span><strong>BPS-Trainer</strong></div><div class="health-row"><span>System</span><strong>${escHTML(summary.system || FRAMEWORK.name)}</strong></div>
      ...
      <div class="health-row"><span>Question Bank</span><strong>${summary.questionBank && summary.questionBank.ok ? "Schema OK" : "Prüfen"}</strong></div>
      <div class="health-row"><span>PWA Status</span><strong>${summary.pwa && summary.pwa.ok ? "Manifest aktiv" : "Prüfen"}</strong></div>
      <div class="health-row"><span>Aktive Module</span><strong>${FEATURE_STATUS.stable.length}</strong></div>
  ...
  bindRuntimeEvents();
  window.addEventListener("resize",()=>{ try{ renderModes(); }catch(e){} }, {passive:true});
  window.addEventListener("orientationchange",()=>{ setTimeout(()=>{ try{ renderModes(); }catch(e){} },120); }, {passive:true});
