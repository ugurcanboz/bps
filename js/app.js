/* STABLE SINGLE-FILE BUNDLE V7.2.3 · js/src wurde bewusst entfernt, damit GitHub Pages nur diese Runtime lädt. */
/* Bootstrap, Version, Feature Flags, PWA Config, Hilfsfunktionen und Framework-Metadaten. */
"use strict";

/*
===============================================================================
Eignungstest-Trainer V7.2.3 · Clean Stable
===============================================================================
Struktur:
...
- MigrationPath auf V7.2.3 aktualisiert
...
window.App = (() => {
  const STORE_KEY = "eignungstest_trainer_v7230_results";
  ...
  const APP_VERSION = "7.2.3-clean-rebuild";
  const PROFILE_KEY = "eignungstest_trainer_profile_v7230";
  ...
  const FOCUS_KEY = "eignungstest_trainer_focus_v7230";
  ...
  const PWA_CONFIG = Object.freeze({
    ...
    cacheName:"cache-kill-switch-v7230-no-runtime-cache",
    ...
    note:"V7.2.3 Global Layout Spacing Stable: GitHub-Pages-Fallback, Desktop/Mobile getrennt, relative Pfade, Update-Prompt und temporäre Cache-Diagnose stabilisiert."
  });
...
/* Guard, Analytics, SessionTracker, ErrorMemory, Coach/Adaptive/Dynamic/Learning/Highscore Engines. */
  ...
  const CoachEngine = {
    build(results) {
      ...
      return {version:"7.2.3",readiness,memory,weaknesses,cognitive,recommendation,adaptive,dynamicMix,learning,simulation,focus:TrainingFocusEngine.current(),updatedAt:new Date().toISOString()};
    },
    renderDashboard(coach) {
      ...
      return `<div class="premium-card ai-card">
        <span class="coach-badge">AI Stable Core V7.2.3</span>
  ...
  const QuestionBankEngine = {
    ...
    importBatch(rawItems=[], source="ocr-import") {
      if(!FEATURE_FLAGS.questionBankRuntimeImport) {
        ...
        QUESTION_BANK.lastImport={source, imported:0, invalid:validation.filter(v=>!v.ok).length, reviewed:validation.length, blocked:true, reason:"Question Bank Runtime Import ist in V7.2.3 bewusst deaktiviert.", createdAt:new Date().toISOString()};
  ...
  const Generators = {
    ...
    bigEDVMulti() {
      ...
      return makeMC(
        ...
        "edvmulti",
        {edvCorrectIds:correctIds, edvMultiSelected:[], edvRequiredCount:correctIds.length, block:"5. EDV Kenntnisse", signatureSeed:"edv-multi-v7230-cloud-highscore"}
      );
    },
    bigEDVCovered(slot) {
      return makeMC(
        ...
        "edvcovered",
        {block:"5. EDV Kenntnisse", signatureSeed:"edv-covered-v7230-"+slot}
      );
    }
  };
...
/* MultiChoiceModule, Memory, Route-Memory, Profilverwaltung und interaktive Spezialmodule. */
  ...
  function renderProfileManager(){
    ...
    return `<div class="premium-card profile-manager-card">
      <span class="coach-badge">Profilverwaltung V7.2.3</span>
...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  function renderAppNav(){
    ...
    // Stable fix V7.2.3: Hauptnavigation bleibt immer im App-Container.
  ...
  function renderSettingsSection(){
    ...
    const frameworkBlock=`<div class="premium-card section-card settings-card"><b>Framework</b><p class="small">Core Cleanup V7.2.3 · Module bleiben getrennt: EDV, Route, Simulation, Storage und Cloud.</p><button class="ghost" onclick="App.showFrameworkHealth()">Framework prüfen</button></div>`;
  ...
  function showDatabaseInfo() {
    ...
    panel.innerHTML = `
      ...
      <div class="db-status-card"><b>Aufgabenbank-Import:</b><br><code>source, sourcePage, category, group, subtype, difficulty, question, answers, correct, explanation, tags, verified</code><br><span class="small">Status V7.2.3: vorbereitet, runtime-deaktiviert.</span></div>
