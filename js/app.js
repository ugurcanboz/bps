...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  const QNAV_DESKTOP_MIN = 1040;
  const QNAV_DRAWER_WIDTH = "min(400px, 38vw)";
  function qnavDrawerIsOpen(){ try{ return localStorage.getItem(QNAV_DRAWER_KEY) === "open"; }catch(e){ return false; } }
  function qnavIsDesktop(){ try{ return (window.innerWidth||0) >= QNAV_DESKTOP_MIN; }catch(e){ return false; } }
  const QNAV_NAV_PROPS=["position","top","right","bottom","left","width","margin","padding","background","border","border-right","border-radius","box-shadow","overflow-y","z-index","transition","transform","visibility","backdrop-filter"];
  const QNAV_BTN_PROPS=["display","position","right","top","transform","flex-direction","align-items","gap","padding","min-width","background","border","border-right","border-radius","color","cursor","z-index","box-shadow","transition","font-family"];
  /* Schubladen-Layout vollständig per Inline-!important:
     kaskaden-immun gegen jede später geladene CSS-Datei. */
  function applyQnavLayout(){
    const nav=$("questionNav"), btn=$("qnavDrawerToggle");
    if(!nav) return;
    const open=qnavDrawerIsOpen();
    const setN=(k,v)=>{ try{ nav.style.setProperty(k,v,"important"); }catch(e){} };
    const setB=(k,v)=>{ try{ if(btn) btn.style.setProperty(k,v,"important"); }catch(e){} };
    if(qnavIsDesktop()){
      setN("position","fixed"); setN("top","84px"); setN("right","0"); setN("bottom","84px"); setN("left","auto");
      setN("width",QNAV_DRAWER_WIDTH); setN("margin","0"); setN("padding","54px 16px 16px");
      setN("background","rgba(6,12,28,.97)"); setN("border","1px solid rgba(96,165,250,.30)"); setN("border-right","none");
      setN("border-radius","22px 0 0 22px"); setN("box-shadow","-24px 0 70px rgba(0,0,0,.55)");
      setN("overflow-y","auto"); setN("z-index","940"); setN("backdrop-filter","blur(14px)");
      setN("transition","transform .38s cubic-bezier(.32,.9,.3,1), visibility 0s linear "+(open?"0s":".38s"));
      setN("transform", open?"translateX(0)":"translateX(calc(100% + 12px))");
      setN("visibility", open?"visible":"hidden");
      if(btn){
        setB("display","flex"); setB("position","fixed"); setB("right","0"); setB("top","50%");
        setB("transform","translateY(-50%)"+(open?" translateX(calc(-1 * "+QNAV_DRAWER_WIDTH+"))":""));
        setB("flex-direction","column"); setB("align-items","center"); setB("gap","6px");
        setB("padding","14px 10px 12px"); setB("min-width","58px");
        setB("background","linear-gradient(160deg, rgba(37,99,235,.94), rgba(124,58,237,.90))");
        setB("border","1px solid rgba(168,85,247,.6)"); setB("border-right","none");
        setB("border-radius","18px 0 0 18px"); setB("color","#fff"); setB("cursor","pointer");
        setB("z-index","941"); setB("box-shadow","-10px 6px 34px rgba(2,6,23,.55)");
        setB("transition","transform .38s cubic-bezier(.32,.9,.3,1)");
        setB("font-family","inherit");
      }
    } else {
      QNAV_NAV_PROPS.forEach(k=>{ try{ nav.style.removeProperty(k); }catch(e){} });
      if(btn){ QNAV_BTN_PROPS.forEach(k=>{ try{ btn.style.removeProperty(k); }catch(e){} }); try{ btn.style.setProperty("display","none","important"); }catch(e){} }
    }
    nav.classList.toggle("qnav-open", open && qnavIsDesktop());
    if(btn){ btn.classList.toggle("open", open); btn.setAttribute("aria-expanded", open?"true":"false");
      const arrow=btn.querySelector?btn.querySelector(".qnav-toggle-arrow"):null;
      if(arrow&&arrow.style&&arrow.style.setProperty){ arrow.style.setProperty("transform","rotate("+(open?"180":"0")+"deg)","important"); arrow.style.setProperty("transition","transform .38s ease","important"); arrow.style.setProperty("display","inline-block","important"); }
    }
  }
  function toggleQnavDrawer(force){
    const nav=$("questionNav");
    if(!nav) return;
    const open = typeof force === "boolean" ? force : !qnavDrawerIsOpen();
  ...
  function ensureQnavToggle(){
    ...
    if(!btn){
      ...
      try{ let qrt=null; window.addEventListener("resize",()=>{ clearTimeout(qrt); qrt=setTimeout(applyQnavLayout,150); }); }catch(e){}
    }
    return btn;
  }
