...
/* UI-Routing, Rendering, Quizablauf, Ergebnislogik, Review, Health Panel und App-Export. */
  ...
  function setQuestionDrawer(open){
    try {
      ...
      if(btn){
        ...
        btn.innerHTML = open ? "× Fragen" : "☷ Fragen";
      }
    } catch(e) {}
  }
  ...
  function highscoreData(){
    ...
    const recent=res.slice(-8).reverse()
      .map(r=>({name:r.player_name||"Gast",mode:r.title||r.mode||"",percent:Math.round(r.percent||0),date:r.date||""}));
    return { top, recent, duels:duellHistory(), prognosisHtml:prognosisCardHtml(), avatarHtml:duellAvatarHtml };
  }
  ...
  function bindRuntimeEvents(){
    ...
    document.addEventListener("click", function(ev){
      ...
      if(actionTarget){
        ...
        if(action === 'cloud-refresh'){ ev.preventDefault(); ev.stopPropagation(); CloudHighscoreEngine.refreshDashboard('manual-button'); return; }
        if(action === 'cloud-health-check'){ ev.preventDefault(); ev.stopPropagation(); CloudHighscoreEngine.refreshDashboard('cloud-health-check'); return; }
        if(action === 'hs-period'){ ev.preventDefault(); ev.stopPropagation(); if(CloudHighscoreEngine.setPeriod) CloudHighscoreEngine.setPeriod(actionTarget.getAttribute('data-period')||'all'); return; }
        if(action === 'hs-show-more'){ ev.preventDefault(); ev.stopPropagation(); if(CloudHighscoreEngine.showMore) CloudHighscoreEngine.showMore(); return; }
        if(action === 'hs-profile-info'){ ev.preventDefault(); ev.stopPropagation(); if(window.EGTAuthProfileShell && EGTAuthProfileShell.openProfile) EGTAuthProfileShell.openProfile(); else showFrameworkHealth(); return; }
        if(action === 'hs-challenges-info'){ ev.preventDefault(); ev.stopPropagation(); if(window.EGTUILayer && EGTUILayer.notice) EGTUILayer.notice('Highscore-Herausforderungen sind aus Rang, Punkten und Streak berechnet.'); return; }
        if(action === 'hs-class-info'){ ev.preventDefault(); ev.stopPropagation(); showFrameworkHealth(); return; }
        if(action === 'hs-player-preview'){ ev.preventDefault(); ev.stopPropagation(); if(window.EGTUILayer && EGTUILayer.notice) EGTUILayer.notice('Teilnehmerdetails werden im Dozenten-/Adminbereich geführt.'); return; }
    ...
    document.addEventListener("touchend", function(ev){
      ...
      if(action === 'cloud-refresh') CloudHighscoreEngine.refreshDashboard('manual-touch');
      else if(action === 'cloud-health-check') CloudHighscoreEngine.refreshDashboard('cloud-health-check');
      else if(action === 'hs-period' && CloudHighscoreEngine.setPeriod) CloudHighscoreEngine.setPeriod(target.getAttribute('data-period')||'all');
      else if(action === 'hs-show-more' && CloudHighscoreEngine.showMore) CloudHighscoreEngine.showMore();
      else if(action === 'hs-profile-info' && window.EGTAuthProfileShell && EGTAuthProfileShell.openProfile) EGTAuthProfileShell.openProfile();
