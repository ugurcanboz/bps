...
(function () {
  ...
  var VERSION = 'ui-g37-2-customer-nav-stabilized';
  ...
  function switchTab (tab) {
    ...
    if (normalized === TAB_SIM) {
      try {
        var sim = moduleById('simulation');
        if (sim) { openSheet(sim); return; }
      } catch (e) {}
    }
  ...
  function renderTabBar () {
    ...
    tabs.className = 'egt-bottom-dock egt-bottom-dock-g37';
    tabs.setAttribute('aria-label', 'Hauptnavigation');
    tabs.setAttribute('role', 'tablist');

    var navItems = [
      { tab: TAB_HOME,      label: 'Home',       icon: 'dashboard', action: 'tab' },
      { tab: TAB_HIGHSCORE, label: 'Highscore',  icon: 'trophy',    action: 'tab' },
      { tab: TAB_SIM,       label: 'Simulation', icon: 'target',    action: 'simulation', center: true },
      { tab: TAB_DUELS,     label: 'Duelle',     icon: 'swords',    action: 'tab' },
      { tab: TAB_PROFILE,   label: 'Profil',     icon: 'user',      action: 'tab' }
    ];

    tabs.innerHTML = navItems.map(function (item) {
      var selected = item.action === 'tab' && item.tab === activeTab;
      var attrs = '';
      if (item.action === 'tab') attrs = 'data-ui-nav="tab" data-tab="' + item.tab + '"';
      if (item.action === 'simulation') attrs = 'data-ui-nav="tab" data-tab="simulation" data-ui-action="open-module" data-module="simulation"';
      return (
        '<button type="button" class="egt-dock-btn' + (selected ? ' is-active' : '') + (item.center ? ' is-center-action' : '') + '" ' + attrs +
                ' aria-label="' + esc(item.label) + '" role="tab" aria-selected="' + (selected ? 'true' : 'false') + '">' +
    ...
    var html =
      '<div class="ui-analysis-wrap ui-duel-page">' +
        ...
        '<div class="ui-action-card-grid ui-g37-duel-actions">' +
          ...
          '<button type="button" class="ui-action-card" data-ui-action="open-module" data-module="simulation"><b>Simulation trainieren</b><small>Prüfungsdaten für spätere Duelle sammeln</small></button>' +
        '</div>' +
  ...
  function openDuelSheet () {
    ...
    var body =
      ...
      '<div class="ui-action-card-grid ui-g37-duel-actions">' +
        ...
        '<button type="button" class="ui-action-card" data-ui-action="open-module" data-module="simulation"><b>Simulation öffnen</b><small>Prüfungsmodus als Grundlage</small></button>' +
      '</div>' +
      '<div class="ui-deep-section-label">Letzte Duelle</div>';
    if (duels.length) {
