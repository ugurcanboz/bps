...
(function () {
      ...
      '<div class="ui-home">' +
        ...
        '<main class="ui-home-scroll" id="uiHomeViewport">' +
          ...
          '<section class="ui-sim-hero" aria-label="Eignungstest Simulation">' +
            '<button type="button" class="ui-sim-hero-card" data-ui-action="open-module" data-module="simulation">' +
              ...
              '<span class="ui-sim-hero-copy">' +
                ...
                '<b>Eignungstest-Simulation</b>' +
                '<small>Komplette Prüfung unter Echtbedingungen: 5 Blöcke, Block-Timer, Fragenübersicht, Auswertung und Coach-Analyse.</small>' +
              '</span>' +
              '<span class="ui-sim-hero-cta">Simulation starten ›</span>' +
            '</button>' +
          '</section>' +
          ...
          '<section class="ui-quick-grid" aria-label="Schnellzugriff">' +
            ...
            '<button type="button" class="ui-quick-card ui-area-individual" data-ui-action="duel-mode"><span class="ui-quick-icon">⚔️</span><b>Duell</b><small>2 Spieler · 1 Gerät</small></button>' +
        ...
        '<button type="button" class="egt-dock-btn" data-ui-action="highscore-sheet" role="tab"><span class="egt-dock-icon">\ud83c\udfc6</span><span class="egt-dock-label">Highscore</span></button>' +
  ...
  /* ── Public API ───────────────────────────────────── */
  /* ============================================================
     HIGHSCORE-ZENTRALE · eigenständige Seite (Deep-Sheet):
     Online-Kursrangliste (Firestore), Bestehensprognose,
     letzte Duelle, letzte Ergebnisse, persönliche Bestleistungen.
     ============================================================ */
  function hsRankBadge (i) {
    if (i === 0) return '<span class="hs-rank gold">🥇</span>';
    if (i === 1) return '<span class="hs-rank silver">🥈</span>';
    if (i === 2) return '<span class="hs-rank bronze">🥉</span>';
    return '<span class="hs-rank">' + (i + 1) + '</span>';
  }
  function hsCloudRowsHtml (rows, meCode, av) {
    if (!rows || !rows.length) return '<div class="ui-deep-empty">Noch keine Einträge im Kurs. Der erste gewertete Lauf erscheint hier automatisch.</div>';
    return '<div class="hs-cloud-table">' +
      '<div class="hs-cloud-head"><span>Rang</span><span>Teilnehmer</span><span class="num">Beste Quote</span><span class="num">Läufe</span></div>' +
      rows.map(function (r, i) {
        var me = meCode && String(r.code || '').toUpperCase() === String(meCode).toUpperCase();
        var when = r.updatedAt ? new Date(r.updatedAt).toLocaleDateString('de-DE') : '';
        return '<div class="hs-cloud-row' + (me ? ' me' : '') + '">' +
          hsRankBadge(i) +
          '<span class="hs-cloud-who">' + av(r.name || r.code, 26) + '<span><b>' + esc(r.name || r.code) + (me ? ' <i>(du)</i>' : '') + '</b>' +
          '<small>' + esc(String(r.lastMode || '').replace(/^\d+\.\s*/, '')) + (when ? ' · ' + esc(when) : '') + '</small></span></span>' +
          '<span class="hs-cloud-best num">' + Math.round(r.bestPercent || 0) + '<i>%</i></span>' +
          '<span class="hs-cloud-runs num">' + (r.runs || 0) + '</span></div>';
      }).join('') + '</div>';
  }
  function loadCloudHighscoreInto (containerId) {
    var el = document.getElementById(containerId);
    if (!el) return;
    el.innerHTML = '<div class="hs-cloud-status">⏳ Lade Online-Bestenliste…</div>';
    var data = null;
    try { data = window.App && App.highscoreData ? App.highscoreData() : null; } catch (e) {}
    var meCode = data && data.session ? data.session.code : null;
    var av = (data && data.avatarHtml) ? data.avatarHtml : function () { return ''; };
    if (!window.App || typeof App.cloudHighscoreTop !== 'function') {
      el.innerHTML = '<div class="hs-cloud-status warn">Online-Bestenliste nicht verfügbar.</div>';
      return;
    }
    App.cloudHighscoreTop(20).then(function (rows) {
      el.innerHTML = hsCloudRowsHtml(rows, meCode, av);
    }).catch(function (err) {
      el.innerHTML = '<div class="hs-cloud-status warn">Online-Bestenliste momentan nicht erreichbar (offline oder fehlende Berechtigung). Lokale Daten unten bleiben verfügbar.</div>' +
        '<button type="button" class="hs-cloud-retry" onclick="EGTUILayer.refreshHighscoreCloud()">↻ Erneut versuchen</button>';
    });
  }
  function refreshHighscoreCloud () { loadCloudHighscoreInto('hsCloudBody'); }
  function openHighscoreSheet () {
    var data = null;
    try { data = window.App && typeof App.highscoreData === 'function' ? App.highscoreData() : null; } catch (e) {}
    var av = (data && data.avatarHtml) ? data.avatarHtml : function (n) { return ''; };
    var body = '';
    // 1) Online-Kursrangliste
    body += '<div class="hs-section-head"><div class="ui-deep-section-label">🌐 Online-Bestenliste (Kurs)</div>' +
      '<button type="button" class="hs-refresh-btn" onclick="EGTUILayer.refreshHighscoreCloud()" title="Aktualisieren">↻</button></div>' +
      '<div id="hsCloudBody"><div class="hs-cloud-status">⏳ Lade Online-Bestenliste…</div></div>';
    // 2) Bestehensprognose
    if (data && data.prognosisHtml) body += data.prognosisHtml;
    // 3) Letzte Duelle
    var duels = (data && data.duels) || [];
    body += '<div class="ui-deep-section-label">⚔️ Letzte Duelle</div>';
    if (duels.length) {
      body += '<div class="hs-duel-list">' + duels.slice(0, 6).map(function (d) {
        ...
        return '<div class="hs-duel-card">' + banner +
          '<div class="hs-duel-score">' + av(d.p1.name, 24) + ' ' + esc(d.p1.name) + ' <b>' + d.p1.correct + ' : ' + d.p2.correct + '</b> ' + esc(d.p2.name) + ' ' + av(d.p2.name, 24) + '</div>' +
          '<div class="hs-duel-meta">' + (d.mode === 'online' ? '📡 Online' : '📱 Lokal') + ' · ' + (d.total || 12) + ' Aufgaben · ' + esc(when) + '</div></div>';
      }).join('') + '</div>';
    } else {
      body += '<div class="ui-deep-empty">Noch keine Duelle gespielt. Fordere jemanden heraus!</div>';
    }
    body += '<button type="button" class="ui-deep-primary" data-ui-action="duel-mode" style="width:100%;margin:10px 0 16px">⚔️ Duell starten</button>';
    // 4) Letzte Ergebnisse (dieses Gerät)
    var recent = (data && data.recent) || [];
    body += '<div class="ui-deep-section-label">📊 Letzte Ergebnisse</div>';
    if (recent.length) {
      body += '<div class="hs-list">' + recent.map(function (r) {
        var when = r.date ? new Date(r.date).toLocaleDateString('de-DE') : '';
        return '<div class="hs-row"><span class="hs-main">' + esc(String(r.mode).replace(/^\d+\.\s*/, '')) + ' <small>' + esc(when) + '</small></span><span class="hs-score">' + r.percent + '%</span></div>';
      }).join('') + '</div>';
    } else {
      body += '<div class="ui-deep-empty">Noch keine Ergebnisse. Starte ein Training oder eine Simulation.</div>';
    }
    // 5) Persönliche Bestleistungen
    var top = (data && data.top) || [];
    body += '<div class="ui-deep-section-label">🏆 Persönliche Bestleistungen (dieses Gerät)</div>';
    if (top.length) {
      body += '<div class="hs-list">' + top.map(function (r, i) {
        return '<div class="hs-row">' + hsRankBadge(i) + '<span class="hs-main">' + esc(r.name) + ' <small>' + esc(String(r.mode).replace(/^\d+\.\s*/, '')) + '</small></span><span class="hs-score">' + r.percent + '%</span></div>';
      }).join('') + '</div>';
    } else {
      body += '<div class="ui-deep-empty">Noch keine Ergebnisse vorhanden.</div>';
    }
    var res = openDeepSheet({
      type: 'highscore', theme: 'orange', title: 'Highscore-Zentrale', kicker: 'Kurs-Rangliste & Historie',
      subtitle: 'Online-Bestenliste, Prognose, Duelle und deine letzten Ergebnisse.', iconHtml: '🏆',
      bodyHtml: body
    });
    try { setTimeout(refreshHighscoreCloud, 40); } catch (e) {}
    return res;
  }
  ...
  window.EGTUILayer = {
    ...
    refreshHighscoreCloud: refreshHighscoreCloud,
    openSheet: openSheet,
