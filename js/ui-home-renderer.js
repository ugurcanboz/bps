...
(function () {
  ...
  function openHighscoreSheet () {
    ...
    // Letzte Duelle mit Sieger-Banner + Avatar
    var duels = (data && data.duels) || [];
    body += '<div class="ui-deep-section-label">⚔️ Letzte Duelle</div>';
    if (duels.length) {
      body += '<div class="hs-duel-list">' + duels.slice(0, 8).map(function (d) {
    ...
    body += '<div class="ui-deep-section-label">🏆 Bestenliste (dieses Gerät)</div>';
    if (top.length) {
      body += '<div class="hs-list">' + top.map(function (r, i) {
        return '<div class="hs-row"><span class="hs-pos">' + (i + 1) + '</span><span class="hs-main">' + esc(r.name) + ' <small>' + esc(String(r.mode).replace(/^\d+\.\s*/, '')) + '</small></span><span class="hs-score">' + r.percent + '%</span></div>';
      }).join('') + '</div>';
    } else {
      body += '<div class="ui-deep-empty">Noch keine Ergebnisse. Starte ein Training oder eine Simulation.</div>';
    }
    return openDeepSheet({
      type: 'highscore', theme: 'orange', title: 'Highscore & Duelle', kicker: 'Rangliste',
      subtitle: 'Bestenliste, Duell-Historie und deine Bestehensprognose.', iconHtml: '🏆',
      bodyHtml: body
    });
  }
