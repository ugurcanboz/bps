/* Novura · UI Home Renderer
   Rendert Startseite, Schnellzugriff und Modul-Sheets. Styling liegt zentral in ui-foundation.css. */
(function () {
  'use strict';

  var BRAND = window.NovuraBrand || { name:'Novura', slogan:"One day, you'll thank today.", modules:{} };
  var BRAND_COPY = window.NovuraBrandCopy || { modules:{} };
  function brandModule(key, fallback) {
    var item = BRAND.modules && BRAND.modules[key];
    return item && item.name ? item.name : fallback;
  }
  function brandDescription(key, fallback) {
    var item = BRAND_COPY.modules && BRAND_COPY.modules[key];
    return item && item.description ? item.description : fallback;
  }
  ...
  var MODULES = [
    {
      ...
      modes: ['novuraExamsBosch', 'simulation', 'eignungstest', 'assessments', 'novuraExams'], startLabel: 'Simulation starten',
  ...
  function openDeepSheet (config) {
    ...
    var navController = window.NovuraDeepSheetNavigation || null;
    var navState = navController && typeof navController.begin === 'function'
      ? navController.begin(config, { sheet: sheet, renderer: openDeepSheet })
      : null;
    var hasLegacyBack = typeof config.onBack === 'function';
    var hasBack = navState ? navState.canBack : hasLegacyBack;
    var backLabel = navState && navState.backLabel ? navState.backLabel : (config.backLabel || 'Zurück');
    var path = navState && Array.isArray(navState.path) && navState.path.length
      ? navState.path
      : [{ index: 0, title: title, current: true }];
    var pathHtml = path.map(function (item, index) {
      return '<li><button type="button" class="ui-deep-path-item" data-ui-sheet-depth="' + index + '"' +
        (item.current ? ' aria-current="page"' : '') + '>' +
        '<span class="ui-deep-path-index" aria-hidden="true">' + (index + 1) + '</span>' +
        '<span class="ui-deep-path-label">' + esc(item.title) + '</span>' +
        '<span class="ui-deep-path-state" aria-hidden="true">' + (item.current ? '•' : '↖') + '</span>' +
      '</button></li>';
    }).join('');

    sheet.setAttribute('data-ui-deep-sheet', config.type || 'generic');
    sheet.setAttribute('aria-labelledby', 'uiSheetContextHeading');
    ...
    sheet.innerHTML =
      ...
      '<header class="ui-deep-head">' +
        '<div class="ui-deep-navbar">' +
          '<div class="ui-deep-nav-left">' +
            (hasBack
              ? '<button type="button" class="ui-deep-back" id="uiSheetBack" data-deep-nav-id="back" aria-label="Zurück zu ' + esc(backLabel) + '"><span class="ui-deep-back-icon" aria-hidden="true">←</span><span class="ui-deep-back-label">' + esc(backLabel) + '</span></button>'
              : '<span class="ui-deep-nav-spacer" aria-hidden="true"></span>') +
          '</div>' +
          '<button type="button" class="ui-deep-current" id="uiSheetContextTitle" data-deep-nav-id="context" aria-haspopup="menu" aria-expanded="false" aria-controls="uiSheetContextMenu">' +
            '<span class="ui-deep-title" id="uiSheetContextHeading" role="heading" aria-level="2">' + esc(title) + '</span>' +
            '<span class="ui-deep-current-chevron" aria-hidden="true">⌄</span>' +
          '</button>' +
          '<button type="button" class="ui-sheet-close ui-deep-close" id="uiSheetClose" aria-label="Deep Sheet schließen">×</button>' +
        '</div>' +
        '<nav class="ui-deep-context-menu" id="uiSheetContextMenu" aria-label="Navigationspfad" hidden>' +
          '<ol class="ui-deep-path-list">' + pathHtml + '</ol>' +
        '</nav>' +
        '<div class="ui-deep-summary">' +
    ...
    var backBtn = $('uiSheetBack');
    if (backBtn && hasBack) backBtn.onclick = function () {
      if (navController && typeof navController.back === 'function' && navController.back()) return;
      if (hasLegacyBack) {
        try { config.onBack(); } catch (e) { try { console.error('[Novura UI] Zurück-Navigation fehlgeschlagen:', e); } catch (x) {} }
      }
    };
    var contextBtn = $('uiSheetContextTitle');
    var contextMenu = $('uiSheetContextMenu');
    if (contextBtn) contextBtn.onclick = function () {
      if (navController && typeof navController.toggleMenu === 'function') navController.toggleMenu();
      else if (contextMenu) {
        var open = contextMenu.hidden;
        contextMenu.hidden = !open;
        contextBtn.setAttribute('aria-expanded', open ? 'true' : 'false');
      }
    };
    if (contextMenu) contextMenu.addEventListener('click', function (event) {
      var item = event.target && event.target.closest ? event.target.closest('[data-ui-sheet-depth]') : null;
      if (!item) return;
      var depth = Number(item.getAttribute('data-ui-sheet-depth'));
      if (navController && typeof navController.goTo === 'function') navController.goTo(depth);
    });
    ...
    try {
      if (typeof config.onNavActivate === 'function') config.onNavActivate();
      ...
      document.dispatchEvent(new CustomEvent('egt:deep-sheet-opened', { detail:{ type: config.type || 'generic', title: title, depth: path.length - 1 } }));
      if (window.EGTUIRouter && typeof window.EGTUIRouter.sync === 'function') window.EGTUIRouter.sync(0);
      setTimeout(function(){ try{ if(window.EGTUIRouter && typeof window.EGTUIRouter.sync === 'function') window.EGTUIRouter.sync(0); }catch(e2){} }, 30);
    } catch(e) {}
    return true;
  }
  ...
  function openBranchMenu(branchId, preferredModule) {
    ...
    if (branchId && branchId !== 'wissen') {
      try { localStorage.setItem('assessments_branch', branchId); updateTopBar(); } catch(e) {}
    }
    var list = branchModules(branchId || '', false);
    var optional = branchOptionalModules(branchId || '');
  ...
  function currentBranchId () {
    try {
      ...
      return localStorage.getItem('assessments_branch') || 'it';
    } catch (e) { return 'it'; }
  }
  ...
  function simulationProfileCard(branchId, simType, title, desc, icon) {
    var mode = simType === 'novuraExams' ? 'novuraExams' : 'assessments';
    var time = simType === 'novuraExams' ? 'ca. 33–35 Min. · 6 Prüfungsblöcke' : '50 Sekunden pro Aufgabe';
    var badge = simType === 'novuraExams' ? 'Exams' : 'Assessments';
    ...
    return '' +
      '<div class="product-flow product-flow-simulation">' +
        '<div class="ui-deep-benefits product-rules">' +
          '<div class="ui-deep-benefit" data-novura-module="assessments"><b>' + esc(brandModule('assessments', 'Novura Assessments')) + '</b><span>50 Sek./Aufgabe</span></div>' +
          '<div class="ui-deep-benefit" data-novura-module="exams"><b>' + esc(brandModule('exams', 'Novura Exams')) + '</b><span>ca. 33–35 Min. · echter Blockablauf</span></div>' +
          '<div class="ui-deep-benefit"><b>Zwei Wege</b><span>Assessments · Exams · Berufsfeld</span></div>' +
        '</div>' +
        '<div class="product-warning"><b>Eignungstest-Simulation:</b> Auswahltests, Novura Exams und berufliche Assessments laufen ohne Hilfe, ohne Sofortfeedback und ohne Coach während der Aufgabe. Sprachtests sind separat auf der Home-Seite erreichbar.</div>' +
        '<div class="ui-deep-section-label">1. Berufsfeld wählen → 2. Eignungstest starten</div>' +
        '<div class="product-track-grid">' +
          '<section class="product-track-card product-track-it">' +
            '<div class="product-track-head"><span>' + iconSvg('it') + '</span><div><b>IT / FISI</b><small>Auswahltest oder Novura Exams</small></div></div>' +
            '<div class="product-sim-options">' +
              simulationProfileCard('it', 'assessments', brandModule('assessments', 'Novura Assessments'), 'IT-passender Aufgabenmix', 'target') +
              simulationProfileCard('it', 'novuraExams', 'Novura Exams · IT Assessment', 'Allgemeinwissen, Mathe, Regelrechnung, Konzentration, Tabellen und Wenn-Dann-Ablauf', 'practice') +
            '</div>' +
          '</section>' +
          '<section class="product-track-card product-track-social">' +
            '<div class="product-track-head"><span>' + iconSvg('social') + '</span><div><b>Sozialpädagogik</b><small>Assessments ohne IT-Fragen</small></div></div>' +
            '<div class="product-sim-options">' +
              simulationProfileCard('sozial', 'assessments', brandModule('assessments', 'Novura Assessments'), 'pädagogiknahe Mischung ohne IT-Fragen', 'target') +
            '</div>' +
          '</section>' +
          '<section class="product-track-card product-track-kaufm">' +
            '<div class="product-track-head"><span>' + iconSvg('kaufm') + '</span><div><b>Kaufmännisch</b><small>Assessments mit kaufmännischem Fokus</small></div></div>' +
            '<div class="product-sim-options">' +
              simulationProfileCard('kaufm', 'assessments', brandModule('assessments', 'Novura Assessments'), 'kaufmännisch passender Aufgabenmix', 'target') +
            '</div>' +
          '</section>' +
        '</div>' +
      '</div>';
  ...
  function openSimulationCenterDeepSheet() {
    return openDeepSheet({
      ...
      theme: 'indigo',
      title: brandModule('assessments', 'Novura Assessments') + ' & ' + brandModule('exams', 'Novura Exams'),
      kicker: 'Prüfungsmodus',
      subtitle: 'Auswahltests, Novura Exams und berufliche Assessments. Sprachtest-Simulation ist als eigener Einstieg getrennt.',
  ...
  function openTrainingCenterDeepSheet() {
    return openDeepSheet({
      ...
      theme: 'blue',
      title: brandModule('assessments', 'Novura Assessments'),
      kicker: 'Vorbereitung',
  ...
  function closeSheet () {
    ...
    if (window.NovuraDeepSheetNavigation && typeof window.NovuraDeepSheetNavigation.reset === 'function') {
      window.NovuraDeepSheetNavigation.reset();
    }
    ...
    try {
      ...
      document.body.classList.remove('egt-layer-open','egt-ui-layer-active','ui-overlay-open');
      document.dispatchEvent(new CustomEvent('egt:deep-sheet-closed', { detail:{ type: activeSheet || 'generic' } }));
      if (window.EGTUIRouter && typeof window.EGTUIRouter.sync === 'function') window.EGTUIRouter.sync(0);
    } catch(e) {}
    activeSheet = null;
  }
  ...
  function openCoachDeepSheet() {
    ...
    var body =
      ...
      '<div class="ui-deep-grid">' +
        actionCardHtml('coach-open-core', brandModule('coach', 'Novura Coach') + ' öffnen', 'Fragen stellen, Erklärungen holen und Lernhinweise prüfen', 'coach') +
      ...
      '</div>';
    return openDeepSheet({ type:'coach-menu', theme:'blue', title:brandModule('coach', 'Novura Coach'), kicker:'Coach', subtitle:'Analyse, Empfehlungen und nächste Trainingsschritte an einem Ort.', iconHtml:iconSvg('coach'), bodyHtml:body });
  }
  ...
  function analysisV2Seed(area, period, view) {
    /* G54.43.5 · Coach-Auswertung final
       ...
       können Sprachtraining, Simulation, Novura Exams/Novura Assessments, Mathematik, Logik, Wissen,
    ...
    if (!base) base = 58;
    var areaMap = {
      all:        { label:'Alle Bereiche',     lines:['Mathematik','Logik','Sprachtraining','Simulation','IT/FISI'], focus:'Zahlenreihen', score:base, best:rs.best || 0 },
      language:   { label:'Sprachtraining',        lines:['Lesen','Hören','Schreiben','Sprechen','Grammatik'], focus:'Grammatik', score:Math.min(100, base + 4), best:rs.best || 0 },
      ...
      novuraExams:        { label:'Novura Exams/Novura Assessments',           lines:['Allgemeinwissen','Mathematik','Zahlenreihen','Koordinaten','Wenn-Dann'], focus:'Zahlenreihen', score:Math.max(0, base - 4), best:rs.best || 0 },
  ...
  function analysisV2EmptyStateHtml(data) {
    ...
    return '<div class="analysis-v2-empty" data-analysis-empty-state="true">' +
      ...
      '<p>Starte Training, Simulation oder Sprachtraining. Danach ersetzt der Adapter diese Vorschau automatisch durch echte gespeicherte Werte.</p>' +
  ...
  function openAnalysisDeepSheet() {
    var areaOptions = [
      { value:'all', label:'Alle Bereiche' }, { value:'language', label:'Sprachtraining' }, { value:'simulation', label:'Simulation' },
      { value:'novuraExams', label:'Novura Exams/Novura Assessments' }, { value:'math', label:'Mathematik' }, { value:'logic', label:'Logik' },
    ...
    var body =
      '<div class="analysis-v2-shell" data-analysis-dashboard-v2="G54.43.8I">' +
        ...
        '</div>' +
      '</div>';
    var ok = openDeepSheet({ type:'analysis-dashboard-v2', theme:'green', title:brandModule('progress', 'Novura Progress'), kicker:'Dashboard V2 · Coach-Auswertung', subtitle:'Kurze Interpretation aus Prognose, Top-Schwäche und Fehleranalyse.', iconHtml:'▥', bodyHtml:body });
  ...
  function openProgressDeepSheet() {
    ...
    var body =
      ...
      '</div>';
    return openDeepSheet({ type:'progress-menu', theme:'green', title:brandModule('progress', 'Novura Progress'), kicker:'Menü', subtitle:'Dein Leistungsstand kompakt im zentralen Sheet.', iconHtml:'▤', bodyHtml:body });
  }
  ...
  function openSettingsDeepSheet() {
    var body =
      ...
      '<div class="ui-deep-grid">' +
        ...
        actionCardHtml('login-open-core', brandModule('admin', 'Novura Admin') + ' öffnen', 'Bestehendes Teilnehmer-, Dozenten- oder Adminportal', 'admin') +
        actionCardHtml('info-center', 'Informationen & Meine Reise', 'Portfolio, Vision, Kontakt und Rechtliches', (window.EGTAppIcons&&window.EGTAppIcons.icon?window.EGTAppIcons.icon('journey'):iconSvg('settings'))) +
        actionCardHtml('nova-control-center', 'Nova & Berechtigungen', 'Einführung, Tour, Begleitungsstil und optionale Rechte verwalten', 'coach') +
  ...
  function updateTopBar () {
    var currentBranch = localStorage.getItem('assessments_branch') || '';
  ...
  function renderGrid () {
    ...
    filtered.unshift({
      id: 'coach_card',
      label: brandModule('coach', 'Novura Coach'),
      iconName: 'coach',
      iconClass: 'c-blue',
      kicker: 'Lernhelfer',
      title: brandModule('coach', 'Novura Coach'),
    ...
    var analyseCard = {
      id: 'analyse_card',
      label: brandModule('progress', 'Novura Progress'),
      iconName: 'progress',
      iconClass: 'c-indigo',
      kicker: 'Statistiken',
      title: brandModule('progress', 'Novura Progress'),
  ...
  function renderTabBar () {
    ...
    var navItems = [
      ...
      { label: 'Exams', icon: 'target', action: 'simulation-center', center: true },
      { label: 'Assessments', icon: 'practice', action: 'training-center' },
      { label: brandModule('progress', 'Novura Progress'),    icon: 'progress',  action: 'analysis' },
  ...
  function renderSystemTab (el) {
    ...
    Array.prototype.forEach.call(el.querySelectorAll('.ui-branch-btn'), function (btn) {
      btn.addEventListener('click', function () {
        ...
        localStorage.setItem('assessments_branch', branchId);
  ...
  function parseScanTextToDraft (text, imageAnalysis) {
    ...
    return {
      type: type,
      branch: localStorage.getItem('assessments_branch') || '',
      module: '',
      difficulty: 'medium',
  ...
  function showScanReviewSheet (payload) {
    ...
    if (save) save.addEventListener('click', function () {
      try {
        ...
        var list = JSON.parse(localStorage.getItem('assessments_scan_drafts') || '[]');
        list.unshift(d);
        localStorage.setItem('assessments_scan_drafts', JSON.stringify(list.slice(0, 100)));
  ...
  function buildShell () {
        ...
        '<header class="ui-header">' +
          '<div class="ui-brand">' +
            ...
            '<div class="ui-brand-text">' +
              '<span class="ui-app-name">' + esc(BRAND.name || 'Novura') + '</span>' +
              '<span class="ui-app-sub">' + esc(BRAND.slogan || "One day, you'll thank today.") + '</span>' +
            '</div>' +
          '</div>' +
        ...
        '<main class="ui-home-scroll" id="uiHomeViewport">' +
          '<section class="ui-hero-ui product-home-hero" aria-label="Simulation starten">' +
            '<div class="ui-hero-copy">' +
              '<span class="ui-hero-pill novura-journey-kicker">' + esc(BRAND_COPY.heroEyebrow || 'Dein nächster Schritt beginnt heute') + '</span>' +
              '<h1 class="ui-hero-title">' + esc(BRAND_COPY.heroTitle || 'Trainiere heute. Danke dir morgen.') + '</h1>' +
              '<p>' + esc(BRAND_COPY.heroBody || 'Baue Fähigkeiten auf, trainiere realistisch und gehe vorbereitet in deinen nächsten Schritt.') + '</p>' +
              '<button class="ui-hero-cta" type="button" data-ui-action="simulation-center"><span class="ui-inline-icon">' + iconSvg('target', 'ui-inline-icon-svg') + '</span>Assessments &amp; Exams öffnen</button>' +
            '</div>' +
          ...
          '<section class="ui-sim-hero" aria-label="Eignungstest Simulation">' +
            '<button type="button" class="ui-sim-hero-card ui-sim-exam-hero-card" data-ui-action="simulation-center">' +
              ...
              '<span class="ui-sim-hero-copy">' +
                ...
                '<b>Novura Assessments &amp; Exams</b>' +
                '<small>Novura Assessments und Novura Exams realistisch starten. Keine Hilfe im Test, Auswertung und KI-Coach erst danach.</small>' +
              '</span>' +
              '<span class="ui-sim-hero-cta">Assessments &amp; Exams öffnen ›</span>' +
            '</button>' +
          '</section>' +

          '<section class="ui-sim-hero ui-language-sim-hero" aria-label="Sprachtest Simulation">' +
            '<button type="button" class="ui-sim-hero-card ui-sim-language-hero-card" data-ui-action="language-test-simulation-open">' +
              '<span class="ui-sim-hero-icon" aria-hidden="true">' + iconSvg('language') + '</span>' +
              '<span class="ui-sim-hero-copy">' +
                '<span class="ui-sim-kicker">Sprachtest · Vollprüfung</span>' +
                '<b>Sprachtest-Simulation</b>' +
                '<small>Deutschprüfung realistisch simulieren: Niveau wählen, komplette Vollprüfung starten, Ergebnis und Coach danach.</small>' +
              '</span>' +
              '<span class="ui-sim-hero-cta">Sprachtest öffnen ›</span>' +
            '</button>' +
          '</section>' +

          '<section class="ui-section-title-block">' +
            '<h2>' + esc(BRAND_COPY.journeyTitle || 'Your Journey') + '</h2>' +
            '<p>' + esc(BRAND_COPY.journeySubtitle || 'Lernen, trainieren, prüfen und wachsen – alles an einem Ort.') + '</p>' +
          '</section>' +
          '<section class="ui-training-area-grid" aria-label="Trainingsbereich wählen">' +
            ...
            '<button type="button" class="ui-training-area-card ui-area-individual language-course-hero-card" data-ui-action="language-course-open">' +
              '<span class="ui-area-icon">' + iconSvg('language') + '</span><b>' + esc(brandModule('learn', 'Novura Learn')) + '</b><small>' + esc(brandDescription('learn', 'Sprachen und Wissen systematisch aufbauen.')) + '</small>' +
            '</button>' +
          ...
          '<section class="ui-quick-grid product-recommend-grid" aria-label="Heute empfohlen">' +
            '<button type="button" class="ui-quick-card ui-area-it" data-ui-action="simulation-center"><span class="ui-quick-icon">' + iconSvg('target') + '</span><b>Eignungstest</b><small>Novura Assessments &amp; Novura Exams</small></button>' +
            '<button type="button" class="ui-quick-card ui-area-social" data-ui-action="language-test-simulation-open"><span class="ui-quick-icon">' + iconSvg('language') + '</span><b>Sprachtest</b><small>Deutsch &amp; Englisch Vollprüfung</small></button>' +
            ...
            '<button type="button" class="ui-quick-card ui-area-knowledge" data-ui-action="analysis"><span class="ui-quick-icon">' + iconSvg('progress') + '</span><b>' + esc(brandModule('progress', 'Novura Progress')) + '</b><small>' + esc(brandDescription('progress', 'Fortschritt und Prüfungsreife sichtbar machen.')) + '</small></button>' +
            '<button type="button" class="ui-quick-card ui-area-individual" data-ui-action="coach"><span class="ui-quick-icon">' + iconSvg('coach') + '</span><b>' + esc(brandModule('coach', 'Novura Coach')) + '</b><small>' + esc(brandDescription('coach', 'Persönliche Erklärungen und Empfehlungen.')) + '</small></button>' +
            ...
            '<button type="button" class="ui-quick-card language-course-hero-card" data-ui-action="language-course-open"><span class="ui-quick-icon">' + iconSvg('language') + '</span><b>Sprachtraining</b><small>Üben & Lernpfad öffnen</small></button>' +
    ...
    var currentBranch = localStorage.getItem('assessments_branch') || '';
    if (!currentBranch) {
      localStorage.setItem('assessments_branch', 'it');
      updateTopBar();
    }
    ...
    setTimeout(function () { if (!window.matchMedia || !window.matchMedia('(max-width: 780px)').matches) notice('Novura ist bereit. Dein nächster Schritt beginnt jetzt.'); }, 600);
  }
  ...
  window.EGTUILayer = {
    ...
    scan: scanQRCode,
    setBranch: function (branchId) { if (!branchId) return; localStorage.setItem('assessments_branch', branchId); updateTopBar(); renderGrid(); if (activeTab === TAB_PROFILE) { var c = $('uiTabContent'); if (c) renderProfileTab(c); } },
