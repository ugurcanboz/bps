...
(function(){
  ...
  var CLOSE_SELECTOR = [
    ...
    '.training-sheet-close', '.ui-sheet-close', '.assessments-sheet-close', '.clean-sheet-close',
    '.global-diagnostics-close', '.python-quest-close', '.novura-coach-close',
    '#fbCloseBtn', '#fbCancelBtn', '#assessmentsSheetClose', '#novuraCoachClose', '#uiSheetClose', '#uiScanClose',
    '[aria-label*="Schließen"]', '[aria-label*="schließen"]', '[aria-label*="Close"]', '[aria-label*="close"]'
  ].join(',');
  var OVERLAY_SELECTOR = [
    '.assessments-sheet-backdrop.show', '.assessments-deep-sheet.show', '.ui-sheet-backdrop.show', '.ui-sheet.show',
    ...
    '.clean-sheet-backdrop.show', '.clean-training-sheet.show', '.novura-coach-backdrop.show', '.novura-coach-sheet.show',
  ...
  var DIALOG_SELECTOR = [
    '.assessments-deep-sheet.show', '.ui-sheet.show', '.egt-admin-panel', '.egt-admin-modal.show', '#egtPasswordChangeModal.show',
    '.update-modal-card.is-visible', '.python-quest-shell.show', '.training-sheet.show', '.clean-training-sheet.show',
    '.novura-coach-sheet.show', '[data-sheet].show', '[data-sheet].is-visible', '[data-modal].show', '[data-modal].is-visible',
    '[role="dialog"].show', '[role="dialog"].is-visible'
  ].join(',');
  var BACKDROP_SELECTOR = '.assessments-sheet-backdrop,.ui-sheet-backdrop,.update-modal-backdrop,.python-quest-backdrop,.training-sheet-backdrop,.clean-sheet-backdrop,.novura-coach-backdrop,[data-overlay-backdrop]';
  var SCROLLABLE_SELECTOR = [
    '.assessments-sheet-body', '.ui-sheet-body', '.egt-admin-body', '.egt-admin-list', '.update-modal-card',
    '.python-quest-body', '.python-quest-shell', '.training-sheet-content', '.training-sheet-router', '.training-mode-panel',
    '.clean-sheet-router', '.clean-mode-panel', '.clean-category-list', '.training-mode-list', '.novura-coach-body', '.novura-coach-sheet',
    '.assessments-command-rewards', '.assessments-path-steps', '[data-sheet-scroll]', '[data-modal-scroll]',
  ...
  function modeForModule(id){
    var map={netzwerk:'it',hardware_os:'it',wirtschaft:'bueroWissen',din5008:'bueroWissen',kommunikation:'deutsch',entwicklung_bindung:'paedagogik',kommunikation_sozial:'situationen',recht_sozial:'paedagogik',doku_beobachtung:'paedagogik',simulation:'novuraExams',logik:'logic',matrizen:'matrixOnlySprint',mathe:'math',deutsch:'sentenceSprint',wissen:'general',englisch:'english',it:'it',konzentration:'concentrationPro',kaufmRechnen:'kaufmRechnen',bueroWissen:'bueroWissen',paedagogik:'paedagogik',situationen:'situationen',python_quest:'python_quest'};
  ...
  function closeUiLayerBeforePortal(){
    ...
    /* G54.45.2: Lock SYNCHRON lösen und Scrollposition sofort wiederherstellen,
       bevor das Admin-Portal seinen eigenen Sheet-Lock setzt. Vorher lief die
       Freigabe erst debounced über syncSoon — das Portal las scrollY=0 und die
       Position des Nutzers ging beim Schließen verloren (Sprung an den Anfang). */
  ...
  function setBranch(branch){ if(!branch) return; try{ localStorage.setItem('assessments_branch', branch); }catch(e){} try{ if(window.EGTUILayer && typeof window.EGTUILayer.refresh === 'function') window.EGTUILayer.refresh(); }catch(e){} }
  ...
  function startProductSimulation(el){
    ...
    var simType = el.getAttribute('data-simtype') || 'assessments';
    var mode = el.getAttribute('data-mode') || (simType === 'novuraExams' ? 'novuraExams' : 'assessments');
  ...
  function handleAction(action, el){
    ...
    if(action === 'language-test-simulation-open') { try{ if(window.LanguageExamShell && typeof window.LanguageExamShell.openSimulation === 'function'){ window.LanguageExamShell.openSimulation(); syncSoon(); return true; } if(window.LanguageExamShell && typeof window.LanguageExamShell.openSimulationGerman === 'function'){ window.LanguageExamShell.openSimulationGerman(); syncSoon(); return true; } if(window.LanguageExamShell && typeof window.LanguageExamShell.open === 'function'){ window.LanguageExamShell.open(); syncSoon(); return true; } }catch(e){ notice('Sprachtest-Simulation: '+(e.message||e)); return true; } notice('Sprachtest-Simulation wird geladen.'); return true; }
    ...
    if(action === 'info-center' || action.indexOf('info-') === 0 || action.indexOf('legal-') === 0) { try { if(window.EGTInfoLegalCenter && typeof window.EGTInfoLegalCenter.handleAction === 'function') { var infoHandled=window.EGTInfoLegalCenter.handleAction(action); if(infoHandled){ syncSoon(); return true; } } } catch(e){ notice('Informationscenter: '+(e.message||e)); return true; } }
  ...
  function run(el, ev){
    ...
    if(el.matches && (el.matches('.egt-dock-btn[data-tab]') || el.matches('[data-ui-nav="tab"][data-tab]'))) {
      ...
      if(el.matches('[data-ui-nav="tab"][data-tab]')){
        ...
        try { qa('.ui-deep-sheet.show,.ui-deep-sheet.is-visible,.assessments-deep-sheet.show,.assessments-sheet-backdrop.show').forEach(function(s){ s.classList.remove('show','is-visible'); }); } catch(e){}
      }
  ...
  function callKnownClose(target){
    var handled=false;
    if(closest(target,'.novura-coach-sheet,.novura-coach-backdrop')){ try{ if(window.EGTLearningCoach && typeof window.EGTLearningCoach.close === 'function'){ window.EGTLearningCoach.close(); handled=true; } }catch(e){} qa('.novura-coach-backdrop.show,.novura-coach-sheet.show').forEach(function(el){ el.classList.remove('show'); handled=true; }); }
    if(closest(target,'.assessments-deep-sheet,.assessments-sheet-backdrop,.training-sheet,.training-sheet-backdrop,.clean-training-sheet,.clean-sheet-backdrop')){ try{ if(window.EGTTrainerDeepSheet && typeof window.EGTTrainerDeepSheet.close === 'function'){ window.EGTTrainerDeepSheet.close(); handled=true; } }catch(e){} try{ if(window.App && typeof window.App.closeTrainingSheet === 'function'){ window.App.closeTrainingSheet(); handled=true; } }catch(e){} qa('.assessments-deep-sheet.show,.assessments-sheet-backdrop.show,.training-sheet.show,.training-sheet-backdrop.show,.clean-training-sheet.show,.clean-sheet-backdrop.show').forEach(function(el){ el.classList.remove('show'); handled=true; }); }
    ...
    if(closest(target,'.egt-admin-modal,.egt-admin-panel')){ try{ if(window.EGTAdminPortal && typeof window.EGTAdminPortal.close === 'function'){ window.EGTAdminPortal.close(); handled=true; } }catch(e){} qa('.egt-admin-modal.show,#egtPasswordChangeModal.show').forEach(function(el){ el.classList.remove('show'); handled=true; }); try{ document.documentElement.classList.remove('egt-admin-sheet-open'); document.body.classList.remove('egt-admin-sheet-open'); }catch(e2){} }
