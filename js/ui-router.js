...
(function(){
  ...
  function openCoreAnalysis(){ if(startHostModule('analysis-entry', { action: 'analysis-open-core' }, 'Analyse-Modul')) return true; try{ if(window.App && typeof window.App.showAnalysis === 'function'){ window.App.showAnalysis(); syncSoon(); return true; } }catch(e){} return openActionMenu('progress') || switchTab(1); }
  ...
  function handleAction(action, el){
    ...
    if(action === 'language-course-open') { if(startHostModule('language-course-entry', { action:'language-course-open' }, 'Sprachkurs-Modul')) return true; try{ if(window.LanguageAcademyCourseEntry && typeof window.LanguageAcademyCourseEntry.open === 'function'){ window.LanguageAcademyCourseEntry.open(); syncSoon(); return true; } }catch(e){ notice('Sprachkurs-Fehler: '+(e.message||e)); return true; } return openActionMenu('language-course'); }
    ...
    if(action === 'analysis') { if(startHostModule('analysis-entry', { action: 'analysis' }, 'Analyse-Modul')) return true; return openAnalysis(); }
  ...
  function run(el, ev){
    ...
    if(el.matches && (el.matches('.egt-dock-btn[data-tab]') || el.matches('[data-ui-nav="tab"][data-tab]'))) {
      ...
      else if(tab === 'analysis') ok=openCoreAnalysis() || openAnalysis();
