...
(function(){
  ...
  function openCoreCoach(){ try{ if(window.EGTLearningCoach && typeof window.EGTLearningCoach.openHub === 'function'){ window.EGTLearningCoach.openHub(); syncSoon(); return true; } }catch(e){} return openActionMenu('coach'); }
  function openCoreAnalysis(){ try{ if(window.App && typeof window.App.showAnalysis === 'function'){ window.App.showAnalysis(); syncSoon(); return true; } }catch(e){} return openActionMenu('progress') || switchTab(1); }
  ...
  function handleAction(action, el){
    ...
    if(action === 'admin-open') { try{ if(window.EGTAdminPortal && typeof window.EGTAdminPortal.open === 'function'){ closeUiLayerBeforePortal(); window.EGTAdminPortal.open(); syncSoon(); return true; } if(window.App && typeof window.App.showFrameworkHealth === 'function'){ window.App.showFrameworkHealth(); syncSoon(); return true; } }catch(e){ notice('Admin-Fehler: '+(e.message||e)); } return false; }
    ...
    if(action === 'duel-hub') { try{ if(window.EGTUILayer && typeof window.EGTUILayer.openDuelSheet === 'function'){ window.EGTUILayer.openDuelSheet(); syncSoon(); return true; } if(window.App && typeof window.App.openDuellSetup === 'function'){ window.App.openDuellSetup(); return true; } }catch(e){ notice('Duell-Fehler: '+(e.message||e)); } return false; }
  ...
  function run(el, ev){
    ...
    if(el.matches && el.matches('.egt-dock-btn[data-tab]')) {
      ...
      if(/^(home|highscore|duels|profile)$/.test(tab)) ok=switchTab(tab);
      else if(tab === 'simulation') ok=trainingMenu('simulation') || openModuleTarget('simulation');
      else if(tab === '0') ok=switchTab('home');
      else if(tab === '1') ok=switchTab('highscore');
      else if(tab === '2') ok=trainingMenu('simulation') || openModuleTarget('simulation');
      else if(tab === '3') ok=switchTab('duels');
      else if(tab === '4') ok=switchTab('profile');
