...
(function(){
  ...
  function isSimulationContext(value){ return (value || getContext()) === 'simulation-de'; }
  ...
  function renderSimulationHomeGerman(){
    ...
    var body = '<div class="la-dashboard la-exam-shell la-simulation-language-exam" data-la-exam-shell="phase8l" data-la-simulation-language="de">' +
      '<section class="la-card la-exam-hero"><span class="la-section-kicker">Sprachtest-Simulation · Deutsch</span><h3>Deutsch-Vollprüfung A1 bis C2</h3><p>Diese Simulation ist immer eine komplette Prüfung. Es gibt keine Teilprüfungsauswahl und keine Hilfen während des Tests. Geprüft werden Lesen, Hören, Grammatik & Sprachbausteine, Schreiben und Sprechen. Auswertung und Coach-Hinweise erscheinen erst nach Abschluss.</p><div class="la-exam-warning"><b>Simulationsregel:</b> Alles hier ist Vollprüfung. Einzelne Bereiche wie Lesen, Hören, Schreiben oder Sprechen gehören ins Sprachtraining.</div></section>' +
      ...
      '<section class="la-card"><span class="la-section-kicker">Niveau wählen</span><h3>Welche Deutsch-Vollprüfung willst du simulieren?</h3><p class="la-note">Deutsch ist zuerst aktiv. Englisch wird erst ergänzt, wenn Deutsch sauber getrennt und getestet ist.</p><div class="la-exam-level-grid">'+levelList().map(levelCard).join('')+'</div></section>' +
  ...
  function renderResult(session){
    ...
    return '<section class="la-card la-exam-final '+finalCls+'">' +
      ...
      '<div class="la-level-actions"><button type="button" class="la-primary" data-ui-action="language-exam-reset">Neue Simulation</button>'+(isSimulationContext(session.context)?'<button type="button" class="la-secondary" data-ui-action="language-exam-simulation-home">Zurück zur Sprachtest-Simulation</button>':'<button type="button" class="la-secondary" data-ui-action="language-course-open">Zurück zum Sprachtraining</button>')+'</div>' +
      '</section>';
  }
  ...
  function start(level){
    ...
    session.examLanguage = isSimulationContext(session.context) ? 'de' : (session.examLanguage || 'de');
    session.simulationMode = isSimulationContext(session.context) ? 'de-full-exam' : 'training-linked-exam';
    return renderSession(saveSession(session));
  }
  ...
  function handleClick(ev){
    ...
    if(action === 'language-exam-simulation-home') return renderSimulationHomeGerman();
    if(action === 'language-exam-home') return isSimulationContext() ? renderSimulationHomeGerman() : renderHome();
    ...
    if(action === 'language-exam-reset'){ var sim=isSimulationContext(); clearSession(); return sim ? renderSimulationHomeGerman() : renderHome(); }
