/* Eignungstest-Trainer · G34.2 Login/Profile Hotfix
   Duell-Vorbereitung, Sync-Status und Offline-Hinweise ergänzen Profil/Firebase,
   ohne echte Duelle zu schreiben oder Üben/Highscore/Simulation umzubauen. */
(function(){
  'use strict';

  var VERSION = 'G34.2-LOGIN-PROFILE-HOTFIX-2026-06-08';
  ...
  var DEMO_SIMULATION_MODES = { ctcLohr:true, ctc:true, bps:true, simulation:true };
  ...
  function localSession(){ return safeJson(localStorage.getItem(SESSION_KEY), null); }
  function saveLocalSession(session){ try { localStorage.setItem(SESSION_KEY, JSON.stringify(session || {})); } catch(e){} emit('egt:auth-profile-updated', { session: session || null }); refreshUi(); }
  function clearLocalSession(){ try { localStorage.removeItem(SESSION_KEY); } catch(e){} emit('egt:auth-profile-updated', { session: null }); refreshUi(); }
  ...
  function currentSession(){
    var db = userDb();
    try { if(db && typeof db.getSession === 'function'){ var remoteSession = db.getSession(); if(remoteSession) return Object.assign({ source: remoteSession.provider || 'userdatabase' }, remoteSession); } } catch(e){}
  ...
  function readCoachContextCache(){ return safeJson(localStorage.getItem(COACH_CONTEXT_CACHE_KEY), null); }
  ...
  function demoState(){
    var demo = safeJson(localStorage.getItem(DEMO_KEY), null) || {};
  ...
  function featureForAction(action, el){
    ...
    if(/^(auth|profile)-/.test(action) || action === 'login' || action === 'login-open-core') return 'auth';
    if(action === 'settings' || action === 'clear-cache') return 'settings';
    if(action === 'feedback' || action === 'feedback-open-core') return 'feedback';
    if(action === 'start-training' || action === 'auth-demo-simulation-start') return 'simulation';
    ...
    if(action === 'practice' || action === 'learn') return action;
    if(action === 'coach' || action === 'coach-open-core') return 'coach';
    if(action === 'analysis' || action === 'analysis-open-core' || action === 'progress') return 'progress';
    if(action === 'python-quest' || action === 'python-quest-open-core') return 'python';
    if(action === 'scan' || action === 'scan-open-core') return 'scan';
    if(action === 'backup' || action === 'clear-progress') return 'data';
    return 'practice';
  }
  function accessDecision(feature, meta){
    ...
    if(role === 'admin' || role === 'teacher' || role === 'participant') return { ok:true, feature:feature, reason:'role:'+role };
    if(role === 'demo') {
  ...
  function openDemoSimulation(){
    var decision = accessDecision('simulation', { mode:'ctcLohr', action:'auth-demo-simulation-start' });
  ...
  function startReservedDemoSimulation(){
    ...
    window.setTimeout(function(){
      var started = false;
      try { if(window.App && typeof window.App.chooseTrainingMode === 'function') window.App.chooseTrainingMode('ctcLohr'); } catch(e0){}
  ...
  function recordDemoSimulation(details){
    ...
    if(demo.used >= demo.max) notice('Demo-Limit erreicht. Für weitere Bereiche brauchst du einen Zugangscode.');
  ...
  function avatarGet(key){
    return openAvatarDb().then(function(db){ return new Promise(function(resolve, reject){
      ...
      var meta = safeJson(localStorage.getItem(AVATAR_META_KEY), {});
  ...
  function openAuthGate(){
    return openDeepSheet({
      type:'auth-profile-gate', theme:'blue', title:'Login & Profil', kicker:'Zugang',
      subtitle:'Benutzerprofil, Zugangscode, Demo und Firebase UserDatabase als sauberer neuer Ast.', iconHtml:'👤',
  ...
  function renderSlot(){
    ...
    var session = currentSession();
    if(session){
      var warn = needsNickname(session) ? '<small class="up-slot-warning">Nickname fehlt</small>' : '<small>' + esc(roleLabel(session.role)) + (session.groupId ? ' · ' + esc(session.groupId) : '') + '</small>';
      slot.innerHTML = '<section class="up-auth-panel up-auth-panel-active">' +
        '<div class="up-auth-copy"><span class="up-kicker">Profil aktiv</span><b>' + esc(session.nickname || 'Benutzer') + '</b>' + warn + '</div>' +
        '<div class="up-auth-actions"><button type="button" class="ui-btn" data-ui-action="profile-open">Profil</button><button type="button" class="ui-btn" data-ui-action="auth-open">Zugang</button></div>' +
      '</section>';
    } else {
      slot.innerHTML = '<section class="up-auth-panel">' +
        '<div class="up-auth-copy"><span class="up-kicker">Neuer Zugang</span><b>Login, Zugangscode oder Demo</b><small>Profil-Ast vorbereitet. Noch ohne harte Sperren.</small></div>' +
        '<div class="up-auth-actions"><button type="button" class="ui-btn" data-ui-action="auth-open">Starten</button><button type="button" class="ui-btn" data-ui-action="auth-demo-start">Demo</button></div>' +
      '</section>';
    }
    updateTopLogin();
    setTimeout(function(){ hydrateAvatars(slot); }, 80);
    return true;
  }
  ...
  function updateTopLogin(){
    ...
    if(!btn) return;
    var session = currentSession();
    btn.setAttribute('data-ui-action', session ? 'profile-open' : 'auth-open');
    btn.textContent = session ? 'Profil' : 'Login';
    btn.setAttribute('aria-label', session ? 'Profil öffnen' : 'Login und Zugang öffnen');
  }
  ...
  function boot(){
    ...
    (function waitForShell(){
      tries++;
      if(renderSlot()){
        try { if(!localStorage.getItem(FIRST_SEEN_KEY)){ localStorage.setItem(FIRST_SEEN_KEY, nowIso()); } } catch(e){}
        return;
      }
