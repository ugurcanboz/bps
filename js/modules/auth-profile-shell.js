...
(function(){
  ...
  var VERSION = 'G35.0-USER-CENTER-STEP1-2026-06-08';
  ...
  function updateTopLogin(){
    ...
    btn.setAttribute('aria-label', isActive ? 'Benutzer öffnen' : 'Login und Registrierung öffnen');
    btn.classList.toggle('is-user-active', isActive);
    var key = session ? avatarKeyFor(session) : '';
    var label = isActive ? 'Benutzer' : 'Login';
    btn.innerHTML = '<span class="ui-login-avatar" data-ui-login-avatar="1"' + (key ? ' data-up-avatar-key="' + esc(key) + '"' : '') + '>' + userIconSvg() + '</span><span class="ui-login-label">' + esc(label) + '</span>';
