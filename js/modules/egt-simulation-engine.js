...
(function () {
  ...
  function renderAnswers(question) {
    ...
    } else if (q.type === 'ctcFlowLogic' && window.App && typeof window.App.renderCtcFlowLogicAnswers === 'function') {
      handledBy = 'module-ctc-flowlogic-adapter';
      result = window.App.renderCtcFlowLogicAnswers(q);
    } else if ((q.type === 'ctcRuleArithmeticInput' || q.type === 'ctcLetterScanInput' || q.type === 'ctcCoordinateTableInput' || q.type === 'ctcNumberSeriesInput') && window.App && typeof window.App.renderCtcLohrInputAnswers === 'function') {
      handledBy = 'module-ctc-lohr-input-adapter';
      result = window.App.renderCtcLohrInputAnswers(q);
    } else {
  ...
  function getResultTarget(mode) {
    if (mode === 'ctc') return 70;
    if (mode === 'bps') return 75;
    return 80;
  }
