/* egt-gate-enforce.js — G54.46.2 Sperrbildschirm-Enforcement
...
(function () {
  ...
  /* G54.46.2: QA-Bypass darf ausschließlich der zentrale Sicherheitskontext
     in einer lokalen Entwicklungsumgebung freigeben. URL/LocalStorage allein
     besitzen niemals Berechtigungswirkung. */
  function qaBypassEnabled() {
    try {
      var sec = window.EGTSecurityContext;
      if (!(sec && sec.qaBypassAllowed && sec.qaBypassAllowed())) {
        localStorage.removeItem('egt_qa_bypass_v1');
        return false;
      }
      if (window.EGTAuthEngine && window.EGTAuthEngine.qaBypass === true) return true;
      return false;
    } catch(e) {
      return false;
    }
  ...
  function startPeriodicCheck() {
    setInterval(function () {
      ...
      var gs = eng.gateStatus();
      if (!gs.open && !(window.NovuraGuidedWelcomeUI && NovuraGuidedWelcomeUI.isOpen && NovuraGuidedWelcomeUI.isOpen())) showGate(gs.reason);
    }, 60000); // jede Minute
  }

  /* Gate-Status-Change: wenn gateOpen=true -> Sperrbildschirm entfernen */
  function handleGateOpen() {
    try { if (window.NovuraAuthAccess && NovuraAuthAccess.close) NovuraAuthAccess.close(); } catch(e){}
