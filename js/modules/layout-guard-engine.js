...
(function () {
  ...
  var enabled = true;
  var EXPERIMENT_CLASSES = ['training-cockpit'];
  ...
  function check(opts) {
    ...
    // 1) Experiment-Klassen am Quiz-Container
    ...
    if (rTop && rQ && overlapRatio(rTop, rQ) > 0.25) {
      ...
      try {
        ...
        [topbar, question, answers, visual].forEach(function (el) {
          if (!el || !el.style) return;
          ['grid-area', 'grid-column', 'grid-row', 'position', 'top', 'left', 'transform']
            .forEach(function (p) { el.style.removeProperty(p); });
        });
        // #questionNav bewusst ausgenommen: die Frageübersicht-Schublade nutzt
        // position:fixed + transform und darf nicht zurückgesetzt werden.
        if (nav && nav.style) { ['grid-area', 'grid-column', 'grid-row'].forEach(function (p) { nav.style.removeProperty(p); }); }
