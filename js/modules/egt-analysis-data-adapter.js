...
(function () {
  ...
  var MODULES = {
    all:        { label:'Alle Bereiche',   keys:['all'],        lines:['Mathematik','Logik','Sprachkurs','Simulation','IT/FISI'], focus:'Zahlenreihen', base:58 },
    language:   { label:'Sprachkurs',      keys:['language','sprachkurs','deutsch-a1-c2','language-course','speaking','hoeren','hören'], lines:['Lesen','Hören','Schreiben','Sprechen','Grammatik'], focus:'Grammatik', base:62 },
    ...
    ctc:        { label:'CTC/BPS',         keys:['ctc','bps','lohr','ctc/bps'], lines:['Allgemeinwissen','Mathematik','Zahlenreihen','Koordinaten','Wenn-Dann'], focus:'Zahlenreihen', base:54 },
  ...
  function readStorageResults() {
    ...
    try {
      var keys = ['eignungstest_trainer_results_v1','eignungstest_trainer_results','egt_results','egt_results_v1','trainer_results','bps_results'];
  ...
  function lineMatchForLabel(item, label, area) {
    ...
    if (area === 'all') {
      ...
      if (label === 'Sprachkurs') return matchesArea(item, 'language');
  ...
  function buildCoachInsight(ctx) {
    ...
    var summary = 'Für diese Auswahl liegen noch keine gespeicherten Ergebnisse vor. Starte zuerst ein Training, eine Simulation oder einen Sprachkurs-Test. Danach bewertet der Coach Leistung, Risiko und nächsten Fokus automatisch.';
