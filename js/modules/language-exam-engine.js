...
(function(){
  ...
  var VERSION = 'G54.38C.14-b1-hardmode-total-qa-engine';
  ...
  function levelComplexitySignals(userText){
    ...
    var ownPosition = /\b(meine meinung|meiner meinung|meine begründete meinung|ich bin der meinung|ich halte|ich finde|meines erachtens|aus meiner sicht|ich vertrete die auffassung|ich stimme|ich lehne)\b/i.test(t);
    var proContra = /\b(vorteil|nachteile?|chance|risiko|problem|kritik|lösung|einerseits|andererseits|dafür|dagegen)\b/i.test(t);
    var example = /\b(zum beispiel|beispielsweise|etwa|konkret|in meinem alltag|ein beispiel)\b/i.test(t);
    return { wordCount:wc, sentenceCount:sentences, argumentConnectors:argumentConnectors, ownPosition:ownPosition, proContra:proContra, example:example };
  }
  function levelDifferentiationCaps(level, part, userText, local){
    ...
    if((part === 'writing' || part === 'speaking') && profile.severity >= 4){
      if(signals.wordCount > 0 && signals.wordCount < Math.round((profile[part === 'writing' ? 'writingWords' : 'speakingWords'][0] || 100) * 0.75)){
        reasons.push(level + ': Umfang liegt deutlich unter dem Niveauprofil. Eine A1/A2/B1-Antwort darf hier nicht als '+level+' bestehen.');
        caps.push(profileCaps.tooShort || 40);
      }
      ...
      if(part === 'writing' && !signals.example){ reasons.push(level + ': konkretes Beispiel fehlt.'); caps.push(Math.min(profileCaps.noArgumentation || 58, 62)); }
    }
  ...
  function combineHybrid(level, part, local, ai){
    ...
    if(part === 'speaking' && local.minWords && local.wordCount < Math.round(local.minWords * 0.65)) hardFail.push('Sprechantwort deutlich unter dem Niveau-Mindestumfang.');
  ...
  function buildRecommendations(breakdown, bp){
    ...
    if(byPart.grammar && byPart.grammar.score !== null && byPart.grammar.score < partMinScore(bp, 'grammar')) rec.push('Grammatik & Sprachbausteine: Wiederhole Konnektoren, Satzstellung, Kasus/Präpositionen und höfliche Formulierungen mit Lückentexten.');
