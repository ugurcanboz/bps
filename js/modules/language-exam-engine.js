...
(function(){
  ...
  var VERSION = 'G54.46.10-calibrated-full-exam-engine';
  ...
  function semanticRequiredHit(text, point, part){
    ...
    if(/anrede|begrüßung|einstieg|adressat|greeting|opening|introduction|thesis/.test(point)) return markerHit(text, [/sehr geehrte/, /guten tag/, /hallo/, /liebe\s+/, /dear/, /hello/, /good morning/, /good afternoon/, /introduction/, /this (essay|response|comment)/, /i would like to/, /merhaba/]);
    if(/abschluss|schlussformel|verabschiedung|closing|conclusion|recommendation/.test(point)) return markerHit(text, [/mit freundlichen grüßen/, /freundliche grüße/, /vielen dank/, /danke/, /beste grüße/, /ich freue mich auf/i, /rückmeldung/i, /kind regards/, /best regards/, /thank you/, /in conclusion/, /to conclude/, /overall/, /i recommend/, /should therefore/]);
    if(/verspätung/.test(point)) return markerHit(text, [/später/, /verspät/, /bus.*spät/, /komme.*nicht.*pünktlich/]);
    if(/problem|beschreiben|mitteilen|abwesenheit|lärm|zugang|funktioniert|beschwerde|situation|issue|complaint/.test(point)) return markerHit(text, [/problem/, /issue/, /does not work/, /cannot/, /unable/, /funktioniert nicht/, /nicht funktioniert/, /zugang/, /access/, /beschwer/, /complain/, /lärm/, /noise/, /laut/, /abwesen/, /absence/, /nicht kommen/, /not attend/, /nicht arbeiten/, /kann .* nicht/, /since\s+\w+/, /seit\s+\w+/, /seit\s+\d+/]);
    if(/folge|nachteil|konsequenz|risk|limit|counterpoint|disadvantage/.test(point)) return markerHit(text, [/deshalb/, /dadurch/, /folge/, /nachteil/, /risiko/, /grenze/, /however/, /on the other hand/, /risk/, /limit/, /disadvantage/, /consequence/, /missed/, /could not/, /cannot attend/, /problem for/]);
    if(/lösung|maßnahme|fordern|vorschlagen|ersatz|rückzahlung|alternative|schichttausch|request|suggestion|solution/.test(point)) return markerHit(text, [/bitte/, /ich bitte/, /ich fordere/, /lösung/, /ersatz/, /rückzahlung/, /zusätzliche/, /neuen termin/, /maßnahme/, /schichttausch/, /tausch/, /alternative/, /vorschlag/, /könnten sie/, /please/, /i request/, /i suggest/, /could you/, /would it be possible/, /solution/, /alternative/]);
    ...
    if(/grund|begründung|reason|argument/.test(point)) return markerHit(text, [/weil/, /da ich/, /wegen/, /grund/, /because/, /since/, /therefore/, /the reason/, /argument/, /termin/, /appointment/, /krank/, /ill/, /behörde/, /arzt/, /familie/, /bus.*spät/]);
    if(/rückmeldung|bestätigung/.test(point)) return markerHit(text, [/rückmeldung/, /bestätigung/, /bestätigen/, /antwort/, /melden/, /bescheid/]);
    if(/sachlich|höflich|ton|respektvoll|register|polite|appropriate/.test(point)) return !markerHit(text, [/idiot/, /schei/, /dumm/, /frech/, /sofort!!!/, /stupid/, /shut up/]) && markerHit(text, [/bitte/, /danke/, /freundlich/, /höflich/, /verständnis/, /please/, /thank/, /kind regards/, /would/, /could/]);
    return null;
  }
  ...
  function levelComplexitySignals(userText){
    ...
    var argumentConnectors = (t.match(/\b(einerseits|andererseits|allerdings|dennoch|außerdem|zudem|folglich|daher|deshalb|während|obwohl|hingegen|im gegensatz|aus diesem grund|beispielsweise|zum beispiel|meiner ansicht|ich bin der meinung|meiner meinung|zusammenfassend|abschließend|on the one hand|on the other hand|however|nevertheless|moreover|furthermore|therefore|consequently|although|whereas|in contrast|for this reason|for example|in my view|in my opinion|to conclude|in conclusion)\b/g) || []).length;
    var ownPosition = /\b(meine meinung|meiner meinung|meine begründete meinung|ich bin der meinung|ich halte|ich finde|meines erachtens|aus meiner sicht|ich vertrete die auffassung|ich stimme|ich lehne|ich würde empfehlen|ich empfehle|meiner auffassung nach|in my opinion|in my view|i believe|i think|i would argue|i recommend|my position|from my perspective)\b/i.test(t);
    var proContra = /\b(vorteil|vorteile|nachteile?|chance|chancen|risiko|risiken|problem|probleme|kritik|lösung|einerseits|andererseits|dafür|dagegen|gegenargument|grenze|kosten|folgen|advantage|advantages|disadvantage|disadvantages|opportunity|opportunities|risk|risks|benefit|benefits|drawback|drawbacks|counterargument|limitation|limitations|on the one hand|on the other hand)\b/i.test(t);
    var example = /\b(zum beispiel|beispielsweise|etwa|konkret|in meinem alltag|aus meinem alltag|in der arbeit|bei der arbeit|ein beispiel|als beispiel|for example|for instance|a concrete example|in my experience|at work|in everyday life)\b/i.test(t);
    var justification = /\b(weil|da|denn|daher|deshalb|aus diesem grund|aufgrund|begründen|begründung|argument|führt dazu|dadurch|folglich|die folge|das liegt daran|because|since|therefore|for this reason|as a result|this means|the reason|argument)\b/i.test(t);
  ...
  function hasFormalFrame(userText){
    var t = cleanText(userText, 12000).toLowerCase();
    var greeting = /(sehr geehrte|guten tag|hallo|liebe|dear|hello|good morning|good afternoon|merhaba)/i.test(t);
    var closing = /(mit freundlichen grüßen|freundliche grüße|vielen dank|danke|beste grüße|gruß|hochachtungsvoll|kind regards|best regards|yours sincerely|thank you)/i.test(t);
    var polite = /(bitte|vielen dank|danke|freundlich|verständnis|rückmeldung|mit freundlichen|please|thank|appreciate|would|could|kind regards)/i.test(t);
  ...
  function writingStructureCheck(userText, task){
    ...
    var subject = /(betreff|beschwerde|anfrage|bitte um|termin|schichttausch|lärm|onlinekurs|rückerstattung|subject|complaint|request|appointment|refund|course|issue)/i.test(t);
    ...
    var registerNeeded = /(beschwerde|teamleitung|hausverwaltung|anbieter|arbeitgeber|behörde|vermieter|complaint|manager|provider|employer|authority|landlord|formal\s+(e-mail|email|message|complaint)|formelle\s+(e-mail|mail|nachricht|beschwerde)|formell\s+schreiben)/.test(taskText) && !/(halbformell|stellungnahme|meinungsbeitrag|argumentativer beitrag)/.test(taskText);
    var argumentativeText = /(stellungnahme|argumentativ|argumentative|meinungsbeitrag|beitrag|essay|comment|response|weigh|position|argument)/.test(taskText) && !registerNeeded;
  ...
  function speakingStructureCheck(userText, task){
    ...
    var sentenceCount = (raw.match(/[.!?]/g) || []).length || raw.split(/\b(weil|deshalb|außerdem|zudem|allerdings|dennoch|andererseits|because|therefore|moreover|however|nevertheless|on the other hand)\b/i).length - 1;
    var sentenceLike = sentenceCount >= 3 || (wc >= 80 && /\b(weil|deshalb|außerdem|zudem|allerdings|dennoch|andererseits|einerseits|because|therefore|moreover|however|nevertheless|on the one hand|on the other hand)\b/i.test(t));
    var taskText = String(((task && task.prompt) || '') + ' ' + ((task && task.requiredPoints || []).join(' ')) + ' ' + ((task && task.requiredSpeechStructure || []).join(' '))).toLowerCase();
    var argumentativeTask = (task && Number(task.minWords || 0) >= 150) || /vorteil|nachteil|argument|position|abwäg|pro|contra|fazit|kompromiss|empfehlung|advantage|disadvantage|opportunit|risk|weigh|conclusion|recommendation/i.test(taskText);
    ...
    var reason = /\bweil\b|\bwegen\b|\bda\b|\bdenn\b|grund|begründ|argument|aufgrund|deshalb|daher|dadurch|folge|führt dazu|because|since|therefore|reason|as a result|krank|ill|termin|appointment|zug|behörde|arbeit|work|familie|urgent/i.test(t);
    var request = /bitte|könnten|würden|vorschlag|möglich|rückmeldung|bestätigung|ersatz|rückerstattung|tauschen|neuen termin|lösung|frage|fragen|please|could|would|suggest|possible|reply|confirmation|replacement|refund|new appointment|solution|question/i.test(t);
    var question = /\?|könnten sie|ist es möglich|wann|wo|wie viel|kost|uhrzeit|treffpunkt|passt|could you|would it be possible|when|where|how much|time|meeting point|does .* suit/i.test(t);
    ...
    if(argumentativeTask){
      var intro = /\b(thema|frage|diskussion|es geht|zunächst|heutzutage|in vielen|aktuell|immer mehr|die frage ist|topic|issue|debate|first|nowadays|currently|in many|the question is)\b/i.test(t) || wc >= 80;
      var proContra = /\b(vorteil|vorteile|nachteil|nachteile|chance|chancen|risiko|risiken|einerseits|andererseits|dafür|dagegen|problem|grenze|kosten|folgen|advantage|advantages|disadvantage|disadvantages|opportunity|opportunities|risk|risks|on the one hand|on the other hand|benefit|drawback|limitation|cost|consequence)\b/i.test(t);
      var ownPosition = /\b(meiner meinung|meine meinung|ich bin der meinung|ich halte|ich finde|aus meiner sicht|meines erachtens|ich vertrete|ich empfehle|ich würde empfehlen|in my opinion|in my view|i believe|i think|i would argue|i recommend|from my perspective)\b/i.test(t);
      var example = /\b(zum beispiel|beispielsweise|konkret|etwa|in meinem alltag|aus meinem alltag|in der arbeit|bei der arbeit|ein beispiel|for example|for instance|a concrete example|in my experience|at work|in everyday life)\b/i.test(t);
      var conclusion = /\b(fazit|abschließend|zusammenfassend|deshalb sollte|daher sollte|ich empfehle|empfehlung|kompromiss|lösung|sinnvoll wäre|realistisch wäre|in conclusion|to conclude|overall|therefore should|i recommend|recommendation|compromise|solution|a realistic approach)\b/i.test(t);
  ...
  function localAssessResponse(options){
    ...
    var local = {
      ...
      part:part,
      taskId:task.id || null,
      language:String(task.language || options.language || 'de').toLowerCase(),
      wordCount:wc,
      minWords:min,
      maxWords:max,
  ...
  function normalizeAiResult(aiResponse){
    ...
    return {
      ...
      fluencyScore: r.evidence && r.evidence.audioAnalyzed && r.fluencyScore != null ? clamp(r.fluencyScore, 0, 100) : null,
      textFlowScore: r.textFlowScore == null ? (r.fluencyScore == null ? null : clamp(r.fluencyScore, 0, 100)) : clamp(r.textFlowScore,0,100),
      pronunciationScore: r.pronunciationScore == null ? null : clamp(r.pronunciationScore,0,100),
      intonationScore: r.intonationScore == null ? null : clamp(r.intonationScore,0,100),
      audioFluencyScore: r.audioFluencyScore == null ? null : clamp(r.audioFluencyScore,0,100),
      evidence:r.evidence||null, speakingAssessment:r.speakingAssessment||null, assessmentScope:r.assessmentScope||null,
      offTopic: !!r.offTopic,
  ...
  async function assessAiExamPart(options){
    ...
    var userText = cleanText(options.userText || options.message, 12000);
    var evidence=part==='speaking'&&speakingEvidenceEngine()&&speakingEvidenceEngine().createEvidence?speakingEvidenceEngine().createEvidence(Object.assign({},options.evidence||{},{transcript:userText})):null;
    ...
    try{
      ...
      var res = await speakingAI().checkExamSpeaking({
        ...
        evidence:evidence,
        rubric: part === 'writing' ? {
          role:'strenger '+level+'-Schreibprüfer im Deutsch-Vollprüfung',
          ...
          role:'strenger '+level+'-Sprechprüfer im Deutsch-Vollprüfung',
          levelProfile: difficulty(level) ? difficulty(level).label : level,
          expectedSkills: difficulty(level) ? difficulty(level).skills : [],
    ...
    if(part==='speaking'){
      hybrid.evidence=evidence;
      hybrid.assessmentScope=evidence&&evidence.assessmentScope||'transcript-only';
      hybrid.assessmentCompleteness=evidence&&evidence.assessmentCompleteness||67;
      hybrid.audioMeasured=!!(evidence&&evidence.audioAnalyzed);
      hybrid.speakingAssessment=speakingEvidenceEngine()&&speakingEvidenceEngine().normalizeAssessment?speakingEvidenceEngine().normalizeAssessment(Object.assign({},ai||{},{overallScore:hybrid.overallScore,topicScore:local.topicScore,taskCompletionScore:local.requiredScore,grammarScore:ai&&ai.grammarScore,vocabularyScore:ai&&ai.vocabularyScore,textFlowScore:ai&&ai.textFlowScore}),evidence):null;
      hybrid.scoreLabel=evidence&&evidence.audioAnalyzed?'Sprechleistung mit Audioanalyse':'Textbasierte Sprechaufgabe';
      hybrid.readiness=(hybrid.passed?'Textbasierter Sprechauftrag bestanden':'Textbasierter Sprechauftrag nicht bestanden')+(evidence&&evidence.audioAnalyzed?' · Audioanalyse vorhanden':' · Aussprache/Intonation offen');
    }
  ...
  function partBreakdown(session, bp){
    ...
    return partNames.map(function(part){
      ...
      return {
        ...
        weight:calibrationEngine()&&calibrationEngine().getWeights?Number(calibrationEngine().getWeights(bp.level||session.level||'B1')[part]||0):20,
        score:score,
        minScore:min,
        passed:passed,
        status:status,
  ...
  function buildTeacherSummary(final){
    ...
    if(final.passed && final.readinessEvidence !== 'single-variant' && final.readinessProbability >= 80) return 'Der Teilnehmer zeigt eine stabile Leistung über alle Prüfungsteile. Das Ergebnis spricht für echte Prüfungsreife, sofern die Leistung wiederholbar bleibt.';
  ...
  function computeFinal(session){
    ...
    var bp = getBlueprint(level) || { level:level, passScore:70, safePassScore:82, partMinScore:55, parts:{} };
    var breakdown = partBreakdown(session, bp);
    var available = breakdown.filter(function(x){ return typeof x.score === 'number'; });
    var scoreMap = available.reduce(function(map,x){ map[x.part]=x.score; return map; },{});
    var calibration = calibrationEngine();
    var overall = available.length ? (calibration&&calibration.weightedOverall?calibration.weightedOverall(scoreMap,level):round(available.reduce(function(sum,x){ return sum+x.score; },0)/available.length)) : 0;
    ...
    var margins = available.map(function(x){return Number(x.score)-Number(x.minScore);});
    var minPartMargin = margins.length?Math.min.apply(Math,margins):-100;
    var helperMode = breakdown.some(function(x){return !!(x.result&&x.result.helperMode);});
    var localOnlyFreeParts = ['writing','speaking'].some(function(part){var r=session.parts&&session.parts[part]; return !!(r && (!r.ai || r.aiError));});
    var evidence=session.stabilityEvidence||{};
    var confidence = calibration&&calibration.confidence ? calibration.confidence({overall:overall,passScore:bp.passScore,minPartMargin:minPartMargin,completedParts:completedParts,totalParts:expectedTotal,passed:passed,helperMode:helperMode,localOnlyFreeParts:localOnlyFreeParts,distinctPassedVariants:Number(evidence.distinctPassedVariants||0)}) : readinessBand(overall,passed,weakParts,completedParts,bp);
    ...
    var verdict;
    if(completedParts<expectedTotal) verdict='Es fehlen noch Prüfungsteile. Eine Trainingsprognose ist erst nach vollständiger Abgabe sinnvoll.';
    else if(passed && confidence.evidence==='single-variant') verdict='Die Variante wurde bestanden. Die Prognose bleibt vorläufig, bis mindestens eine weitere unterschiedliche Variante ohne kritischen Teilbereich bestanden wurde.';
    else if(passed) verdict='Die Leistung wurde über mehrere Varianten stabilisiert. Sie bleibt eine interne Trainingsprognose und ersetzt keine offizielle Prüfung.';
    else verdict='Die Simulation zeigt noch kritische Teilbereiche. Vor einer echten Prüfung sollte gezielt weitertrainiert und anschließend eine neue Variante vollständig bearbeitet werden.';
    var final = {
      level:level,
      completedParts:completedParts,
      totalParts:expectedTotal,
      overallScore:overall,
      scoreMethod:'weighted-parts',
      partWeights:calibration&&calibration.getWeights?calibration.getWeights(level):{reading:20,listening:20,grammar:20,writing:20,speaking:20},
      passScore:bp.passScore,
      safePassScore:bp.safePassScore,
      passed:passed,
      readiness:confidence.label || (passed?'Vorläufig positive Trainingsprognose':'Noch nicht prüfungsbereit'),
      readinessProbability:Number(confidence.probability||0),
      readinessEvidence:confidence.evidence||'single-variant',
      readinessCap:Number(confidence.cap||0),
      readinessVerdict:verdict,
      weakParts:weakParts,
      partBreakdown:breakdown.map(function(item){
        return { part:item.part, label:item.label, weight:item.weight, score:item.score, minScore:item.minScore, passed:item.passed, status:item.status, risk:item.risk, readiness:item.readiness, hardFailReasons:item.hardFailReasons, missingPoints:item.missingPoints, aiWeaknesses:item.aiWeaknesses, aiStrengths:item.aiStrengths };
      }),
      calibration:{schema:calibration&&calibration.schema||null,version:calibration&&calibration.__version||null,timeModel:calibration&&calibration.timeModel?calibration.timeModel(level,bp):null,helperMode:helperMode,localOnlyFreeParts:localOnlyFreeParts,minPartMargin:minPartMargin,distinctPassedVariants:Number(evidence.distinctPassedVariants||0)},
      ...
      certificateNote:'Dies ist eine interne Hardmode-Sprachtestsimulation und kein offizielles Sprachzertifikat. Die Prozentangabe ist eine vorsichtige Trainingsprognose, keine Garantie für ein Prüfungsergebnis.',
      nextRequiredAction: passed ? 'Leistung mit mindestens einer weiteren unterschiedlichen Variante absichern.' : 'Schwache Teilbereiche gezielt wiederholen und Simulation mit neuer Variante erneut starten.',
      generatedAt:nowIso()
    };
  ...
  function validateB2FullExamQaScenarios(){
    ...
    return { ok:results.every(function(x){ return x.ok && x.totalParts === 5; }), phase:'G54.46.10', scenarios:results, checkedAt:nowIso() };
  }
  function validateB2HardmodeTotalQa(){
    ...
    return { ok:validation.ok && checks.every(function(c){ return c.ok; }), phase:'G54.46.10', validation:validation, checks:checks, localSamples:local, checkedAt:nowIso() };
  }
  ...
  function validateLevelDifferentiation(){
    ...
    return { ok:checks.every(function(c){ return c.ok; }), phase:'G54.46.10', checks:checks, samples:{ simpleB2:simpleB2, validA1:validA1, b2Full:b2Full }, checkedAt:nowIso() };
  }
