...
(function () {
  ...
  var VERSION = 'G54.50.2C-AUTH-BOOTSTRAP-2026-07-12';
  ...
  /* ── Einmaliger Admin-Bootstrap ── */
  async function registerBootstrapAdmin(email, password, nickname, bootstrapCode) {
    email=String(email||'').trim(); password=String(password||''); nickname=String(nickname||'').trim(); bootstrapCode=String(bootstrapCode||'').trim();
    if(!email||!password||!nickname||!bootstrapCode) return {ok:false,reason:'missing-fields',message:'Bitte alle Bootstrap-Felder ausfüllen.'};
    if(password.length<8) return {ok:false,reason:'weak-password',message:'Passwort muss mindestens 8 Zeichen haben.'};
    var fb=null, fbUser=null, createdNow=false;
    try{
      fb=await loadFb();
      try{
        var created=await fb.authMod.createUserWithEmailAndPassword(fb.auth,email,password);
        fbUser=created.user; createdNow=true;
      }catch(createError){
        if(createError&&createError.code==='auth/email-already-in-use'){
          var signed=await fb.authMod.signInWithEmailAndPassword(fb.auth,email,password);
          fbUser=signed.user;
        }else throw createError;
      }
      state.firebaseUser=fbUser;
      try{if(!fbUser.emailVerified) await fb.authMod.sendEmailVerification(fbUser);}catch(_verificationError){}
      var sec=window.EGTSecurityContext;
      if(!sec||typeof sec.call!=='function') throw new Error('Sicherheitskontext ist nicht bereit.');
      await sec.refreshClaims(false);
      var result=await sec.call('bootstrapAdmin',{
        courseId:cfg().courseId||'course_2026_gk',nickname:nickname,bootstrapCode:bootstrapCode
      },{allowWithoutAppCheck:true});
      await sec.refreshClaims(true);
      var claimInfo=await trustedClaims(true);
      if(claimInfo.role!=='admin'||!claimInfo.trusted) throw new Error('Admin-Claim konnte nicht bestätigt werden. Bitte neu einloggen.');
      var session={
        source:'firebase-auth',role:'admin',nickname:nickname,code:'',groupId:'ADMIN',
        profileId:fbUser.uid,firebaseUid:fbUser.uid,email:email,emailVerified:fbUser.emailVerified,
        trustedClaims:true,claimsIssuedAt:nowIso(),bootstrapAdmin:true,updatedAt:nowIso()
      };
      saveSession(session);
      try{localStorage.setItem('novura_admin_bootstrap_completed_v1','1');}catch(_storageError){}
      return {ok:true,session:session,role:'admin',bootstrapCompleted:true,emailVerificationRequired:!!(result&&result.emailVerificationRequired)};
    }catch(e){
      if(createdNow&&fbUser){
        try{await fb.authMod.deleteUser(fbUser);}catch(_deleteError){}
      }
      var details=e&&e.details&&typeof e.details==='object'?e.details:{};
      var msg={
        'auth/invalid-email':'Ungültige E-Mail-Adresse.',
        'auth/wrong-password':'Falsches Passwort für das bestehende Konto.',
        'auth/invalid-credential':'E-Mail oder Passwort falsch.',
        'auth/weak-password':'Passwort zu schwach (min. 8 Zeichen).',
        'functions/already-exists':'Der Admin-Bootstrap wurde bereits abgeschlossen.',
        'functions/permission-denied':'Bootstrap-Code ist ungültig.',
        'functions/unavailable':'Firebase ist vorübergehend nicht erreichbar. Bitte kurz warten und erneut versuchen.'
      }[e&&e.code]||(e&&e.message)||'Admin-Bootstrap fehlgeschlagen.';
      if(details.action==='enable-firestore'){
        msg='Die Firestore-Datenbank ist noch nicht bereit. Aktiviere Firestore im Firebase-Projekt und deploye anschließend Firestore sowie Functions erneut.';
      }
      if(details.diagnosticId){
        msg+=' Diagnose: '+String(details.diagnosticId)+(details.stage?' · '+String(details.stage):'');
      }
      return {ok:false,reason:e&&e.code||'bootstrap-error',message:msg,details:details};
    }
  }
  ...
  window.EGTAuthEngine = Object.freeze({
    ...
    startDemo: startDemo,
    register: register,
    registerBootstrapAdmin: registerBootstrapAdmin,
    login: login,
    logout: logout,
