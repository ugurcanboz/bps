...
(function(){
  ...
  async function call(functionName,payload,options){
    ...
    var bootstrapException=options.allowWithoutAppCheck===true && functionName==='bootstrapAdmin';
    if(cfg().requireAppCheck && !state.appCheckReady && !isDevelopment() && !bootstrapException) throw new Error('App Check ist nicht bereit. Privilegierte Cloudaktion wurde sicher blockiert.');
