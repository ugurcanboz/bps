...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      '.egt-live-visual-qa{position:fixed;z-index:2147483600;right:12px;bottom:calc(12px + env(safe-area-inset-bottom,0px));width:min(420px,calc(100vw - 24px));max-height:min(72vh,640px);overflow:hidden;border:1px solid rgba(148,163,184,.45);border-radius:18px;background:rgba(15,23,42,.96);color:#f8fafc;box-shadow:0 20px 60px rgba(0,0,0,.42);font:13px/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}',
      '.egt-live-visual-qa *{box-sizing:border-box}',
      '.egt-live-visual-qa__head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid rgba(148,163,184,.25)}',
      '.egt-live-visual-qa__title{font-weight:800;font-size:14px}.egt-live-visual-qa__meta{font-size:11px;color:#cbd5e1}',
      '.egt-live-visual-qa__body{padding:12px 14px;overflow:auto;max-height:calc(min(72vh,640px) - 116px)}',
      '.egt-live-visual-qa__actions{display:flex;gap:8px;flex-wrap:wrap;padding:10px 14px;border-top:1px solid rgba(148,163,184,.25)}',
      '.egt-live-visual-qa button{min-height:36px;border:0;border-radius:999px;padding:8px 12px;background:#e2e8f0;color:#0f172a;font-weight:700;cursor:pointer}',
      ...
      '@media(max-width:520px){.egt-live-visual-qa{left:8px;right:8px;bottom:calc(8px + env(safe-area-inset-bottom,0px));width:auto;border-radius:16px}.egt-live-visual-qa__summary{grid-template-columns:repeat(3,1fr)}}'
    ].join('\n');
  ...
  function openOverlay() {
    ...
    el.innerHTML = '<div class="egt-live-visual-qa__head"><div><div class="egt-live-visual-qa__title">Live Visual QA</div><div class="egt-live-visual-qa__meta">' + VERSION + ' · echter Browser-Check</div></div><button type="button" data-action="close" aria-label="Overlay schließen">×</button></div><div class="egt-live-visual-qa__body" data-role="body">Bereit. Starte „Analyse prüfen“, um DOM, Layout, Touch-Ziele und sichtbare Analysebereiche zu prüfen.</div><div class="egt-live-visual-qa__actions"><button type="button" data-primary="true" data-action="run">Analyse prüfen</button><button type="button" data-action="copy">Report kopieren</button></div>';
