...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      '.egt-vdiff{position:fixed;z-index:2147483598;right:12px;top:12px;width:min(520px,calc(100vw - 24px));max-height:min(88vh,760px);overflow:hidden;border:1px solid rgba(168,85,247,.48);border-radius:18px;background:rgba(15,23,42,.97);color:#f8fafc;box-shadow:0 22px 72px rgba(0,0,0,.48);font:13px/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}',
      '.egt-vdiff *{box-sizing:border-box}.egt-vdiff__head{display:flex;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid rgba(148,163,184,.25)}',
      '.egt-vdiff__title{font-weight:850;font-size:14px}.egt-vdiff__meta{font-size:11px;color:#cbd5e1}.egt-vdiff__body{padding:12px 14px;overflow:auto;max-height:calc(min(88vh,760px) - 150px)}',
      '.egt-vdiff__actions{display:flex;gap:8px;flex-wrap:wrap;padding:10px 14px;border-top:1px solid rgba(148,163,184,.25)}.egt-vdiff button{min-height:36px;border:0;border-radius:999px;padding:8px 12px;background:#e2e8f0;color:#0f172a;font-weight:800;cursor:pointer}.egt-vdiff button[data-primary="true"]{background:#c4b5fd;color:#111827}',
      ...
      '@media(max-width:560px){.egt-vdiff{left:8px;right:8px;top:8px;width:auto;border-radius:16px}.egt-vdiff__grid{grid-template-columns:repeat(2,1fr)}}'
    ].join('\n');
  ...
  function onAction(ev) {
    ...
    if (!action) return;
    if (action === 'close') { state.overlay.remove(); return; }
