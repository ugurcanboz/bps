...
(function () {
  ...
  function collectOverflowFindings() {
    ...
    qsa('body *').forEach(function (el) {
      ...
      if (el.scrollWidth > el.clientWidth + 2 && out.clipped.length < 30) {
        var cs = getComputedStyle(el);
        var hasAllowedInnerScroller = false;
        try {
          hasAllowedInnerScroller = !!(el.querySelector && Array.prototype.some.call(el.querySelectorAll('.egt-portal-tab-group,.egt-admin-tabs,.egt-portal-tabs,.egt-portal-tabbar'), isAllowedHorizontalScroller));
        } catch (e) { hasAllowedInnerScroller = false; }
        if (/hidden|clip/i.test(cs.overflowX || cs.overflow || '') && !hasAllowedInnerScroller) out.clipped.push({ selector: selectorFor(el), text: clampText(textOf(el), 80), scrollWidth: el.scrollWidth, clientWidth: el.clientWidth });
      }
    });
    return out;
  }
  ...
  /* G54.43.8I · Admin Portal Tabbar Residual Overflow Fix
  ...
  function collectVisualMonsterFindings() {
    ...
    return {
      version: 'G54.43.8I-monster-detector',
      summary: {
        total: all.length,
        fails: failCount,
        warnings: warnCount,
