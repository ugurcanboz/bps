...
(function () {
  ...
  function ensureStyles() {
    ...
    style.textContent = [
      '.egt-vsc{position:fixed;z-index:2147483599;left:12px;bottom:calc(12px + env(safe-area-inset-bottom,0px));width:min(460px,calc(100vw - 24px));max-height:min(76vh,680px);overflow:hidden;border:1px solid rgba(59,130,246,.45);border-radius:18px;background:rgba(2,6,23,.96);color:#f8fafc;box-shadow:0 22px 70px rgba(0,0,0,.45);font:13px/1.45 system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}',
      '.egt-vsc *{box-sizing:border-box}.egt-vsc__head{display:flex;justify-content:space-between;gap:10px;padding:12px 14px;border-bottom:1px solid rgba(148,163,184,.24)}',
      '.egt-vsc__title{font-weight:850;font-size:14px}.egt-vsc__meta{font-size:11px;color:#cbd5e1}.egt-vsc__body{padding:12px 14px;overflow:auto;max-height:calc(min(76vh,680px) - 130px)}',
      '.egt-vsc__actions{display:flex;gap:8px;flex-wrap:wrap;padding:10px 14px;border-top:1px solid rgba(148,163,184,.24)}.egt-vsc button{min-height:36px;border:0;border-radius:999px;padding:8px 12px;background:#e2e8f0;color:#0f172a;font-weight:800;cursor:pointer}.egt-vsc button[data-primary="true"]{background:#93c5fd;color:#07111f}',
      ...
      '@media(max-width:520px){.egt-vsc{left:8px;right:8px;bottom:calc(8px + env(safe-area-inset-bottom,0px));width:auto;border-radius:16px}.egt-vsc__grid{grid-template-columns:repeat(3,1fr)}}'
    ].join('\n');
  ...
  function openOverlay() {
    ...
    el.innerHTML = '<div class="egt-vsc__head"><div><div class="egt-vsc__title">Visual-State-Capture</div><div class="egt-vsc__meta">' + VERSION + ' · QA Screenshot-/Layout-Recorder</div></div><button type="button" data-action="close" aria-label="Overlay schließen">×</button></div><div class="egt-vsc__body" data-role="body">Bereit. „State aufnehmen“ erzeugt einen lokalen Layout-Snapshot. „PNG aufnehmen“ nutzt optional die Browser-Screen-Capture-API und benötigt einen Nutzerklick.</div><div class="egt-vsc__actions"><button type="button" data-primary="true" data-action="capture">State aufnehmen</button><button type="button" data-action="png">PNG aufnehmen</button><button type="button" data-action="copy">JSON kopieren</button><button type="button" data-action="download">JSON laden</button></div>';
    document.body.appendChild(el);
    state.overlay = el;
    el.addEventListener('click', function (ev) {
      var action = ev.target && ev.target.getAttribute && ev.target.getAttribute('data-action');
      if (action === 'close') el.remove();
      if (action === 'capture') captureCurrentState({ reason: 'manual' });
      if (action === 'png') capturePngFromScreen();
      if (action === 'copy') copyLastCapture();
      if (action === 'download') downloadLastCapture();
    });
    return el;
  }
  ...
  function collectTouchTargets() {
    return qsa('button,a,select,input,textarea,[role="button"],.dock-item,.qa-clickable').filter(function (el) { return visible(el); }).slice(0, 220).map(function (el) {
      var r = rectOf(el);
      return {
        selector: selectorFor(el),
  ...
  function collectVisibleTextBlocks() {
    var blocks = qsa('h1,h2,h3,h4,p,li,button,a,label,.card,.analysis-v2-kpi,.analysis-v2-coach').filter(visible).map(function (el) {
