...
(function () {
  ...
  function openOverlay() {
    ...
    el.innerHTML = '<div class="egt-vsc__head"><div><div class="egt-vsc__title">Visual-State-Capture</div><div class="egt-vsc__meta">' + VERSION + ' · QA Screenshot-/Layout-Recorder</div></div><div class="egt-vsc__head-actions"><button type="button" data-action="minimize" aria-label="Overlay minimieren">−</button><button type="button" data-action="close" aria-label="Overlay schließen">×</button></div></div><div class="egt-vsc__body" data-role="body">Bereit. „State aufnehmen“ erzeugt einen lokalen Layout-Snapshot. „PNG aufnehmen“ nutzt optional die Browser-Screen-Capture-API und benötigt einen Nutzerklick.</div><div class="egt-vsc__actions"><button type="button" data-primary="true" data-action="capture">State aufnehmen</button><button type="button" data-action="png">PNG aufnehmen</button><button type="button" data-action="copy">JSON kopieren</button><button type="button" data-action="download">JSON speichern</button></div>';
    ...
    el.addEventListener('click', function (ev) {
      ...
      if (!btn && el.getAttribute('data-minimized') === 'true') {
      ...
      ev.preventDefault();
      ev.stopPropagation();
      if (action === 'close') { el.remove(); state.overlay = null; return; }
      if (action === 'minimize') {
