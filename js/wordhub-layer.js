...
(function () {
  ...
  var MODULES = [
    ...
    },
    {
      ...
      desc: 'Interaktiver Python-Lernpfad mit Prüfungen, Tests und dem Code-Coach.',
      tags: ['Python', 'Code-Coach', 'Prüfungen'],
      modes: [],
      startLabel: 'Quest starten',
      isPythonQuestCard: true
    },
  ...
  function renderSystemTab (el) {
    ...
    el.innerHTML =
      '<div class="wh-analysis-wrap">' +
        ...
        '</div>' +
        '<div class="wh-footer-version">Version 9.2.0 · Learning Coach Stable</div>' +
      '</div>';
  ...
  /* ── OCR Scanner · V9.1.x Preview-first stable workflow ───── */
  ...
  function injectScanStyles () {
    ...
    st.textContent =
      ...
      '.wh-scan-actions{display:flex;gap:10px;flex-wrap:wrap;align-items:center}' +
      ...
      '.wh-scan-mini-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}@media(max-width:420px){.wh-scan-mini-grid{grid-template-columns:1fr}.wh-scan-actions button{flex:1 1 auto}}';
      ...
      '<div class="wh-sheet-body">' +
        ...
        '<div class="wh-branch-list">' +
          BRANCHES.map(function (b) {
            var isSel = b.id === currentBranch ? ' is-active' : '';
            return (
              '<button class="wh-branch-select-btn' + isSel + '" data-branch="' + b.id + '">' +
