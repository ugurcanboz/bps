/* Eignungstest-Trainer · Apple-Style WordHub UI Layer
...
(function () {
  ...
  var VERSION = 'G22-structure-stability-100';
  ...
  var BRANCHES = [
    ...
    { id: 'sozial',    label: 'Sozialpädagogik / Erziehung', icon: '🤝' },
    { id: 'wissen',    label: 'Allgemeinwissen / Basis',     icon: '🌐' }
  ];
  ...
  var MODULES = [
    ...
    },
    {
      ...
      iconClass: 'c-green',
      kicker: 'Coding',
      title: 'Python Quest Academy',
      desc: 'Lerne Python-Grundlagen von Grund auf. Meistere Lektionen, löse Coding-Aufgaben und bestehe Prüfungen.',
      tags: ['Programmierung', 'Syntax', 'Logik'],
      modes: ['python_quest'],
      startLabel: 'Quest starten',
      branches: ['it']
    }
  ];
  ...
  function openSheet (mod) {
    ...
    sheet.innerHTML =
      ...
      '<div class="wh-sheet-body">' +
        ...
        (mod.branches ? '<div class="wh-structure-note">G22-Sortierung: fachlich geprüft · Fremdmodule ausgeschlossen.</div>' : '') +
  ...
  function renderGrid () {
    ...
    Array.prototype.forEach.call(grid.querySelectorAll('.wh-mod-card'), function (card) {
      function activate () {
        ...
        } else if (mod.id === 'python_quest') {
          try {
            if (window.PythonQuest && typeof window.PythonQuest.open === 'function') {
              window.PythonQuest.open('dashboard');
            } else {
              notice('Python Quest nicht bereit.');
            }
          } catch (e) { notice('Fehler beim Öffnen.'); }
        } else {
          openSheet(mod);
  ...
  function openBranchSelectionSheet (allowClose) {
    ...
    sheet.innerHTML =
      ...
      '<div class="wh-sheet-sep"></div>' +
      '<div class="wh-sheet-body">' +
        '<p class="wh-sheet-desc">Wähle zuerst eine Fachrichtung. Danach zeigt die Startseite nur passende Kacheln plus gemeinsame Grundlagen wie Mathe, Logik, Sprache, Konzentration und Simulation.</p>' +
    ...
    Array.prototype.forEach.call(sheet.querySelectorAll('.wh-branch-select-btn'), function (btn) {
      btn.addEventListener('click', function () {
        var branchId = btn.getAttribute('data-branch');
        localStorage.setItem('bps_branch', branchId);
        notice('Fachrichtung: ' + btn.querySelector('.wh-branch-label').textContent);
        closeSheet();
        updateTopBar();
        renderGrid();
  ...
  function hideLegacy (shell) {
    // G22.2: no-op. Legacy hiding caused the Hero/Card regression.
    restorePremiumDom();
  }

  /* ── App Shell DISABLED FOR G22.2 ─────────────────────────────
     Wichtig: Die alte WordHub-Shell (#whShell) war die Ursache der Design-Regression.
     Sie hat die neue Premium-/Hero-Startseite per .wh-hidden-legacy ausgeblendet.
     WordHub bleibt nur noch als Infrastruktur für Notice, Sheet und Legacy-APIs aktiv.
  */
  ...
  function restorePremiumDom () {
    var oldShell = $('whShell');
    ...
    Array.prototype.forEach.call(start.children, function (child) {
      child.classList.remove('wh-hidden-legacy');
  ...
  function buildShell () {
    ensureInfra();
    restorePremiumDom();
    window.__WORDHUB_SHELL_DISABLED__ = true;
    window.__WORDHUB_REASON__ = 'G22.2: Premium Hero/Card UI is authoritative; legacy whShell renderer disabled.';
    setTimeout(function () { restorePremiumDom(); }, 0);
    setTimeout(function () { restorePremiumDom(); }, 350);
  }
